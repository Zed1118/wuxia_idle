...
void main() {
  ...
  test('每个有敌人的 isBossStage 关卡 enemyTeam 恰有 ≥1 个 isBoss 敌人', () {
    ...
    final bossStages = stages
        .where((s) => s.isBossStage && s.enemyTeam.isNotEmpty)
        .toList();
    expect(bossStages, isNotEmpty, reason: 'production 应有带敌人的 boss stage');
    for (final s in bossStages) {
      expect(
        s.enemyTeam.where((e) => e.isBoss).length,
  ...
  test('恰好标注 36 个 boss 敌人(production 红线)', () {
    final stages = GameRepository.instance.stageDefs.values;
    final total = stages.fold<int>(
      0,
      (n, s) => n + s.enemyTeam.where((e) => e.isBoss).length,
    );
    expect(total, 36, reason: 'Ch17 +2(卷沙手章中 + 沙海领路人章末)');
  });
}
