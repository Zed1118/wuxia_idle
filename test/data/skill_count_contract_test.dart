...
void main() {
  ...
  test('skills 209 + encounter skills 40 = merged SkillDef 249', () async {
    ...
    expect(genericIds, hasLength(209));
    ...
    expect(mergedIds, hasLength(249));
    ...
    expect(gdd, contains('| 通用 / 战斗招式（skills.yaml） | 209 招 |'));
    expect(gdd, contains('| 奇遇专属武学领悟招式（encounter_skills.yaml） | 40 招 |'));
    expect(gdd, contains('| 招式总池（SkillDef） | 249 招（209 + 40） |'));
  });
}
