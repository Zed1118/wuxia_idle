...
''';
    ...
    test(
      'P1 #44 · continued_lore_obtained / continued_lore_boss_defeated 池解析',
      () async {
        Future<String> mockLoader(String path) async => '''
id: weapon_test
name: 测试剑
...
''';
  ...
  group('data/lore/ 35 个真实 yaml 红线', () {
    test('全部解析成功 + 文件名 == yaml.id + default_lore 非空', () async {
      ...
      for (final f in files) {
        ...
        expect(
          c.defaultLore.isNotEmpty,
          isTrue,
          reason: '$basename default_lore 段为空',
        );
        for (final seg in c.defaultLore) {
          expect(seg.text.isNotEmpty, isTrue, reason: '$basename 有空 text 段');
        }
      }
    });
  });
  ...
  group('P1 #44 · 35 件 continued_lore 池红线', () {
    test('5 strict red line:漏件 / 占位符白名单 / 占位符分池 / 长度 / 网游词', () async {
      final dir = Directory('data/lore');
      final files =
          dir
              .listSync()
      ...
      for (final f in files) {
        ...
        void validatePool(
          List<LoreSegment> pool,
          String poolName,
          Set<String> poolAllowed,
        ) {
          for (var i = 0; i < pool.length; i++) {
            ...
            for (final m in placeholderPattern.allMatches(text)) {
              ...
              expect(
                allPlaceholders.contains(variable),
                isTrue,
                reason: '$basename / $poolName / 条 $i: 未约定占位符 {$variable}',
              );
              expect(
                poolAllowed.contains(variable),
                isTrue,
                reason: '$basename / $poolName / 条 $i: 占位符 {$variable} 不属于此池',
              );
            }
        ...
        validatePool(
          c.continuedLoreObtainedPool,
          'continued_lore_obtained',
          obtainedAllowed,
        );
        validatePool(
          c.continuedLoreBossDefeatedPool,
          'continued_lore_boss_defeated',
          bossDefeatedAllowed,
        );
      }
    ...
    test('soft · 文风审计(emoji / < 10 字 / 同池重复)— print warning 不 fail', () async {
      final dir = Directory('data/lore');
      final files =
          dir
              .listSync()
