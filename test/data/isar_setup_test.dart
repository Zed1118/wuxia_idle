...
void main() {
  ...
  group('IsarSetup', () {
    ...
    test('波A A4 迁移:0.17 旧档奇遇 unlock 池并入 skillUnlockProgress(幂等)', () async {
      ...
      await IsarSetup.init(directory: tempDir, inspector: false);
      final save = (await IsarSetup.instance.saveDatas.get(0))!;
      expect(save.saveVersion, '0.29.0', reason: '迁移后升版到当前(0.29.0)');
      expect(
    ...
    test('Phase 3 迁移:0.18 旧档升当前(BattleReplayRecord collection 已删,无数据迁移)',
      ...
      await IsarSetup.init(directory: tempDir, inspector: false);
      final save = (await IsarSetup.instance.saveDatas.get(0))!;
      expect(save.saveVersion, '0.29.0',
          reason: '升版 → 0.29.0(经伤势系统 Character 新字段升版段,无迁移动作纯标记)');
    });
    ...
    test('首次 init 应自动建 SaveData(id=0) 并填默认值', () async {
      ...
      expect(save.saveVersion, '0.29.0',
          reason: '新建存档写当前 saveVersion 0.29.0');
