...
void main() {
  ...
  group('§12.1 #7 v1.4 · combat.schools.gang_meng_quake', () {
    test('damage=500 + 穿透防御 + 不暴击 + 主攻击命中才触发', () {
      ...
      expect(q.piercesDefense, isTrue, reason: '穿透守方防御率(语义上是震的硬伤)');
      expect(
        q.piercesCritical,
        isTrue,
  ...
  group('§12.1 #7 v1.4 · combat.schools.yin_rou_internal_injury', () {
    test('N=3 守方 tick × 200/tick + 穿透防御 + 同源刷新覆盖', () {
      ...
      expect(i.damagePerTick, 200, reason: 'v1.4 决议:每 tick 扣 200 固定');
      expect(i.piercesDefense, isTrue, reason: '穿透防御 + 可致死');
      expect(i.stackRule, 'refresh', reason: '同源刷新(覆盖)不叠层 — 重复触发重置 turns');
      expect(i.followsMainHit, isTrue, reason: '主攻击命中才施加,闪避不施加');
    });
  });
  ...
  group('§12.1 #7 v1.4 · retreat.time_of_day_bonus[zhengWu]', () {
    test('+20% 乘到 internal_force_points + 仅 gangMeng 触发', () {
      ...
      expect(
        r.zhengWuYangSchoolMultiplier,
        closeTo(1.20, 0.001),
        reason: 'v1.4 决议:正午阳刚 +20%',
      );
      expect(
        r.zhengWuTargetAttribute,
        'internal_force_points',
        reason: '加成乘到 internalForcePoints 维度(刚猛=内力外放)',
      );
      expect(
        r.zhengWuAppliesToSchool,
