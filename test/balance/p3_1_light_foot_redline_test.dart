...
void main() {
  ...
  group('P3.1 §12.3 轻功对决 R5 跨地形红线压测', () {
    ...
    setUpAll(() async {
      tempDir = await Directory.systemTemp.createTemp('wuxia_r5_light_foot_');
    ...
    List<BattleCharacter> buildPlayerTeam({
      ...
      final mainTechDef = repo.techniqueDefs.values.firstWhere(
        ...
        orElse: () =>
            throw StateError('r5 light_foot: 找不到 ${tier.name} cap 心法 def'),
      );
    ...
    test('R5.1 5 关 × 50 种子分布 · leftWins+draws ≥ rightWins(平行支线玩家主导)', () {
      ...
      for (final (stageId, tier, layer) in stageMatrix) {
        ...
        for (var seed = 0; seed < 50; seed++) {
          final finalState = ground.runToEnd(
            modified,
            numbers,
            rng: Random(seed),
          );
    ...
    test('R5.2 terrain modifier cap e2e · clamp ≤0.95 + §5.4 红线不动', () {
      final repo = GameRepository.instance;
      final numbers = repo.numbers;
      final lightFootDef = numbers.lightFoot;
    ...
    test('R5.3 unlock 链 e2e · stage_06_05 → light_foot_01 → ... → 05 顺序', () {
      ...
      expect(LightFootService.orderedStageIds(lightFootDef), [
        'stage_light_foot_01',
        'stage_light_foot_02',
        'stage_light_foot_03',
        'stage_light_foot_04',
