...
///
/// 验证「玩家方满 build vs zongShi 跨阶 boss 三人组」的跨阶难度合理体现
...
///   章末跨 1 阶 zongShi boss 期望「拉锯格局」(玩家方满 build 撑得住但赢不易,
///   draws 是合理结果)— 暗示玩家需升 zongShi 才能稳赢。本 R5 验「玩家方
///   不一边倒被压垮」,不验「玩家方稳赢」。
///
/// **玩家方 build**:
///   - jueDing·dengFeng + const 10 + jueDing cap 装备(zhongQi 阶)hp_max 满 3 件
///   - jianghu 心法 jiJing 层(满修炼度 → 伤害倍率 3.0)
///   - 派生 maxHp / totalEquipmentAttack / speed 走 CharacterDerivedStats
///
/// **敌方 build**:走 `data/stages.yaml stage_05_05` 现行 yaml
///   (西凉霸主三弟子 zongShi·qiMeng·yinRou + 中州论剑顶 jueDing·dengFeng·gangMeng
...
void main() {
  ...
  group('Ch5 stage_05_05 R5 跨阶红线压测', () {
    late Directory tempDir;

    setUpAll(() async {
      tempDir = await Directory.systemTemp.createTemp('wuxia_r5_ch5_redline_');
    ...
    /// 构造 jueDing·dengFeng 满 build 的 BattleCharacter 三人队。
    ///
    ...
    List<BattleCharacter> buildR5Players() {
      ...
      BattleCharacter buildOne(int slotIndex, TechniqueSchool school) {
        ...
        final mainTech = Technique.create(
          ...
          ownerCharacterId: -200 - slotIndex,
          tier: jiangHuTechDef.tier,
    ...
    test(
      ...
      () async {
        ...
        // 需升 zongShi 才能稳赢,jueDing·dengFeng 满 build 不该 100% leftWin)被破坏。
        expect(
          rightWins + draws,
