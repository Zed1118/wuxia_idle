...
void main() {
  group('W18-A1.2 hot-loop 升级版:applySynergy 红线 cap 压测', () {
    ...
    const tierBaseStats =
        <RealmTier, ({int baseMaxHp, int baseMaxIf, double baseDefRate})>{
          RealmTier.xueTu: (
            baseMaxHp: 5850,
            baseMaxIf: 1100,
            baseDefRate: 0.05,
    ...
    void expectRedLines(BattleCharacter ch, {required String label}) {
      ...
      expect(ch.speed, greaterThan(0), reason: '$label speed > 0(防战斗卡死)');
      expect(
    ...
    for (final tierEntry in tierBaseStats.entries) {
      for (final synEntry in synergyMultipliers.entries) {
        test('${tierEntry.key.name} × ${synEntry.key} → 6 字段 ≤ §5.4 红线', () {
          ...
          expectRedLines(
            result,
            label: '${tierEntry.key.name}×${synEntry.key}',
          );
        });
      }
    }
    ...
    test(
      '极端 1·历史回归:wushen 人造旧值 base maxHp 21800 + hpPct 0.20 → applySynergy cap 仍兜到 20000',
      () {
    ...
    test(
      '极端 1·新基线回归:wushen 真实极值 16550 + hpPct 0.20 → 19860 ≤ 20000 不触发 cap',
      () {
    ...
    test(
      '极端 3:wushen base defRate 0.35 + 假想 synergy defensePct 0.80 → clamp 0.95',
      () {
    ...
    test(
      'hot-loop C1:synergy_gang_yin_hu_zhi (def 0.15 + hp 0.15) 各维 ≤ 0.30 + 实战 ≤ §5.4 红线',
      () {
    ...
    test(
      'hot-loop C2:synergy_ling_gang_hui_liu (atk 0.10 + spd 0.20) 各维 ≤ 0.30 + 实战 ≤ §5.4 红线',
      () {
    ...
    test(
      'hot-loop C3:synergy_ling_yin_hua_die (spd 0.20 + def 0.10 + IFgrowth 0.15) ≤ §5.4 红线',
      () {
    ...
    test(
      'hot-loop C4:synergy_long_hu_jiao_tai (atk 0.15 + spd 0.15 + hp 0.15) ≤ §5.4 红线',
      () {
        const m = SynergyMultipliers(
          attackPct: 0.15,
          speedPct: 0.15,
    ...
    test(
      'hot-loop C5:synergy_you_ying_chuan_hua (atk 0.25 + IFmax 0.15) ≤ §5.4 红线',
      () {
        const m = SynergyMultipliers(
          attackPct: 0.25,
    ...
    test(
      'hot-loop C6:synergy_jin_gang_bu_huai (def 0.20 + hp 0.20 + IFmax 0.10) ≤ §5.4 红线',
      () {
        const m = SynergyMultipliers(
          defensePct: 0.20,
          hpPct: 0.20,
