...
///
/// 验证「玩家方满 build vs wuSheng 跨阶 boss 三人组」的跨阶难度合理体现
...
///   - ❌ 不写「胜率 60% / 32 win」之类瞬时数字
///
/// **跨阶设计意图**(GDD §3 + memory `feedback_wuxia_boss_balance_crosstier`):
///   章末跨 1 阶 wuSheng boss 期望「拉锯格局」(玩家方满 build 撑得住但赢不易,
///   draws 是合理结果)— 暗示玩家需升 wuSheng 才能稳赢。本 R5 验「玩家方
///   不一边倒被压垮」,不验「玩家方稳赢」。
///
/// **玩家方 build**:
///   - zongShi·dengFeng + const 10 + zongShi cap 装备(baoWu 阶)hp_max 满 3 件
///   - shichuan 心法 jiJing 层(满修炼度 → 伤害倍率 3.0)
///   - 派生 maxHp / totalEquipmentAttack / speed 走 CharacterDerivedStats
///
/// **敌方 build**:走 `data/stages.yaml stage_06_05` 现行 yaml
///   (西凉霸主 wuSheng·qiMeng·yinRou + 三弟子刚猛 zongShi·dengFeng·gangMeng
///   + 三弟子灵巧 zongShi·dengFeng·lingQiao)。
...
void main() {
  ...
  group('Ch6 stage_06_05 R5 跨阶红线压测', () {
    late Directory tempDir;

    setUpAll(() async {
      tempDir = await Directory.systemTemp.createTemp('wuxia_r5_ch6_redline_');
    ...
    /// 构造 zongShi·dengFeng 满 build 的 BattleCharacter 三人队。
    ///
    ...
    List<BattleCharacter> buildR5Players() {
      ...
      // zongShi cap 心法(shiChuanShenGong 阶)— 找任一 shichuan 阶 main role 可用心法
      ...
      BattleCharacter buildOne(int slotIndex, TechniqueSchool school) {
        ...
        final mainTech = Technique.create(
          ...
          ownerCharacterId: -300 - slotIndex,
          tier: shiChuanTechDef.tier,
