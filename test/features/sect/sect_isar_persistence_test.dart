...
void main() {
  ...
  group('Sect / SectEvent Isar 持久化', () {
    ...
    test('R4.1 SectSchema/SectEventSchema 加入 _allSchemas → init 不抛', () async {
    ...
    test('R4.2 saveVersion 已升当前版本', () async {
      await IsarSetup.init(directory: tempDir, inspector: false);
    ...
    test(
      'R4.6 B1 月度 tick compute → writeTxn 落库 e2e(真 Isar · 触发+decay)',
      () async {
        await IsarSetup.init(directory: tempDir, inspector: false);
        final isar = IsarSetup.instance;
    ...
    test('R4.4 SectEvent put 后 filter().statusEqualTo(pending) 查得到', () async {
      await IsarSetup.init(directory: tempDir, inspector: false);
      final isar = IsarSetup.instance;
    ...
    test(
      'R4.5 SectEvent resolve → status=resolved 写库 → historical 查得到',
      () async {
        await IsarSetup.init(directory: tempDir, inspector: false);
        final isar = IsarSetup.instance;
