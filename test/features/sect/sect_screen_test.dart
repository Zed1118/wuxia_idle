...
void main() {
  ...
  ProviderScope withScope({
    ...
    overrides: [
      ...
      activeSectEventsProvider.overrideWith((ref) => Stream.value(active)),
      historicalSectEventsProvider.overrideWith(
        (ref) => Stream.value(historical),
      ),
    ],
    child: child,
  );
  ...
  group('P3.4 sect_screen widget · 顶部 + tab + 空状态', () {
    ...
    testWidgets('R4.2 TabBar 2 标签 + 空 active list 提示「当前无门派事件」', (tester) async {
      await tester.pumpWidget(
        withScope(
          sect: defaultSect(),
          child: const MaterialApp(home: SectScreen()),
  ...
  group('P3.4 sect_screen widget · active 注入 + AsyncValue 三态', () {
    ...
    testWidgets('R4.5 sect=null → 显「门派尚未创建」兜底文案', (tester) async {
      await tester.pumpWidget(
        withScope(sect: null, child: const MaterialApp(home: SectScreen())),
      );
      await tester.pump();
