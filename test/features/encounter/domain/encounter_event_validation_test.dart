...
void main() {
  ...
  group('C2 奇遇 events 加载层强校验', () {
    ...
    test(
      'events choice outcome_id 不在 outcomeMapping 时抛 StateError (防奖励静默丢失)',
      () async {
        ...
        await expectLater(
          ...
          throwsA(
            isA<StateError>().having(
              ...
              'message',
              allOf(
                contains('nonexistent_outcome_id'),
                contains('outcomeMapping'),
              ),
            ),
          ),
        );
...
''');
    ...
    test(
      '真实 data/events 全 68 条对齐 → loadAllDefs 不抛 (回归守门 · 68/68 干净)',
      () async {
