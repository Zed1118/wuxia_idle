...
void main() {
      ...
      ProviderScope(
        overrides: [
          ...
          activeCharacterIdsProvider
              .overrideWith((ref) async => player == null ? [] : [player.id]),
          if (player != null)
            characterByIdProvider(player.id)
  ...
  testWidgets('装备 Tab 按部位分组：武器/护甲/饰品三段标题', (tester) async {
    await pumpInv(tester, equipments: [
      mkEq(id: 10, tier: EquipmentTier.shenWu, slot: EquipmentSlot.weapon, enhanceLevel: 12),
      mkEq(id: 11, tier: EquipmentTier.liQi, slot: EquipmentSlot.armor, enhanceLevel: 5),
      mkEq(id: 12, tier: EquipmentTier.xunChang, slot: EquipmentSlot.accessory, enhanceLevel: 3),
    ]);
  ...
  testWidgets('真实 defId → 格子显示装备名 + 图标', (tester) async {
    ...
    final eq = mkEq(
      id: 100, tier: def.tier, slot: def.slot, defId: def.id);
    await pumpInv(tester, equipments: [eq]);
    expect(find.text('龙泉剑'), findsOneWidget,
        reason: '格子应渲染 EquipmentDef.name');
  ...
  testWidgets('境界不达装备 → 灰化 + 锁图标', (tester) async {
    // 神物装备 + 学徒境界玩家 → 不可装备
    final eq = mkEq(
        id: 20, tier: EquipmentTier.shenWu, slot: EquipmentSlot.weapon,
        defId: 'weapon_shenwu_tian_wen_jian');
    await pumpInv(tester,
        equipments: [eq], player: mkCharacter(realmTier: RealmTier.xueTu));
  ...
  testWidgets('境界达标装备 → 无锁', (tester) async {
    final eq = mkEq(
        id: 21, tier: EquipmentTier.xunChang, slot: EquipmentSlot.weapon,
        defId: 'weapon_xunchang_tie_jian');
    await pumpInv(tester,
        equipments: [eq], player: mkCharacter(realmTier: RealmTier.wuSheng));
  ...
  testWidgets('缺图/未知 defId → 走 EquipGlyph 占位不崩', (tester) async {
    ...
    expect(find.byType(EquipGlyph), findsWidgets,
        reason: '未知 defId 应降级 EquipGlyph');
  });

  testWidgets('师承遗物 → 显师承标记', (tester) async {
    final eq = mkEq(
        id: 40, tier: EquipmentTier.baoWu, slot: EquipmentSlot.weapon,
        defId: 'weapon_baowu_bing_po_zhen', isLineageHeritage: true);
    await pumpInv(tester, equipments: [eq]);
  ...
  testWidgets('TabBar 2 tab + 默认装备 Tab 显部位段', (tester) async {
    await pumpInv(tester,
        equipments: [mkEq(id: 10, tier: EquipmentTier.xunChang, slot: EquipmentSlot.weapon)],
        items: [mkItem(id: 1, defId: 'item_mojianshi', itemType: ItemType.moJianShi, quantity: 5)]);
  ...
  testWidgets('物料 Tab 单行 → 磨剑石 × N 渲染', (tester) async {
    await pumpInv(tester, equipments: [], items: [
      mkItem(id: 1, defId: 'item_mojianshi', itemType: ItemType.moJianShi, quantity: 2001),
    ]);
    await tester.tap(find.text('物料'));
    await tester.pumpAndSettle();
  ...
  testWidgets('物料 Tab 2 行 / 2 种 → 按 enum 顺序分组（磨剑石在前）', (tester) async {
    await pumpInv(tester, equipments: [], items: [
      mkItem(id: 1, defId: 'item_mojianshi', itemType: ItemType.moJianShi, quantity: 2001),
      mkItem(id: 2, defId: 'item_xinxuejiejing', itemType: ItemType.xinXueJieJing, quantity: 201),
    ]);
    await tester.tap(find.text('物料'));
    await tester.pumpAndSettle();
    expect(find.text('磨剑石 × 2001'), findsOneWidget);
    ...
    final mojiTitleY = tester
        .getTopLeft(find.ancestor(of: find.text('磨剑石'), matching: find.byType(Row)).first)
        .dy;
    final xinxueTitleY = tester
        .getTopLeft(find.ancestor(of: find.text('心血结晶'), matching: find.byType(Row)).first)
        .dy;
  ...
  testWidgets('物料 quantity == 0 的行被过滤', (tester) async {
    await pumpInv(tester, equipments: [], items: [
      mkItem(id: 1, defId: 'item_mojianshi', itemType: ItemType.moJianShi, quantity: 0),
    ]);
    await tester.tap(find.text('物料'));
    await tester.pumpAndSettle();
    expect(find.text('暂无物料'), findsOneWidget);
