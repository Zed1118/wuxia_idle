...
void main() {
  ...
  group('_LoreSection 延续典故渲染', () {
    ...
    // ── A. 仅 preset 段 ────────────────────────────────────────────────────────
    testWidgets('A. 仅 preset 段:3 段全显 + 0 个延续 chip', (tester) async {
      ...
      for (final presetY in presetYs) {
        for (final chipY in chipYs) {
          expect(
            ...
            lessThan(chipY),
            reason:
                'preset 段(y=$presetY) 必须渲染在延续 chip(y=$chipY) 之前(更靠上)',
          );
        }
      }
    });
