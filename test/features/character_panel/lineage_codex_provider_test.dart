...
void main() {
  group('groupGenerations', () {
    test('单代：1 祖师 + 2 弟子(masterId 指向祖师)', () {
      final chars = [
        _c(1, founder: true, role: LineageRole.founder),
        _c(2, master: 1, role: LineageRole.senior),
        _c(3, master: 1, role: LineageRole.junior),
      ];
      final gens = groupGenerations(
        characters: chars,
        heritage: const [],
        currentFounderId: 1,
        activeIds: const [1, 2, 3],
        recruitedIds: const [],
      );
    ...
    test('多代：按 founder id 升序，太祖在前', () {
      final chars = [
        _c(1, founder: true, role: LineageRole.founder),
        _c(2, founder: true, master: 1, role: LineageRole.senior),
      ];
      final gens = groupGenerations(
        characters: chars,
        heritage: const [],
    ...
    test('当代弟子兜底走 active∪recruited(masterId 为空也不漏)', () {
      final chars = [
        _c(1, founder: true, role: LineageRole.founder),
        _c(2, master: null, role: LineageRole.senior),
      ];
      final gens = groupGenerations(
        characters: chars,
        heritage: const [],
    ...
    test('多代无双计入：旧代师兄弟仍滞留 active/recruited 也只归旧代', () {
      ...
      final char1 = _c(1, founder: true, role: LineageRole.founder)
        ..isActive = false;
      ...
      final totalRelics = gens.fold<int>(
        0,
        (sum, g) => sum + g.heritageEquipments.length,
      );
    ...
    test('无 founder → 返回空列表', () {
      final chars = [
        _c(1, role: LineageRole.disciple),
        _c(2, master: 1, role: LineageRole.disciple),
      ];
      final gens = groupGenerations(
        characters: chars,
        heritage: const [],
