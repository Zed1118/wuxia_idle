...
void main() {
  ...
  // ── T1: ensureInitialized：空 islandBuildings → 建全量建筑 ──────────────
  test(
    'T1: ensureInitialized 空档→建全量建筑、processor 有默认 activeRecipeId、时间被设',
    () async {
      await seedFounder();
      final isar = IsarSetup.instance;
      final save = (await isar.saveDatas.get(0))!;
      ...
      expect(
        ...
        7,
        reason: '7 个建筑（4 source + 3 processor）',
      );
      ...
      final zhuZaoTai = updated.islandBuildings
          .where((b) => b.type == BuildingType.zhuZaoTai)
          .firstOrNull;
      expect(zhuZaoTai, isNotNull);
  ...
  // ── T2: ensureInitialized：旧 4 栋存档 → 只补缺失建筑 ─────────────────
  test('T2: ensureInitialized 旧4栋存档→补齐缺失建筑且保留旧状态/时间', () async {
    await seedFounder();
    final isar = IsarSetup.instance;
    final oldLastSettledAt = DateTime(2026, 6, 25, 10, 0);
    ...
    final updated = (await isar.saveDatas.get(0))!;
    expect(updated.islandBuildings.length, 7);
    expect(
    ...
    final tie = updated.islandBuildings.firstWhere(
  ...
  test('T2b: settle 旧4栋存档→先补齐缺失建筑再结算', () async {
    await seedFounder(tier: RealmTier.xueTu);
    final isar = IsarSetup.instance;
    final past = DateTime(2026, 6, 25, 0, 0);
    final now = past.add(const Duration(hours: 4));

    await isar.writeTxn(() async {
      final s = (await isar.saveDatas.get(0))!;
      s.islandBuildings = [
        IslandBuildingState()..type = BuildingType.tieJiangChang,
        IslandBuildingState()..type = BuildingType.caoYaoYuan,
        IslandBuildingState()
          ..type = BuildingType.daZaoTai
          ..activeRecipeId = 'forge_mojianshi',
        IslandBuildingState()
          ..type = BuildingType.danFang
          ..activeRecipeId = 'brew_ningshen',
      ];
      s.islandLastSettledAt = past;
      await isar.saveDatas.put(s);
    });
  ...
  test('T3: ensureInitialized 已全量初始化→不改动已有建筑', () async {
    await seedFounder();
    final isar = IsarSetup.instance;
    ...
    final updated = (await isar.saveDatas.get(0))!;
    expect(updated.islandBuildings[0].level, 3, reason: '已初始化不应重置 level');
    expect(updated.islandLastSettledAt, now1, reason: '已初始化不应更新时间');
  });

  // ── T4: settle：storage 增长、time 更新、inventory 不变 ──────────────────
  test(
    'T4: settle N 小时→ storage 增长、islandLastSettledAt 更新、inventory 不变',
    () async {
      await seedFounder(tier: RealmTier.xueTu);
      final isar = IsarSetup.instance;
      final past = DateTime(2026, 6, 25, 0, 0);
  ...
  // ── T5: harvest：成品入背包、stored 清小数尾、IslandHarvest 有条目 ────────
  test(
    'T5: harvest→ InventoryItem 增加、stored 清到小数尾、IslandHarvest.gained 非空',
    () async {
      await seedFounder(tier: RealmTier.xueTu);
      final isar = IsarSetup.instance;
      final past = DateTime(2026, 6, 25, 0, 0);
  ...
  // ── T6: offline 长时段 harvest：多建筑成品齐全 ────────────────────────────
  test('T6: 长时段(72h)harvest→ 多建筑成品(磨剑石+凝神丹)同时出现在 gained', () async {
    await seedFounder(tier: RealmTier.xueTu);
    final isar = IsarSetup.instance;
    final past = DateTime(2026, 6, 25, 0, 0);
    ...
    expect(
      harvest.gained.containsKey('item_jingyandan_small'),
      isTrue,
      reason: '丹房 72h 应产凝神丹',
    );
    ...
    final jingyandan = await isar.inventoryItems.getByDefId(
  ...
  // ── T7: harvest 累加——第二次 harvest 叠加在第一次数量上 ─────────────────
  test('T7: 两次 harvest→ InventoryItem quantity 累加', () async {
    await seedFounder(tier: RealmTier.xueTu);
    final isar = IsarSetup.instance;
    ...
    expect(
      h1.gained.containsKey('item_mojianshi'),
      isTrue,
  ...
  // ── T8: founder 不存在时 realmIndex fallback 0 ─────────────────────────
  test('T8: 无 founder → founderRealmIndex=0 fallback 不崩', () async {
  ...
  // ── T9: founderRealmIndex 公开化——直接调用返回正确 index ─────────────────
  test(
    'T9: founderRealmIndex public helper：有 founder 时返回正确 realmTier.index',
    () async {
