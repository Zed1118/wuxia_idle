...
/// GameRepository.loadAllDefs（从文件系统加载真 towers.yaml）。
///
/// **层数一律从 [_maxFloor] 派生，不写死**（A0 解层数硬编码）：塔扩层后
/// 本文件断言自动跟随数据，不会因为写死 30 而在扩层时静默失效。
/// 「maxFloor 参数确实生效」的正面证明在 `tower_cycle_progress_test.dart`
/// （刻意用非 30 的层数，实现若回退成硬编码则全红）。
...
void main() {
  ...
  group('getOrCreate', () {
    test('首次调用 → 建一行 + 默认 highest=0 + totalAttempts/Defeats=0', () async {
      final p = await TowerProgressService(
      ...
      expect(p.id, isNot(Isar.autoIncrement), reason: 'put 后应分配真实 id');
    });
    ...
    test('P0.2 #40 Phase 1 新加 3 字段默认值', () async {
      ...
      expect(
        p.perFloorClearTimes,
        isEmpty,
        reason: 'List<int> 默认空,recordClear 时按 floorIndex 写入',
      );
      expect(p.bestClearTime, isNull, reason: 'null 语义 = 无通关数据,UI 显「—」');
      expect(p.lastClearedAt, isNull, reason: 'null 语义 = 无任何通关,UI 不显最近活跃时间');
    });

    test('二次调用同 saveDataId → 复用同一行（不重复建）', () async {
      final p1 = await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
      final p2 = await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
      expect(p2.id, p1.id);
      expect(
  ...
  group('availableFloor / canChallenge', () {
    test('全新进度 → availableFloor=1，canChallenge(1) true / (2) false', () async {
      ...
      expect(TowerProgressService.availableFloor(p, maxFloor: _maxFloor), 1);
      expect(
      ...
      expect(
        TowerProgressService.canChallenge(
          progress: p,
          floorIndex: _maxFloor + 1,
          maxFloor: _maxFloor,
        ),
        isFalse,
        reason: '超出塔顶层号不可挑战（上界跟随数据，不写死 31）',
      );
    });

    test(
      '通到 10 层 → availableFloor=11，canChallenge(1..11) 全 true / (12) false',
      () async {
        ...
        expect(TowerProgressService.availableFloor(p, maxFloor: _maxFloor), 11);
        for (var i = 1; i <= 11; i++) {
          expect(
            TowerProgressService.canChallenge(
              progress: p,
    ...
    test('塔挑战门槛只看层进度，不按弟子/队伍人数额外锁门', () {
      ...
      expect(TowerProgressService.availableFloor(p, maxFloor: _maxFloor), 20);
      expect(
        TowerProgressService.canChallenge(
          progress: p,
          floorIndex: 20,
    ...
    test('全塔通关 → availableFloor 封顶在顶层', () async {
      await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
      for (var i = 1; i <= _maxFloor; i++) {
        await TowerProgressService(isar: IsarSetup.instance).recordClear(
          floorIndex: i,
      ...
      expect(p.highestClearedFloor, _maxFloor);
      expect(
        TowerProgressService.availableFloor(p, maxFloor: _maxFloor),
        _maxFloor,
      );
    });
  });
  ...
  group('recordClear', () {
    test(
      '首通下一层 → isFirstClear=true + highest++ + highestClearedAt 更新',
      () async {
        await TowerProgressService(
          isar: IsarSetup.instance,
        ).getOrCreate(saveDataId: 1);
        final t = DateTime(2026, 5, 11, 14, 30);
        final result = await TowerProgressService(isar: IsarSetup.instance)
            .recordClear(
              floorIndex: 1,
              now: t,
              elapsedMs: 1000,
    ...
    test('重打已通层 → isFirstClear=false + highest 不变 + totalAttempts++', () async {
      ...
      expect(p.highestClearedFloor, 1);
      expect(p.highestClearedAt, firstAt, reason: '重打不更新 highestClearedAt');
      expect(p.totalAttempts, 2);
    });

    test(
      '跳层挑战（floorIndex 非 highest+1）→ service 不抛但 isFirstClear=false',
      () async {
        await TowerProgressService(
          isar: IsarSetup.instance,
        ).getOrCreate(saveDataId: 1);
    ...
    test('Phase 2:重打不覆盖 perFloorClearTimes(GDD §5.1 反主流防刷)', () async {
      ...
      expect(p.perFloorClearTimes, [
        5000,
      ], reason: '重打 elapsedMs 2000 不覆盖首通 5000');
      expect(p.bestClearTime, 5000, reason: 'bestClearTime 派生自首通,不受重打影响');
    });
    ...
    test('Phase 2:lastClearedAt 任何通关(首通/重打/跳层)都更新', () async {
      ...
      var p = await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
      ...
      p = await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
      expect(
        p.lastClearedAt,
        t2,
        reason: '重打也更新 lastClearedAt(与 highestClearedAt 区分)',
      );
  ...
  group('recordDefeat', () {
    test('战败 → totalAttempts++ + totalDefeats++ + highest 不变', () async {
      ...
      await TowerProgressService(
        isar: IsarSetup.instance,
      ).recordDefeat(now: DateTime(2026, 5, 12));
      final p = await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
    ...
    test('多次战败累加', () async {
      ...
      await TowerProgressService(
        isar: IsarSetup.instance,
      ).recordDefeat(now: DateTime(2026, 5, 11));
      await TowerProgressService(
        isar: IsarSetup.instance,
      ).recordDefeat(now: DateTime(2026, 5, 12));
      await TowerProgressService(
        isar: IsarSetup.instance,
      ).recordDefeat(now: DateTime(2026, 5, 13));
      final p = await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
  ...
  group('floorList', () {
    test('全新进度 → floor 1 available，其余全 locked', () async {
      final p = await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
      ...
      expect(entries.length, _maxFloor);
    ...
    test('通到 5 层 → 1-5 cleared、6 available、其余 locked', () async {
      await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
      for (var i = 1; i <= 5; i++) {
    ...
    test('全塔通关 → 全 cleared（无 available）', () async {
      await TowerProgressService(
        isar: IsarSetup.instance,
      ).getOrCreate(saveDataId: 1);
      for (var i = 1; i <= _maxFloor; i++) {
      ...
      expect(
        entries.every((e) => e.status == TowerFloorStatus.cleared),
        isTrue,
      );
    });
  });
  ...
  group('advanceCycle', () {
    test(
      '当前周目未全通 → advanceCycle no-op（maxClearedCycle < currentCycleIndex）',
      () async {
        ...
        expect(p.currentCycleIndex, 1, reason: '未全塔通关时 advanceCycle 应 no-op');
      },
    );

    test(
      '全塔通关后 advanceCycle → currentCycleIndex++ + highestClearedFloor=0',
      () async {
        final svc = TowerProgressService(isar: IsarSetup.instance);
        await svc.getOrCreate(saveDataId: 1);
        for (var i = 1; i <= _maxFloor; i++) {
        ...
        expect(
          p.currentCycleIndex,
          2,
          reason: '全塔通关后 advanceCycle 应 currentCycleIndex=2',
        );
        expect(
    ...
    test(
      'advanceCycle：currentCycleIndex >= maxCycleCap 时 no-op（config cap 防越线）',
      () async {
        final svc = TowerProgressService(isar: IsarSetup.instance);
        await svc.getOrCreate(saveDataId: 1);
        ...
        expect(
          ...
          maxCycleTower,
          reason:
              'currentCycleIndex 已达 maxCycleCap=$maxCycleTower，'
