...
void main() {
  ...
  Widget app(TowerProgress p) {
    return ProviderScope(
      overrides: [towerProgressProvider.overrideWith((ref) async => p)],
  ...
  testWidgets('空态(highest=0)显示「尚未通关任何爬塔层」', (tester) async {
    ...
    expect(
      find.text(UiStrings.leaderboardHighestLayer),
      findsNothing,
      reason: '空态不显 3 指标 tile',
    );
  });

  testWidgets('通 5 层 + 3 指标全显(highestFloor/bestClearTime/totalAttempts)', (
    tester,
  ) async {
    ...
    await tester.pumpAndSettle();
    expect(
      find.text('5 ${UiStrings.leaderboardLayerSuffix}'),
      findsOneWidget,
  ...
  testWidgets('bestClearTime null → 显示「—」(无通关耗时数据)', (tester) async {
    ...
    expect(find.text(UiStrings.leaderboardBestClearTime), findsOneWidget);
    expect(
      find.text(UiStrings.leaderboardNoData),
      findsOneWidget,
