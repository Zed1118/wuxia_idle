...
List<String> _fillMains(Character character, List<SkillDef> mainSkills) {
...
void main() {
  ...
  group('SkillLoadout.autoFill 集成：lineage 倾向对最终 loadout 生效', () {
    ...
    test('大弟子(senior)：最终 loadout 总计 2 个主修槽被填（无空槽）', () {
      ...
      final mains = _fillMains(senior, pool);
      expect(mains.length, 2, reason: '主修槽应被完整填满（pool 有 4 个候选，2 槽应全填）');
    });
    ...
    test('大弟子(senior)：另一个主修槽为高倍率技（不全换成破防）', () {
      ...
      expect(
        [highA.id, highB.id, lowD.id].contains(other),
        isTrue,
        reason: '另一个主修槽应是非破防技',
      );
    });
    ...
    test('祖师：autoFill 按 power 降序选槽，破防技（低 power）不进主修槽', () {
      ...
      expect(mains.contains(highA.id), isTrue, reason: '祖师主修槽应包含最高倍率技 high_a');
      expect(mains.contains(highB.id), isTrue, reason: '祖师主修槽应包含第二高倍率技 high_b');
      expect(
        mains.contains(defBreak.id),
        isFalse,
    ...
    test('徒孙（grandDisciple）：power-sort default，行为与无 lineage 身份完全一致', () {
      final grandDisciple = _character(
        isFounder: false,
        lineageRole: LineageRole.grandDisciple,
      );
      final mains = _fillMains(grandDisciple, pool);

      expect(mains.contains(highA.id), isTrue, reason: '徒孙主修槽应包含最高倍率技 high_a');
      expect(mains.contains(highB.id), isTrue, reason: '徒孙主修槽应包含第二高倍率技 high_b');
      expect(
        mains.contains(defBreak.id),
        isFalse,
    ...
    test('软锁不锁红线：所有候选技能 canEquipAtRealm 仍通过（eligible 集合不缩小）', () {
      for (final s in pool) {
        expect(
          s.canEquipAtRealm(RealmTier.xueTu),
          isTrue,
    ...
    test('通用弟子(disciple)：不再获得破防倾向，行为同 power-sort default', () {
      ...
      expect(
        mains.contains(highA.id),
        isTrue,
        reason: 'disciple 不获得破防倾向，high_a 应进槽',
      );
      expect(
        mains.contains(highB.id),
        isTrue,
