...
void main() {
  for (final tier in [
    ...
    testWidgets('TreasureDropContent 渲染 $tier 不崩 + 缺图兜底 + 属性 + 典故', (t) async {
      // t=1.0:印章已落定、内容已渐入;缺图 iconPath 触发 errorBuilder→EquipGlyph 不破。
      await t.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: TreasureDropContent(highlight: _h(tier), t: 1.0),
          ),
        ),
      );
      ...
      MaterialApp(
        home: Scaffold(
          body: TreasureDropContent(
            highlight: _h(EquipmentTier.shenWu, tagline: null),
            t: 1.0,
          ),
        ),
      ),
