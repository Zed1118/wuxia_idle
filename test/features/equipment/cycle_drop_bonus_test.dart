...
void main() {
  group('CycleDropBonusConfig', () {
    test('fromYaml 解析 + qtyMultFor 按周目', () {
      final c = CycleDropBonusConfig.fromYaml({
        'material_qty_mult_ng_plus': 1.5,
      });
    ...
    test('fromYaml 空 map → none(倍率 1.0)', () {
      expect(
        CycleDropBonusConfig.fromYaml(const {}).materialQtyMultNgPlus,
        1.0,
      );
    });
  });
  ...
  group('applyCycleMaterialBonus', () {
    ...
    DropResult drops() => const DropResult(
      equipments: [],
      items: [
    ...
    test('加成后不低于原值(防 floor 把 1 个材料抹成 0)', () {
      const single = DropResult(
        equipments: [],
        items: [ItemDropResult(defId: 'item_jingtie', quantity: 1)],
      );
