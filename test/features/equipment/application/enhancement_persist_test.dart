...
void main() {
  ...
  test(
    'success → eq.enhanceLevel +1 + mojianshi row 扣 cost + jieJing 不变',
    () async {
      ...
      expect(await readQty(ItemType.xinXueJieJing), 5, reason: '成功路径不动结晶');
    },
  );

  // ── 2) failure 路径 ─────────────────────────────────────────────────

  test('failure → eq 不变 + mojianshi 扣 penaltyCost + jieJing +1', () async {
    await seedInventory(type: ItemType.moJianShi, quantity: 1000);
    await seedInventory(type: ItemType.xinXueJieJing, quantity: 5);
    final eq = await seedEq(enhanceLevel: 20);
  ...
  test(
    'guarantee success → eq.enhanceLevel +1 + jieJing 扣 crystalCost',
    () async {
      await seedInventory(type: ItemType.moJianShi, quantity: 1000);
  ...
  test('P1.y · success 到 enhanceLevel=9 → tutorialStep 不推进', () async {
    ...
    final tutorialSvc = TutorialService(IsarSetup.instance);
    expect(await tutorialSvc.getCurrentStep(), 7, reason: '+9 未到开锋锚点不推 step 8');
  });
  ...
  test('P1-7 · mojianshi 真实余量 < spent → StateError 且余量不落负数', () async {
    ...
    expect(await readQty(ItemType.moJianShi), 5, reason: '余量不足应整笔回滚,不落负数');
    final eqBack = await IsarSetup.instance.equipments.get(eq.id);
  ...
  test('P1-7 · jieJing 真实余量 < crystalsSpent → StateError 且不落负数', () async {
    ...
    expect(await readQty(ItemType.xinXueJieJing), 2, reason: '结晶不足应整笔回滚,不落负数');
  });
}
