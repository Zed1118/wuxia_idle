...
void main() {
  ...
  test(
    'seedP4 → 2 件 +0 利器（主 battleCount=2000 已装 / 对照 battleCount=0 未装）',
    () async {
  ...
  test('seedMasterDisciple → 3 师徒 + 师徒关系双向 + 祖师 id=1 + 默认入阵', () async {
  ...
  test('seedMasterDisciple → 3 师徒各自有主修 + 装备齐 weapon/armor/accessory', () async {
    await Phase2SeedService(isar: IsarSetup.instance).seedMasterDisciple();
    final isar = IsarSetup.instance;
  ...
  test('seedMasterDisciple 反复调用 reseed 一致（_clearAll 保证干净）', () async {
    await Phase2SeedService(isar: IsarSetup.instance).seedMasterDisciple();
    await Phase2SeedService(isar: IsarSetup.instance).seedMasterDisciple();
    final isar = IsarSetup.instance;
  ...
  test(
    'seedVisualCheckW7W11 → 师徒齐 + Ch1 01-04 标 cleared + stage_01_05 可挑战',
    () async {
  ...
  test('seedVisualCheckW7W11 反复调用 → MainlineProgress 幂等不重复 append', () async {
    await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW7W11();
    await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW7W11();
    final isar = IsarSetup.instance;
    final progress = await isar.mainlineProgress.where().findFirst();
    expect(
      progress!.clearedStageIds.length,
      4,
  ...
  test(
    'seedVisualCheckW14_3 → EncounterProgress.unlockedSkillIds 覆盖 tier 1-7',
    () async {
      ...
      expect(
        progress,
        isNotNull,
        reason: '应通过 EncounterService.getOrCreate 创建',
      );
      expect(
        progress!.unlockedSkillIds.length,
        7,
  ...
  test(
    'seedVisualCheckW14_3 → 大弟子 (id=2) 预装备 tier 3 encounter skill',
    () async {
      await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW14_3();
      final isar = IsarSetup.instance;
  ...
  test('seedVisualCheckW14_3 反复调用 → unlock 池仍 7（覆盖而非 append）', () async {
    await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW14_3();
    await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW14_3();
    final isar = IsarSetup.instance;
  ...
  test(
    ...
    () async {
      ...
      expect(
        r2Ids.contains(founder!.equippedWeaponId),
        isFalse,
  ...
  test('seedVisualCheckW15Resonance → 6 件武器入背包,'
      'battleCount / enhanceLevel / forgingSlots 按 spec 落', () async {
    ...
    expect(
      resIds.contains(founder!.equippedWeaponId),
      isFalse,
    ...
    for (final entry in expectations.entries) {
      ...
      final unlockedSlots = eq.forgingSlots.where((s) => s.unlocked).length;
      expect(
        unlockedSlots,
        entry.value.slots,
  ...
  test(
    'seedVisualCheckW15Resonance → 师承遗物强制标(weapon_liqi_pan_long_dao)',
    () async {
      await Phase2SeedService(
        isar: IsarSetup.instance,
      ).seedVisualCheckW15Resonance();
  ...
  test('seedVisualCheckW15Resonance → forgingSlots type 配置自洽', () async {
    await Phase2SeedService(
      isar: IsarSetup.instance,
    ).seedVisualCheckW15Resonance();
    final isar = IsarSetup.instance;
  ...
  test(
    'seedVisualCheckW15Fresh → 3 active 全员 xueTu.qiMeng + experience=0',
    () async {
      await Phase2SeedService(
        isar: IsarSetup.instance,
  ...
  test('seedVisualCheckW15Fresh → 主线 / 塔 / 奇遇 progress 全清', () async {
    ...
    expect(
      mainBefore!.clearedStageIds.length,
      4,
    ...
    expect(await isar.mainlineProgress.count(), 0, reason: '主线进度清零');
    expect(await isar.towerProgress.count(), 0, reason: '塔进度清零');
    expect(await isar.encounterProgress.count(), 0, reason: '奇遇进度清零');
  });

  test('seedVisualCheckW15Fresh → 0 装备 + 3 主修心法 + 3 流派分布 + 物料起步态', () async {
    await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW15Fresh();
    final isar = IsarSetup.instance;

    expect(
      await isar.equipments.count(),
      0,
    ...
    expect(techs.length, 3, reason: '3 角色各 1 主修(刚猛入门 / 灵巧入门 / 阴柔入门)');
    ...
    for (final t in techs) {
      expect(t.tier, TechniqueTier.ruMenGong, reason: 'tier 0 入门功(学徒境界三系锁死合规)');
  ...
  test('seedVisualCheckW15Fresh 反复调用 → experience 回 0(派单可反复 reseed)', () async {
    final isar = IsarSetup.instance;
    ...
    expect(fresh!.experience, 0, reason: 'reseed 后 EXP 回 0,派单时无须手动清存档');
  });

  // ── W18-A1 心法相生 fixture · seedVisualCheckW18A1 ─────────────────────────

  test(
    'seedVisualCheckW18A1 → 7 角色 yiLiu.qiMeng + activeCharacterIds 全塞 + 主修非空',
    () async {
      ...
      expect(
        ...
        7,
        reason:
            '7 角色对应 7 相生组合各 1 命中(覆盖 GameRepository.synergies 7 id 全集,2026-05-20 T01 +2)',
      );
      for (final ch in chars) {
        expect(
          ch.realmTier,
          RealmTier.yiLiu,
          reason:
              '${ch.name} 必须 yiLiu(equipment cap=liQi / technique cap=menPaiJueXue,mingJiaGong+changLianGong 全在三系锁死安全区)',
        );
        ...
        expect(
          ...
          isNotNull,
          reason:
              '${ch.name} 必须有主修(SynergyService.detectActive 硬约束 + StageBattleSetup)',
        );
        expect(
          ch.assistTechniqueIds.isNotEmpty,
          isTrue,
          reason:
              '${ch.name} 必须有第 1 辅修(SynergyService.detectActive 用 assistTechniqueIds.first)',
        );
        expect(
          ch.school,
          isNotNull,
      ...
      final save = await isar.saveDatas.get(0);
      expect(
        save!.activeCharacterIds.length,
        7,
        reason:
            '7 角色全入 activeCharacterIds,CharacterPanelScreen TabBar 显 7 个(lineageTabLabels 扩 5→7 配套)',
      );
      expect(save.founderCharacterId, 1, reason: 'A·阴阳 id=1 为 founder(沿既有体例)');
    },
  );
  ...
  test(
    'seedVisualCheckW18A1 → 7 角色 detectActive 命中集合 = synergies yaml 全集',
    () async {
      await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW18A1();
      final isar = IsarSetup.instance;
      final repo = GameRepository.instance;
      ...
      expect(
        ...
        yamlIdsNonSpecific,
        reason:
            '7 角色命中的 synergy id 集合 == yaml 非 specificTechniques 类型全集'
      ...
      expect(
        ...
        7,
        reason:
            '7 角色 → 7 唯一 synergy(候选 2 加 synergy 8 specificTech 因 fixture '
  ...
  test(
    'seedVisualCheckW18A1 → 5 角色 requirementType 覆盖 3 类(schoolPair*3/sameSchool*1/sameTier*1)',
    () async {
      await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW18A1();
      final isar = IsarSetup.instance;
      final repo = GameRepository.instance;
      ...
      expect(
        typeCount[SynergyRequirementType.schoolPair],
        5,
        reason:
            '5 角色覆盖 schoolPair(原 3 + 2 新反向:yinRou+gangMeng / lingQiao+gangMeng)',
      );
      expect(
        typeCount[SynergyRequirementType.sameSchool],
        1,
        reason: '1 角色覆盖 sameSchool(优先级先于 sameTier)',
      );
      expect(
        typeCount[SynergyRequirementType.sameTier],
        1,
        reason:
            '1 角色覆盖 sameTier(E·同辈 lingQiao+yinRou,刻意避开 schoolPair 6 方向中的此对)',
      );
    },
  );

  test(
    'seedVisualCheckW18A1 → 主线 / 塔 / 奇遇 progress 全清 + Ch1 01-04 cleared',
    () async {
      await Phase2SeedService(isar: IsarSetup.instance).seedVisualCheckW18A1();
      final isar = IsarSetup.instance;
      ...
      expect(
        main!.clearedStageIds,
        ['stage_01_01', 'stage_01_02', 'stage_01_03', 'stage_01_04'],
  ...
  test('seedVisualCheckW18A1 反复调用 → 7 角色仍 7(派单可反复 reseed)', () async {
    final isar = IsarSetup.instance;
  ...
  test(
    'seedVisualCheckP5Plus → founder wuSheng·dengFeng + 2 stage cleared + 师徒 3',
    () async {
      ...
      expect(
        progress!.clearedStageIds.contains('stage_inner_demon_07'),
        true,
        reason: 'inner_demon_07 已 mark cleared',
      );
      expect(
        progress.clearedStageIds.contains('stage_06_05'),
        true,
  ...
  test(
    'seedVisualCheckP5Plus 反复调用 → 幂等(MainlineProgress 不 append 重复)',
    () async {
      final isar = IsarSetup.instance;
      // 第 1 跑
      ...
      expect(cleared1, 2, reason: '只 inner_demon_07 + stage_06_05 2 关 cleared');
    },
  );
  ...
  test(
    'seedRefineInsight → 主修非空 + insightPoints>0 + tutorialStep>=3',
    () async {
      ...
      final ch = await isar.characters.get(1);
      expect(ch, isNotNull);
      expect(ch!.mainTechniqueId, isNotNull, reason: '凝练按钮可见前提:主修心法已习得');
      expect(
        ch.insightPoints,
        greaterThan(0),
