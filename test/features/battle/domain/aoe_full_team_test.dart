...
void main() {
  ...
  test('aoe 命中全体:3 活敌全部扣血(各 currentHp 下降),不只打 first', () {
    ...
    final left = [
      attacker(skills: const [aoePower, normal]),
    ];
    final right = [
    ...
    // 主控用的应该是 aoe 技。
    final firstAttack = s.actionLog.firstWhere(
      (a) => a.actorId == 1 && a.skill != null,
    );
    expect(
      firstAttack.skill!.id,
      aoePower.id,
      reason: 'AI 应自动挑 aoe powerSkill(倍率 > 普攻)',
    );

    for (final orig in right) {
      final after = s.rightTeam.firstWhere(
        (c) => c.characterId == orig.characterId,
      );
      expect(
        after.currentHp,
        lessThan(orig.currentHp),
        reason:
            'aoe 应对全体敌人(charId=${orig.characterId})各造成伤害,'
  ...
  test('aoe 无衰减:对某敌扣血 == 同条件 single 技对该敌单体伤害', () {
    ...
    final aoeEnemy12 = aoeState.rightTeam.firstWhere(
      (c) => c.characterId == 12,
    );
  ...
  test('aoe 确定性:同 seed 跑含 aoe 大招的完整战斗两遍逐 action + 终态全等', () {
    ...
    expect(
      ...
      equals(second),
      reason:
          'aoe 多目标结算 loop 不得破坏 rng 确定性:同 seed 两次跑应逐 action'
          '(含伤害/暴击/破招)+ 终态血量 + 胜负全等',
    );
  });
}
