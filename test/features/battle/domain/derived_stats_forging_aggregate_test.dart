...
void main() {
  group('forgingAggregatePct 跨全身装备求和', () {
    test('空装备列表 → 0', () {
      expect(
        CharacterDerivedStats.forgingAggregatePct([], ForgingSlotType.pierce),
        0.0,
      );
    });
    test('单件 pierce 15 → 0.15', () {
      ...
      expect(
        CharacterDerivedStats.forgingAggregatePct(eqs, ForgingSlotType.pierce),
        0.15,
      );
    });
    ...
    test('未解锁槽不计入', () {
      final eqs = [
        _eq([_slot(ForgingSlotType.pierce, 20, unlocked: false)]),
      ];
      expect(
        CharacterDerivedStats.forgingAggregatePct(eqs, ForgingSlotType.pierce),
        0.0,
    ...
    test('类型过滤:查 lifesteal 不计 pierce 槽', () {
      final eqs = [
        _eq([
          _slot(ForgingSlotType.pierce, 20),
          _slot(ForgingSlotType.lifesteal, 10, idx: 2),
        ]),
      ];
      expect(
        CharacterDerivedStats.forgingAggregatePct(
          eqs,
