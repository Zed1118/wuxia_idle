...
void main() {
  ...
  group(
    ...
    () {
      ...
      test('仅 leftTeam 烘焙:rightTeam stat 全部不动(玩家战略选择,与 LightFoot 双方对等不同)', () {
        ...
        expect(
          modified.rightTeam.first.criticalRate,
          closeTo(0.15, 1e-9),
      ...
      test(
        'clamp 走 rateCap 配置:传 0.80 时 critRate 0.90+yanXing 0.10 → 0.80(证配置驱动非写死)',
        () {
          final state = _makeState(criticalRate: 0.90);
          final config = _testConfig();
  ...
  group('MassBattleStrategy wave 间 intermission 状态切换语义', () {
    ...
    test('多 wave ctor:waveCount = enemyTeamsPerWave.length(2/3/4)', () {
      ...
      expect(
        strat4.waveCount,
        4,
        reason: '4 wave 守城(spec stage_mass_battle_04/05)',
      );
    });

    test(
      'empty enemyTeamsPerWave:ctor 不抛 + waveCount=0(runToEnd 兜底由 service 防)',
      () {
    ...
    test(
      'config.waveIntermission 4 字段语义沿默认(actionPoint reset / HP+IF preserve / cd reset)',
      () {
        final config = _testConfig();
        ...
        expect(wi.preserveHp, isTrue, reason: 'wave 间 HP 保留 → 守城压力跨 wave 累积');
        expect(
  ...
  group('MassBattleStrategy production stepOne/tick wiring', () {
    test('P0-1 stepOne 生产路径装入 wave0 + 烘焙 formation,且不重复叠加', () {
      final strategy = MassBattleStrategy(
        formation: Formation.fengShi,
        enemyTeamsPerWave: [
      ...
      var s = _makeState(withRight: true); // 生产入口先带 stage.enemyTeam 占位

      s = strategy.stepOne(s, n);
      expect(
        s.rightTeam.single.characterId,
        -10,
        reason: '生产 stepOne 应替换为 enemyTeamsPerWave[0],不再打 3 人占位队',
      );
      expect(s.leftTeam.first.criticalRate, closeTo(0.20, 1e-9));
      expect(s.leftTeam.first.attackPowerMultiplier, closeTo(1.10, 1e-9));

      s = strategy.stepOne(s, n);
      expect(
        s.leftTeam.first.criticalRate,
        closeTo(0.20, 1e-9),
        reason: 'formation bake 应只进一次,不能每个 stepOne 叠加',
      );
    });

    test('P2 wave leftWin 后切下一波并清空 actorQueue/pendingTargets', () {
      final wave0 = [_makeEnemyWaveChar(-10, 0)];
      final wave1 = [_makeEnemyWaveChar(-20, 0)];
      final strategy = MassBattleStrategy(
        formation: Formation.yanXing,
        enemyTeamsPerWave: [wave0, wave1],
        config: _testConfig(),
      );
      final n = GameRepository.instance.numbers;
      final s0 =
          BattleState.initial(
            ...
            actorQueue: const [(charId: 1, teamSide: 0)],
            pendingTargets: const {1: -10},
          );

      final next = strategy.tick(s0, n);
...
BattleCharacter _makeEnemyWaveChar(int characterId, int slotIndex) => _makeChar(
  characterId: characterId,
  teamSide: 1,
  slotIndex: slotIndex,
  criticalRate: 0.15,
