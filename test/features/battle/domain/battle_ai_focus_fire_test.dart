...
void main() {
  ...
  test('测 A:有破绽窗口敌(HP高) → 集火破绽窗口敌,忽略血更低的普通敌', () {
    ...
    final state = BattleState.initial(leftTeam: [actor], rightTeam: [e0, e1]);
    ...
    expect(
      ...
      e1.characterId,
      reason:
          '第六阶段集火:应优先攻击破绽窗口敌(e1,stagger=3),'
          '而非血最低敌(e0,HP=3000)',
    );
  });

  test('测 B:无破绽窗口敌 → 回落 HP 最低逻辑(e0)', () {
    final actor = makeActor();
  ...
  test('测 C:多个破绽窗口敌 → 在破绽敌集合内选 HP 最低;HP 相等时选 slotIndex 小', () {
    final actor = makeActor();
    ...
    final (_, targetIds2) = BattleAI.decide(
      actor2,
      state2,
      GameRepository.instance.numbers,
    );
  ...
  test('测 D:死亡敌(isAlive=false, stagger>0) + 活着普通敌 → 不集火死亡敌,选活着敌', () {
    final actor = makeActor();
