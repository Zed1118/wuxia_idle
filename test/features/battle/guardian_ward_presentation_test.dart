...
/// - 护法存活时 Boss 头像旁渲染护罩标签（[UiStrings.guardianWardActive]）
/// - 护法全灭后标签消失
...
void main() {
  ...
  group('CharacterAvatar 护罩标签渲染', () {
    ...
    testWidgets('护法存活 → 护罩标签渲染在 Boss 头像旁', (tester) async {
      ...
      expect(find.text(UiStrings.guardianWardActive), findsOneWidget);
    ...
    testWidgets('护法全灭 → 护罩标签消失', (tester) async {
      ...
      await pump(tester, state);
      expect(find.text(UiStrings.guardianWardActive), findsNothing);
