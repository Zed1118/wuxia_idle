...
BattleCharacter _player({
  int id = 1,
  int slot = 0,
  RealmTier realm = RealmTier.sanLiu,
  ...
  List<SkillDef> skills = const [],
  double lifestealPct = 0,
}) => BattleCharacter(
  characterId: id,
  name: '玩家$id',
  realmTier: realm,
  ...
  maxHp: maxHp,
  currentHp: currentHp,
  maxInternalForce: maxForce,
  currentInternalForce: curForce,
  speed: 100,
  criticalRate: 0,
  evasionRate: 0,
  defenseRate: 0.1,
...
AttackResult _hit({
  ...
  double schoolCounterMultiplier = 1.0,
  int lifestealHeal = 0,
}) => AttackResult(
  finalDamage: damage,
  mainDamage: damage,
  ...
  isCritical: false,
  isDodged: false,
  schoolCounterMultiplier: schoolCounterMultiplier,
...
void main() {
  ...
  test('realm_gap: 敌方最高境界高于己方 → 指向境界短板', () {
    final s = _lost(
      left: [_player(realm: RealmTier.sanLiu)],
      right: [_enemy(realm: RealmTier.erLiu)],
      log: const [],
    );
    final d = BattleDiagnosis.from(s, _cfg)!;
    expect(d.ruleId, 'realm_gap');
    expect(d.shortfall, DefeatShortfall.realm);
    expect(d.suggestions.first.jump, DiagnosisJumpTarget.cultivation);
  });

  test('countered_by_school: 受克制伤害占比高 → 指向阵容', () {
    final s = _lost(
      left: [_player(school: TechniqueSchool.gangMeng)],
      right: [_enemy(id: 100, school: TechniqueSchool.lingQiao)],
      log: [
        BattleAction(
          tick: 10,
          actorId: 100,
          targetId: 1,
          skill: _normalSkill,
          attackResult: _hit(damage: 700, schoolCounterMultiplier: 1.25),
          description: '',
        ),
        BattleAction(
          tick: 20,
    ...
    final d = BattleDiagnosis.from(s, _cfg)!;
    expect(d.ruleId, 'countered_by_school');
    expect(d.shortfall, DefeatShortfall.counter);
    expect(d.suggestions.first.jump, DiagnosisJumpTarget.roster);
  });
  ...
  test('killed_by_internal_wound: 内伤占比 ≥ 0.30', () {
    ...
    expect(d.shortfall, DefeatShortfall.supplies);
    expect(d.suggestions.first.jump, DiagnosisJumpTarget.supplies);
  });
  ...
  test('优先级: charge 高于 mob_overrun', () {
    ...
    final boss = _enemy(
      id: 100,
      boss: true,
      chargeSkillId: 'skill_boss_charge',
    );
    final mob = _enemy(id: 101, boss: false);
    final s = _lost(
      left: [_player()],
    ...
    final s = _lost(
      ...
      log: [
        ...
        BattleAction(
          ...
          attackResult: _hit(damage: 500),
          description: '',
        ), // 累计 1100 ≥ 1000 @tick40, 40/200=0.2 ≤ 0.5
      ],
    );
    final d = BattleDiagnosis.from(s, _cfg)!;
    expect(d.ruleId, 'frontline_fragile');
    expect(d.shortfall, DefeatShortfall.equipment);
    expect(d.suggestions.first.jump, DiagnosisJumpTarget.equipment);
  });

  test('supplies_exhausted: 总承伤高且无回复手段 → 指向行囊补给', () {
    final s = _lost(
      left: [_player(alive: true, currentHp: 10, maxHp: 1000)],
      right: [_enemy(id: 100, boss: true, currentHp: 100, maxHp: 1000)],
      tick: 100,
      log: [
        BattleAction(
          tick: 60,
    ...
    final d = BattleDiagnosis.from(s, _cfg)!;
    expect(d.ruleId, 'supplies_exhausted');
    expect(d.shortfall, DefeatShortfall.supplies);
    expect(d.suggestions.first.jump, DiagnosisJumpTarget.supplies);
  });
