...
void main() {
  group('章级周目纯派生(无 Isar)', () {
    test('highestClearedCycleForChapter 取该章最大 cycle,未通返 0', () {
      ...
      expect(
        MainlineProgressService.highestClearedCycleForChapter(p, 'ch2'),
        1,
      );
      expect(
        MainlineProgressService.highestClearedCycleForChapter(p, 'ch3'),
        0,
      );
      expect(
        MainlineProgressService.highestClearedCycleForChapter(p, 'innerDemon'),
        1,
      );
    });
    ...
    test('currentChallengeCycleForChapter = 最高+1,clamp maxCycle', () {
      ...
      expect(
        MainlineProgressService.currentChallengeCycleForChapter(
          p,
          'ch1',
          maxCycle: 3,
        ),
  ...
  group(
    'chapterKeyForStage + recordVictory 章末 Boss(Isar + GameRepository)',
    () {
      setUpAll(() async {
      ...
      test('chapterKeyForStage:主线 ch{N} / 副本 stageType.name', () {
        ...
        final innerDemon = repo.stageDefs.values.firstWhere(
          (s) => s.stageType == StageType.innerDemon,
        );
        expect(
          MainlineProgressService.chapterKeyForStage(innerDemon),
          'innerDemon',
        );
      });
      ...
      test('章末 Boss 关通关记 chapter cycle key,非 Boss 关不记', () async {
        ...
        final ch1NonBoss = repo.stageDefs.values.firstWhere(
          (s) =>
              s.stageType == StageType.mainline &&
              s.chapterIndex == 1 &&
              !s.isBossStage,
      ...
      test(
        '0.21→0.22 迁移:旧 Boss 关 stage cycle key → chapter cycle key',
        () async {
          final repo = GameRepository.instance;
          final ch1Boss = repo.stageDefs.values.firstWhere(
            (s) =>
