...
void main() {
  ...
  group('FounderAncestorBuff schema', () {
    test('NumbersConfig 生产 yaml 加载 P1.1 E.5 决议数值', () {
      ...
      expect(
        n.founderAncestorBuff.isActive,
        true,
      ...
      expect(
        n.founderAncestorBuff.applyToDisciplesOnly,
        false,
    ...
    test(
      'enabled_when_alive: true + sect_wide_buff: null → 0 数值但 isActive=true',
      () {
        final buff = FounderAncestorBuff.fromYaml(const {
          'enabled_when_alive': true,
          'sect_wide_buff': null,
        });
    ...
    test('数值红线 §5.4:internalForceMaxWithLineage 乘法叠加 18900 → clamp 15000', () {
      ...
      final equipped = List.generate(
        4,
        (_) => Equipment()..isLineageHeritage = true,
      );
    ...
    test('数值红线 §5.4:maxHp founder buff +5% 推过 20000 → clamp 20000', () {
      ...
      final result = CharacterDerivedStats.maxHp(
        c,
        equipped,
        n,
        founderBuffActive: true,
  ...
  group('R5 P4.1 1.1 cross_sect · isBuffActiveFor per-character', () {
    ...
    test(
      'R5.1 P1.1 维持:target.isInSect=false → true(active 含 founder)',
      () async {
        final svc = FounderBuffService(IsarSetup.instance);
        final n = GameRepository.instance.numbers;
        ...
        expect(
          active,
          true,
          reason: 'P1.1 fallback:isInSect=false → 单 founder 享',
        );
      },
    );

    test(
      'R5.2 跨派系 NPC 不享:isInSect=true, sectId=2 ≠ playerSectId=1 → false',
      () async {
        final svc = FounderBuffService(IsarSetup.instance);
        final n = GameRepository.instance.numbers;
        final npc = Character()
    ...
    test(
      'R5.3 同 sect 成员享:isInSect=true, sectId=1 == playerSectId=1 → true',
      () async {
        final svc = FounderBuffService(IsarSetup.instance);
        final n = GameRepository.instance.numbers;
    ...
    test(
      'R5.4 playerSectId=null fallback isInSect=false → true(P1.1 路径)',
      () async {
        final svc = FounderBuffService(IsarSetup.instance);
        final n = GameRepository.instance.numbers;
    ...
    test(
      'R5.5 整体 inactive(无 isFounder=true active)→ 任何 target false',
      () async {
        ...
        expect(
          await svc.isBuffActiveFor(target: t1, numbers: n, playerSectId: 1),
          false,
        );
        ...
        final t2 = Character()
          ...
          ..sectId = 1;
        expect(
          await svc.isBuffActiveFor(target: t2, numbers: n, playerSectId: 1),
          false,
        );
        ...
        final t3 = Character()
          ...
          ..sectId = 2;
        expect(
          await svc.isBuffActiveFor(target: t3, numbers: n, playerSectId: 1),
          false,
        );
      },
    );
...
class NumbersConfigStub implements NumbersConfig {
  const NumbersConfigStub({required FounderAncestorBuff buff}) : _buff = buff;
