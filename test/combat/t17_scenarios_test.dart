...
void main() {
  ...
  test('场景 A：单击伤害落在 2000-8000 区间（无暴击）', () {
    ...
    for (final d in damages) {
      expect(d, greaterThanOrEqualTo(2000), reason: '伤害下限 2000，实际 $d');
      expect(d, lessThanOrEqualTo(8000), reason: '伤害上限 8000，实际 $d');
    }
  });
  ...
  test('场景 C：纯武器攻击，+12强化+默契共鸣比裸装高 ≥ 60%（IF=0, PM=0 隔离）', () {
    ...
    expect(
      ratio,
      greaterThanOrEqualTo(1.60),
      reason:
          '+12强化+默契 eqAtk×1.92 → 比裸装高 ≥ 60%，实际比值 ${ratio.toStringAsFixed(3)}',
    );
  });
