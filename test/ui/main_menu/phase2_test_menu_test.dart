...
/// T32 子提交 3d：[Phase2TestMenu] widget 测试。
///
/// 4 用例覆盖：
///   - 4 场景按钮 label + hint 全部可见且顺序正确（P1 → P2 → P3 → P4）
///   - AppBar 标题 phase2MenuTitle 可见
///   - 4 个 _ScenarioButton InkWell 可点
...
void main() {
  ...
  testWidgets('4 场景按钮 label + hint 全部可见且顺序正确', (tester) async {
    await tester.pumpWidget(app());
  ...
  testWidgets('4 个场景按钮均为 InkWell（可点）', (tester) async {
    await tester.pumpWidget(app());
    // AppBar 默认 implyLeading 在 home 模式下不渲染 back button；4 个 _ScenarioButton
    // 各 1 个 InkWell，预期恰好 4 个。
