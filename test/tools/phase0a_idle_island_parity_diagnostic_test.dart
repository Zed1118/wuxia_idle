...
class _Observation {
  ...
  const _Observation({
    ...
    required this.theoreticalOutput,
...
void main() {
  ...
  test('Phase 0A 丹房在线/离线 parity diagnostic', () {
    ...
    if (Platform.environment[_updateEnv] == '1') {
      ...
      expect(
        File(_csvPath).readAsStringSync(),
        csv,
        reason: '提交 CSV 与当前 observations 不一致，请用 UPDATE...=1 刷新',
      );
      expect(
        File(_mdPath).readAsStringSync(),
        md,
...
_Observation _observe({
  ...
  final herbAvailable = omitHerbSource
      ? 0.0
      : (herbCfg.baseRatePerHour * settledHours).clamp(
          0.0,
          herbCfg.capFor(1).toDouble(),
        );
  final springAvailable = omitSpringSource
      ? 0.0
      : (springCfg.baseRatePerHour * settledHours).clamp(
          0.0,
          springCfg.capFor(1).toDouble(),
        );
  ...
  final bottleneck = limits.entries
      .where((entry) => (entry.value - minLimit).abs() <= _epsilon)
      .map((entry) => entry.key)
      .join('|');
  final difference = [
...
String _markdown(List<_Observation> rows) {
  ...
  for (final entry in groups.entries) {
    final maxDiff = entry.value
        .map((r) => r.difference)
        .reduce((a, b) => a > b ? a : b);
    final bottlenecks = entry.value.map((r) => r.bottleneck).toSet().toList()
      ..sort();
    lines.add(
      '| `${entry.key}` | ${entry.value.length} | $maxDiff | ${bottlenecks.join(', ')} |',
    );
  }
  lines.addAll([
    '',
