...
void main() {
  ...
  test('根因A 挂机经济曲线 · B1/B2/B3 平衡验证', () {
    ...
    expect(
      ...
      reason:
          'B2 设计锚: 二流 xuanYaPuBu 72h EXP 折 Boss=${xuanYa.bossEq.toStringAsFixed(1)} '
    ...
    for (final r in rows) {
      ...
      expect(
        ...
        reason:
            'B2 ${r.mapType}: 72h 挂机推进 ${r.layersGained} 层 > 21(3 个大境界),'
            'EXP 失控冲淡主动战斗',
      );
    }
    ...
    for (final r in lowTier) {
      expect(
        r.finalTier.index,
        lessThan(RealmTier.erLiu.index),
        reason:
            'B2 finding: 学徒图 ${r.mapType} 满挂落点 ${r.finalTier.name} '
...
void _writeSummary(
  ...
  buf.writeln('');
  buf.writeln(
    'Ch3 大Boss(stage_03_05)baseExpReward = **$ch3Boss** · '
    '早期修炼层(初窥→小成)= **$earlyLayer** · '
    'insight→修炼比率 = **$ratio** · base_learn = **$baseLearn**/h',
  );
  buf.writeln('');
  buf.writeln(
    '| 图 | requiredRealm | scale | B2 idleEXP | 折Ch3Boss | tier-fair升层 | '
  ...
  buf.writeln('');
  buf.writeln(
    '- 折Ch3Boss 仅在二流(Ch3 同期 tier)有意义;高 tier(宗师 duanYaJueBi)'
