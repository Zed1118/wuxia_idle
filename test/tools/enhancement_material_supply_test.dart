...
void main() {
  ...
  test('强化材料供需模拟:首通供给与 +15/+30/+49 期望需求同表输出', () {
    ...
    expect(supply.mojianshi, closeTo(66.5, 0.01));
    expect(supply.xinxueJiejing, closeTo(4679.5, 0.01));
    ...
    // Ch15 扩 75 关后 2124(≈8.05 件·264×8=2112·刚过 8 件上界抬至 9 件线·绝顶段收官续放宽·spec §8「沿放宽口径复核」已拍·[balance] 绝对值待用户终拍)。
    // Ch16 扩 80 关后 2450(≈9.28 件·264×9=2376·刚过 9 件上界抬至 10 件线·宗师段首章续放宽·沿逐章 +5 关自然增轨迹·spec §6「沿放宽口径复核」已拍·[balance] 绝对值待用户终拍)。
    // Ch17 扩 85 关后 2806(≈10.63 件·264×10=2640·刚过 10 件上界抬至 11 件线·宗师段第 2 章续放宽·沿逐章 +5 关自然增轨迹·spec §9「沿放宽复核」已拍·[balance] 绝对值待用户终拍)。
    // Ch18 扩 90 关后 3192(≈12.09 件·264×12=3168·刚过 12 件上界抬至 13 件线·宗师段收官续放宽·沿逐章 +5 关自然增轨迹·spec §7「沿放宽复核」已拍·[balance] 绝对值待用户终拍)。
    // Ch19 扩 95 关后 3607(≈13.66 件·264×13=3432·刚过 13 件上界抬至 14 件线·武圣段首章续放宽·沿逐章 +5 关自然增轨迹·章级 spec §5「沿放宽复核」已拍·[balance] 绝对值待用户终拍)。
    // Ch20 扩 100 关后 4047(≈15.33 件·264×15=3960·刚过 15 件上界抬至 16 件线·武圣段中章续放宽·沿逐章 +5 关自然增轨迹·章级 spec §5「沿放宽复核」已拍·[balance] 绝对值待用户终拍)。
    // Ch21 扩 105 关后 4512(≈17.09 件·264×17=4488·刚过 17 件上界抬至 18 件线·武圣段收官续放宽·沿逐章 +5 关自然增轨迹·章级 spec §5「沿放宽复核」已拍·[balance] 绝对值待用户终拍)。
    // 批 A 塔扩 49 层(2026-08-04)后 4679.5(≈17.72 件·仍不足 18 件·塔材料掉落随 49 层重排
    // 自然增·上界 18 件线**未动**·[balance] 绝对值待 A5 探针复校)。
    // **主线终章:此后无扩章来源再抬本上界**。
    expect(
      supply.xinxueJiejing,
      lessThan(to49.guaranteeCrystalCost * 18),
      reason: '105 关首通结晶支撑约 17.09 件 +49 保底,不足 18 件,不是无约束溢出',
    );
