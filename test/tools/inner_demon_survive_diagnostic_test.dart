...
///
/// ── 修订背景 ─────────────────────────────────────────────────────────────
/// 初版把 07 配成**纯 +40% 镜像 + survive**，并如实发现：无脆弱窗口时先手
...
/// 玩家爆发 → 击杀 +40% 镜像变难 → survive 撑关成为真实「熬过去」通道，且
...
/// 蓄力技，无需改注入代码），07 窗口外承伤 = 0.08（终关最狠，05:0.12 / 06:0.10）：
///
...
///      伤害被窗口 gate 到打不穿 +40% 镜像（自然斩杀需 ≥29 tick），但自身足够
...
///      窗口下窗口外免疫 + 镜像 +40% 反打 → glass cannon 被翻盘，死于 tick<20
///      （survive 无法救活已团灭的队伍）→ 稳定 ~20% 战败（16/20 胜）。这证明
...
///
/// 最终校准值：07 outOfWindowDamageMult = 0.08，winCondition.ticks (N) = 20。
...
void main() {
  ...
  test('(0) 前置：07 winCondition surviveTicks=N + 生产路径镜像有脆弱窗口', () {
    ...
    expect(vuln!.outOfWindowDamageMult, 0.08,
        reason: '07 窗口外承伤应 = 0.08（终关最狠）');
    expect(id.mirrorBuffPerStage['stage_inner_demon_07'], 0.40,
        reason: '07 镜像 +40% buff 不变');
    ...
    expect(mirrors.first.vulnerabilityMult, 0.08,
        reason: '07 镜像应注入 vulnerabilityMult=0.08（窗口活）');
    expect(mirrors.first.chargeSkillId, 'skill_inner_demon_charge');
    expect(
        mirrors.first.availableSkills.any((s) => s.id == 'skill_inner_demon_charge'),
        isTrue,
        reason: '蓄力技应进 availableSkills（AI 才选得到 → 开窗）');
  });

  test('(a)(c)(e) 击败通道 + 窗口真开 + 不变式：on-level 全 seed 窗口内斩杀取胜', () {
    ...
    final earlyKills = outcomes
        .where((o) =>
            o.result == BattleResult.leftWin && o.tick < _n && o.rightAllDead)
        .length;
    print('on-level: charged=$charged/$_seeds earlyKills=$earlyKills/$_seeds '
        'ticks=${outcomes.map((o) => o.tick).toList()}');

    // (a) 窗口真开：镜像每 seed 都至少蓄力一次（否则窗口永不开 = 永久免疫 no-op）。
    expect(charged, _seeds,
        reason: '07 镜像应每 seed 都蓄力开窗；实测 $charged/$_seeds');

    // (c) 击败通道：on-level 应全 seed 在 tick<N 团灭右队取胜（窗口内爆发斩杀）。
    expect(earlyKills, _seeds,
        reason: 'on-level 应全 seed 提前斩杀镜像取胜（击败通道确定性可达）；'
  ...
  test('(b)(e) survive 通道：龟缩耐战 build 全 seed 撑满 N 存活、右队未团灭取胜', () {
    final wc = repo.getStage('stage_inner_demon_07').winCondition!;
    // 龟缩/耐战：门派绝学 + 寻常货装（最低阶）+ 0 强化 + 体质 40。伤害被窗口
    // gate 到打不穿 +40% 镜像（自然斩杀需 ≥29 tick），但自身硬扛过 N=20。
    final turtle = _team(repo, 'tech_gangmeng_menpai', 0, 40,
    ...
    final surviveWins = outcomes
        .where((o) =>
            ...
            !o.rightAllDead)
        .length;
    print('turtle: surviveWins=$surviveWins/$_seeds '
        'ticks=${outcomes.map((o) => o.tick).toList()} '
    ...
    expect(surviveWins, _seeds,
        reason: '龟缩耐战 build 应全 seed 撑满 N=$_n、存活、右队未团灭（survive 通道）；'
  ...
  test('(d)(e) 真实难度：高爆发 BiS 在窗口 + survive 下仍有非平凡战败率', () {
    final wc = repo.getStage('stage_inner_demon_07').winCondition!;
    // 高爆发 BiS：传说神功 60% 强化（一击可秒 20k HP 镜像）。窗口下窗口外免疫
    // + 镜像 +40% 反打 → glass cannon 被翻盘，死于 tick<N（survive 救不活已
    // 团灭的队伍）。证明窗口 + survive 门**不是**初版「恒胜 no-op」。
    final bis = _team(repo, 'tech_gangmeng_chuanshuo', 60, 6);
    final outcomes = _sweep(repo, bis, wc);

    final wins = outcomes.where((o) => o.result == BattleResult.leftWin).length;
    final losses = outcomes
        .where((o) => o.result == BattleResult.rightWin && !o.leftAlive)
        .length;
    print('BiS: wins=$wins/$_seeds losses=$losses '
        'ticks=${outcomes.map((o) => o.tick).toList()} '
        'results=${outcomes.map((o) => o.result?.name).toList()}');

    // (d) 非全胜（窗口对纯爆发 build 制造真难度，alpha-strike 无法碾过）。
    expect(wins, lessThan(_seeds),
        reason: 'BiS 高爆发应有非平凡战败（窗口 + survive 门存在，非初版恒胜 no-op）；'
            '实胜 $wins/$_seeds');
    // 但也非硬墙（BiS 仍可攻略，主要靠击败通道）。
    expect(wins, greaterThanOrEqualTo(10),
