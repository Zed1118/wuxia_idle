...
> 📊 **2026-05-25 P4.1 §12.2 帮派门派 B1+B2 闭环 + v2.1+v2.2 工具完善 + T17-T22 全闭环 · 1.0 ~85% → ~87%**

**2026-05-25 P4.1 §12.2 帮派门派 B1 schema + B2 service+trigger ✅**(Mac+Opus xhigh ~1h+~1h · 1458 测全过 / 0 analyze):**B1 schema**(branch `feat/p4_1_sect_management_b1_schema` · 8 file):`SectRank` enum 三阶(Q5=A 组织 ≠ 修炼七阶) + `Character.{isInSect,sectId,sectRank}` 3 字段(Q2=C 双向 fk + Q3=A 复用) + `Sect.{territoryIds,memberCount}` 2 字段 + `data/territories.yaml` 6 territory 跨 §5.3 阶 2-5 + `numbers.yaml sect_management` 段 + `NumbersConfig.sectManagement` 强类型 + `TerritoryDef` static def + `GameRepository.territoryDefs` graceful 加载。**B2 service+trigger**(branch `feat/p4_1_sect_management_b2_service` · 5 file · commit `6ecc754`):`SectMemberService`(recruit/promoteRank/dismiss · caller 持锁) + `TerritoryService`(claim/release/availableForClaim/ownerOf) + sect_providers 扩 7 项(2 Service Provider + 3 Query + 2 Mutation Notifier) + **Q7 B mission hook 占位**(ResolveSectEventNotifier outcome=win + event.type=mission 分支 → 50% rng SaveData.recruitedDiscipleIds 池招入 · Demo 无 trigger,1.1 预埋) + **AscendService.performAscend sect.founderId rewire hook**(P5+ 真传位单 sect 假设)。**挂账**(spec §1 范围 OUT 已对齐):Q6 A encounter recruit / Q6 B stage_boss 招降 / founder_buff_service 作用域真扩 / multi-sect 跨派系语义 全留 1.1。speed 锚点 ×0.25-0.30(spec 估 B1+B2 7-9h · 实测 ~2h xhigh)。**B3 UI / B4 R5+closeout 留下波**。详 `docs/handoff/p4_1_b1_schema_closeout_2026-05-25.md` + spec `docs/spec/p4_1_sect_management_spec_2026-05-25.md`(149 行 ≤150)。
