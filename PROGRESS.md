...
**M4 PoC #46 美术 Stage 0+1+1.5 全收官 · 5 类型 PoC 通过 · Stage 2 量产 spec 起草**(2026-05-20,主对话 opus xhigh ~6h)。**Stage 0** 28 张候选锁 4 sref(主角色 / 备用 / 主环境 ⭐ 雪景古松 / 备用)→ **Stage 1** 5 件剑类 PoC + **修订方案 B 改类型** → **Stage 1.5** 防具+饰品追加。**5 大装备类型全验证 ✅**:剑 jian(铁/青锋/龙泉) / 刀 dao(盘龙刀)/ 鞭索(缠丝索)/ 防具(锦袍)/ 饰品(玉龙佩)— 7 件 × 2 = 14 张 + 1 alt = **15 张 PoC 产物**全规范命名于 `~/Desktop/MJ_Stage1_PoC/`。**核心 10 条教训 sealed memory** `feedback_mj_wuxia_prompt_pitfalls`:① jian/dao 锁定咒语;② **`--sw 50` 防 sref 风格污染**(非剑装备必带,关键);③ 印章接受作国画落款(GDD §1 一致);④ 3 重水墨锚定 `ink wash + sumi-e + monochrome` + 3 重防护 `--no oil painting, red, vibrant, cinematic`;⑤ 暖色词放大坑;⑥ 7 阶精致度梯度词汇(crude → divine);⑦ 黑名单词永禁;⑧ stylize 100/200/250 分级;⑨ 混合双轨 icon + detail;⑩ 实战锚点。**产物**:`art_poc_stage1_closeout_2026-05-20.md` + `art_poc_stage2_full_production_2026-05-20.md`(28 件装备待跑 + 3 角色 + 5 闭关 + UI ~10 张 = ~88 张 6 周批次规划)。**1.0 路线图加权 ~25% → ~30%**(M4 PoC 完工 4/4)。**下波**:Stage 2 W1 第 1 批寻常货剩 4 件(折刀/软鞭/粗布衣/玉佩 × 2 = 8 张 ~20min Fast)。
