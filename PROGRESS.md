...
- **2026-09-07 门派人数缓存修复候选（已批准 1A）**：分支 `codex/isar-sect-member-count-049-20260907`，代码 `5e0fb071a`，版本 `0.49.0`；启动时按核实的当前 founder 与成员关联重建负计数，冲突留存诊断并在重开时重试，招收入口拒绝异常负数。其他 14 项默认、62 项静态归位与旧迁移不变。定向 19 文件 195/195；build_runner、analyze 0 issue、全量 900 文件 6307/6307（新增 22 项）、format 1775 文件 0 改动、macOS release 均通过。未合 main、未 push、未部署或迁移实际玩家存档；完整边界及真实日志见 `docs/audit/isar_sect_member_count_repair_2026-09-07.md`。
