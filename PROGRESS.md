...
- 45. **0 引用 provider/字段清理**(T07/T08 audit 2026-05-19 新挂):T07 6 死字段(stage.narrativeId @Deprecated 等)+ T08 4 死 provider(leftTeamProvider/rightTeamProvider/unlockedCodexCountProvider P1.z 遗留未接 + gameEventServiceProvider)。删/扩 caller 下波 spec 起,详 `typedef_extension_audit_*.md` + `deadcode_scan_*.md`

> 已销账条目(#1-#36/#38-#42)详见末尾归档。**剩余 P1**:#37 / #43 / #44 / #45。

## 下一步

**下波候选**(优先级排):① **T08 unlockedCodexCountProvider 接 BaikeScreen chip**(P1.z 遗留 5-10min);② **T07/T08 死代码删除 spec**(优先 stage.narrativeId @Deprecated + 4 死 provider,30-60min);③ **T03 挂回 yu_zhong_qiao_men**(rain×inn 槽位空缺 0.5-1h Mac + DeepSeek);④ **#44 延续典故文案抽 yaml**(4-7h);⑤ **#43 高阶占位补齐**(P1.1 起手 5-8h);⑥ **美术 PoC + 水墨 LoRA**(opus + 用户主导 6-10h)。
