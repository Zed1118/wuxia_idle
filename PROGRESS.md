...
> 🔊 **2026-06-29 音频专属素材补齐分支完成(未 push · `codex/next-dedicated-audio-assets`)**:核查 worktree 与主项目 `assets/audio/_suno_candidates` 后确认暂无可直接入库的 `battleUlt` / `battleChargeStart` 专属候选;现有正式 mp3 仍为转用裁切素材(`battleUlt`≈`realmAdvance` 裁切 2.4s,`battleChargeStart`≈`defeat` 裁切 2.208s),不冒充最终素材。新增 `dedicated_audio_assets.dart` 只读状态表,把两个槽位标记为 `temporaryBorrowed` 并记录来源/目标时长,播放路径与 `SoundManager` no-op 降级不变;新增专属素材 manifest + Suno/SFX prompt + 独立计划文件。验证:`flutter test test/shared/audio/audio_assets_test.dart test/shared/audio/sfx_for_action_test.dart test/shared/audio/charge_transition_sfx_test.dart test/shared/audio/dedicated_audio_assets_test.dart` **26/26 passed**;`flutter analyze` **0 issue**。阻塞:仍需外部生成/筛选最终专属 mp3 后替换并把状态改 `finalAsset`。
