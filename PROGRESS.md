...
- **Batch 2.2.A vertical slice**(`71bd0a7` ~45min):InnerDemonDef domain(206 行 · fromYaml/empty fallback)+ InnerDemonService.isLayerLocked(55 行 · 非 wuSheng 短路 / qiMeng 跨 tier 放行 / wuSheng 内查 prev layer 拦截关 cleared)+ NumbersConfig.innerDemon 加载 + applyExperience hook 参数(EXP 留账不消费 §5.1)+ R1 14 测(unit 8 + integration 6)+ **1206 pass / 0 analyze ✅**(原 1192 + 新 14)。**spec 估 ~1h · 实际 45min · 精度 0.75×**。**Batch 2.2.A 调整**:spec doc Batch 2.2 列的 InnerDemonStrategy implements BattleStrategy 不建(YAGNI · BattleStrategy 是 tick 层,enemy 构造在 setup 层职责)+ InnerDemonScreen/BreakthroughBlocker UI 推 Batch 2.3 前(narrative 文案 ready 后 widget 才有内容)+ 3 callers wire 推 Batch 2.2.B 与 recordVictory + 镜像 enemy 一起(default null caller 0 改动,production 路径 hook 暂不生效)
...
**Phase 2 剩余 ~5-6h opus xhigh**(Batch 2.2.B 镜像 enemy + 3 caller wire + battle_resolution hook ~1.5h → 2.3 narrative ~1.5h + UI 占位 → 2.4 doc ~25min → 2.5 R2-R5 ~1.5h → closeout ~25min)。**1.0 进度 ~52% → ~56%**(P2.2 Batch 2.1 + 2.2.A 完成)。

**下波 候选**:① ⭐ **Batch 2.2.B 镜像战斗**(InnerDemonService.buildMirrorEnemyTeam + stage_battle_setup 分支 + battle_resolution recordVictory hook + 3 callers wire isLayerLocked + R2-R3 集成测 ~1.5h opus xhigh)② MJ Discord 派单 Ch4-6 enemy ~20 张异步 ③ Codex Pen Windows 视觉验收 Ch4-6 主线 narrative(Pen ~1.5h)
