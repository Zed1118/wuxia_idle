...
> 🔁 **2026-06-26 续2·副本屏chip收口 + 周目难度平衡(用户「123推进」·high·TDD·合main·HEAD 9caef667)**:① **3副本屏删逐关战斗方式chip**(`274ef468`):承接 9231e4ae(主线+爬塔已删),群战/心魔/轻功同删 `if(cleared) StageAutoPlayControl` 块+2无用import,全局「自动战斗」开关在设置面板,逐关冗余挤占列表(首通仍强制拖招·重打按全局);3屏无screen级widget测,删除机械镜像参考commit,analyze(无用import/编译)+全量兜底。② **周目平衡难度侧**(`9caef667`·用户 AskUserQuestion 拍「双调·温和」):`scale_per_cycle` 0.06→0.10(二周目敌+10%/三周目+20%)+ 主线二周目 assignment `[yuti]`→`[yuti, zhenqi]`(加真气·敌内力+20%多放一次大招),提升二周目区分感。**红线安全**:生产 stage_battle_setup `.clamp(0,bossHpMax=60000)` 强制·西凉霸主 cycle3 62400→clamp 60000不破§5.4·玩家伤害独立于敌人scale·balance_simulator peak仍 21万/13.5万不进百万(软红线稳)。同步4测文件(派生断言自动跟随·硬编码0.06/1.12/1.06 + traitsFor c2主线 + §7.2无clamp反例阈值52000→45000因60000/1.20=50000·逐条改·补c2含cycle_zhenqi正向断言)。analyze**0**/全量**3114+1skip**(0回归)。③ **周目平衡回报侧**(`f03d5b34`·xhigh):主线+扫荡二周目起回报加大,爬塔不变(rare bonus 仅首通触发·二周目非首通)。**稀有彩头 cycle 感知**(`RareBonusTier.chanceNgPlus`+`chanceFor`·selectRareBonusTier/rollRareBonus 加 cycle·numbers.yaml +1阶5%→8%/+2阶1.5%→3%·缺省=chance 向后兼容)+ **普通掉落材料加成**(新 `CycleDropBonusConfig`+`applyCycleMaterialBonus` 纯函数·二周目材料类 miscMaterial残页精铁/磨剑石/心血结晶 ×1.5 向下取整·装备/经验丹/秘籍/银两不动)。**cycle 透传**:`BattleResolutionService.resolve` 加 cycle → `applyVictoryResolution`(主线 runStageFlow 传 targetCycle·扫荡 settleMainlineSweepVictory 传 cycle)。红线全守(§5.3 高阶可拿不可装/§5.4 仅放掉落量/§5.1 不放货币门控物)。TDD +16(rare bonus cycle4+材料加成10+真yaml契约1+resolve级1)。analyze**0**/全量**3130+1skip**(0回归)。**balance 初值待真机校**(scale 0.10/真气/彩头 8%·3%/材料 ×1.5)。

> 🔁 **2026-06-26 续·周目可见化 5 改(用户实玩反馈逐轮·TDD·合 main·HEAD 45f171ce·commit `b48e0629..45f171ce`)**:用户反馈一键扫荡/周目系统不透明(选二周目却处处显一周目、打完跳回、敌人无区分提示)。① **扫荡周目可见化**(`b48e0629`):扫荡按钮带「第N周目」标签(主线+爬塔)+主线周目选择器上移到按钮上方(先选后扫)+未达门槛从整块隐藏改灰显「第N周目需先手工通关全部关卡」(§5.7 灰掉·仅通关过的章/塔显·全新仍隐藏)+战报「扫荡·第N周目」行+连播 HUD 周目徽章。② **周目可见化2**(`ca8f7e4b`):周目选择改响应式(`selectedChallengeCycleProvider` `ref.read`→`ref.watch`·选周目即时刷新按钮/关卡)+关卡状态按周目算(`statusFor`·cycle≥2 本周目通过显已通关、未通显可挑战·修旧 bug 沿用首周目 clearedStageIds 全显已通关的误导)+战斗屏 cycleHint 改 `battleCycleHint(N)` 明确「第N周目·敌人更强(属性提升·额外反制词条)」。③ **keepAlive 修选定周目丢失**(`bf007431`):`SelectedChallengeCycle` `@riverpod`→`@Riverpod(keepAlive:true)`——选二周目打完一关进战斗导航后 autoDispose 回收致选择重置回一周目(非Boss关不写章周目key故退回1)·改会话级保活(主 checkout 需重跑 build_runner)。④ **删逐关战斗方式 chip**(`9231e4ae`):主线列表+爬塔重打 dialog 移除逐关「自动/拖招」覆盖(设置面板已有全局「自动战斗」开关·逐关冗余挤占·首通仍强制拖招·3副本屏未点名未动)+掉落悬停卡 bottomLeft→bottomRight 右对齐。⑤ **悬停卡 IgnorePointer**(`45f171ce`):浮层向下盖住后续关且自身吃鼠标→跨关切不动+边界卡顿·改 IgnorePointer 纯展示鼠标穿透·切换丝滑(长清单滚动交 ⓘ 弹窗)。**保持§5.7门槛**(用户拍板:扫第N周目须先手工通该周目全部关卡)。**平衡未动**(用户拍板先不动):二周目敌人仅 +6%三围+御体词条、掉落同一周目·已记 backlog §十一待拍板。analyze**0**/全量**3114+1skip**(基线3104·**+10**·0回归·主checkout实测)。**未真机目检**:各改均 debug app 跑起,视觉手感待用户确认(周目标签/灰显/关卡状态/战斗横幅/悬停丝滑)。
