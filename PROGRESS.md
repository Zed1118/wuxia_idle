...
> 🩹 **2026-06-29 角色伤势状态表现增强完成(独立 worktree `codex/night-tier2-injury-state-visibility`)**:完成第二梯队「角色伤势状态表现增强」。新增集中伤势状态 formatter/widget,角色卡档案头明确显示「无伤 / 带伤 / 重伤」及影响,受伤时复用 `ItemUseService` + 疗伤丹 def 提供恢复入口;战前情报新增「我方伤势」段,显示出战角色伤势;主线战后摘要新增伤势行;既有战后疗伤丹面板追加当前伤势行,覆盖普通 overlay / 主线 / 爬塔已接入口。**不新增治疗经济 / 不改 numbers.yaml / 不改 schema/saveVer / 不依赖暂缓塔 Boss 分支**。验证:`dart run build_runner build --delete-conflicting-outputs`;targeted `flutter test --no-pub` 70/70 passed; touched-file `dart analyze` 0 issue。剩余风险:未做 macOS 真机视觉目检,文本密度待集成后看一眼。
