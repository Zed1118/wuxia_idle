...
> **2026-06-23 续47-48 已压缩归档**(git log/audit `drop_consistency_2026-06-23.md`/各 session 可溯·2826→2848测):续47 子系统 A 掉落表一致性审计(4 路只读扇出 47+30 dropTable 逐条核·整体健康)+ F1 真 content bug 实装(3 件 special 装备 dropSourceTags 声明授予通道但 0 实装→`MilestoneEquipmentGrantService` 按 tag 筛授·saveVer 0.27→0.28·群战/心魔/飞升 post-victory hook·`docs/spec/2026-06-23-f1-milestone-equipment-grant-*`)·续48 子系统 B F2 主线 preview 首通门控偏差全面修(`fromDropTable` 表级布尔→`FirstClearGating` 枚举 scrollOnly/wholeChannel 逐条判 gated·抽 `isTechniqueScrollDefId` 谓词 runtime/enums/preview 三方共用·footer「秘籍首通必得,重打不补」·11 文件·掉落行为零变)。
