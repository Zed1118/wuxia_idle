...
> 🌙 **2026-06-29 codex/night-tower-solo-boss-mechanics 爬塔单人路径 + 20/25/30 Boss 机制增强完成(独立 worktree/分支)**:按 CLAUDE §8.0 先建计划 `docs/superpowers/plans/2026-06-29-night-tower-solo-boss-mechanics.md` 并小切片提交。**改动**:`data/towers.yaml` 后段 Boss 不再固定单体:20/25/30 改主 Boss + 护法/影侍/左右使多目标压力,20 层补门派绝学阶段招,25/30 从二阶段扩三阶段,三层均有 `schoolDamageTakenMult` 弱点/抗性 + `bossPhases` focus/aggressive + `chargeCounter` 反制窗口;5/10/15/20/25/30 主 Boss 全标 `isBoss:true`。加载红线从「Boss 固定 1 敌」改为「Boss 层 1-3 敌且至少 1 个 isBoss 主敌」,保持塔解锁仅按 `highestClearedFloor + 1` 顺序进度,不按弟子/队伍人数锁门。**验证**:`flutter test --no-pub --no-test-assets test/features/tower/domain/tower_floor_def_test.dart test/features/tower/application/tower_progress_service_test.dart test/data/boss_phase_redline_test.dart test/data/weakness_redline_test.dart` 53 pass;`flutter analyze` 0 issue;轻量 Ruby 结构检查 20/25/30 多目标/阶段/弱点/血量红线通过。**风险**:未做真机手感与全量模拟;新增护法可能让单人 on-level 后段更硬,预期靠高境界/超阶与破招窗口推进,后续可用 balance_simulator 做胜率校值。
