...
## 当前阶段（2026-09-13 旧档迁移核验）
- **当前候选与验收边界**：本单基于 `a48aab668` 的 `codex/save-migration-verify-20260912`，仅本地文档提交；main 保持 `342d1927529b8306582431be597ef1975bd69deb`。正式 M0–M9 仍 1/10（仅 M1），工程生产迁塔 7/49、main 0/49。
- **已完成·候选内五项**：`c93b50512` 标注未接线配置并补真气未消费守卫；`7db8e6983` 外置迁层阻塞闸门的翻转权；`e88b4c701` 补塔层平价测试的目标层丢弃前提断言；`0ec23aa0f` 补记两个 commit 的 yaml 影响清单；`a48aab668` 修正塔层目标比对的错误结论并接入真实映射。
- **已完成·另线待集成**：`2b35f7eae` 收紧塔层目标 clause 形状断言，是 a48 的直接后继，尚未进入本候选；由另一单负责并入，本单未合并它。
- **已验证·本次真实副本**：3 个容器槽实际原版均为 `0.40.0`；main 注入目录开档后均为 `0.49.0`（此为 main 迁移终点，非原始版）。另一组副本直达 `0.50.0`，另复制 main 终点完成 3 槽 `0.49.0→0.50.0`；两路径每槽全部 26 类持久化数据相同。
- **已验证·数据与旧链**：3 槽角色/装备/背包分别 6/111/16、1/6/3、1/11/4，塔/主线原进度逐字段保留；新余数均 0.0，结算锚点取原 lastOnlineAt，桃花岛 3 项加工库存转为正确物品且数量守恒。8 月 28 日备份实读 `0.40.0→0.50.0`，6/97/16 保留；4 样本迁后重开均幂等。
- **已验证·回归证据**：本次复核并复用 exact a48 原日志 `flutter test --coverage --no-pub`：6562 PASS、0 失败，墙钟 870.316 秒；coverage 49118/56864=86.38%（阈值 81.25%）。本次另跑 format：1702 文件、0 changed；analyze：0 issue（命令与日志见迁移报告）；未争用 CI 专单的全量测试锁。
- **已验证·CI 与保全**：在线复核 run `34698815782` exact a48 的 macos-build success；仅证明编译。源 3 槽、指定历史备份及主工作区 4 个用户文件 SHA-256 全部保持；未启动应用、未写容器/-lck、未 push/合 main，lib/配置/闸门/CI 文件零改动。
- **已知风险·CI 预算**：run `34698815782` test 因超过 30 分钟被取消。main 上次成功 run `34177493561` 的 test job 为 28m06s，余量 114s；28m10s 是整个 workflow 耗时。修复另有专单，本单不改 CI。
- **已知风险·多敌迁层**：生产已迁 1–7 层全为单敌；49 层实际为单敌 13、双敌 5、三敌 31，8 层首次双敌、14 层首次三敌，11 等层有例外。8–14 已有独立 60 组两路侦察，不能再称“多敌一层未验”；仍未进入统一迁层回归、完整入口/持久化及真人验收。
- **已知风险·验收缺口**：GUI 真机开档未做，以保持容器原档全程只读；不代签 Windows/真人、M4 性能或原主线点击超时根因。迁移结论仅覆盖本次实际样本，不代表全部历史版本已穷尽。
- **下批建议**：由 CI 预算专单统一集成/推送并复核新 exact SHA；下一批迁层前由用户翻转 `review_gaps_block_floor_migration`。当前值仍 false，registry 已规定只能用户翻转，执行链不得自清后扩层；本单保持生产集合 1–7。
- **证据**：[本次迁移报告、完整快照与哈希](/Users/a10506/Desktop/wuxia_save_migration_probe_20260913_091939/report.md)；[复核的原全量日志](/Users/a10506/Documents/Codex/2026-09-12/wuxia-ci-save-migration-215936/test-coverage.log)。
- **另线侦察证据**：[8–14 报告](/Users/a10506/.codex/worktrees/tower-multi-recon-20260912/挂机武侠/docs/audit/phase2_m7_tower_8_14_multi_enemy_recon_2026-09-12.md)，`codex/tower-multi-recon-20260912` / `1aaa08940`；该报告不在本候选，相对路径不可当已集成证据。
### 历史集成记录（以下为当时状态）
