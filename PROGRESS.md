...
> 🔧 **2026-06-11 进行中(爆品展示动画 + reward 重做 · worktree `worktree-treasure-drop-anim` `b3b84c8b` 待真玩验收合 main)**:brainstorming→spec→plan→subagent-driven 8 task 全实装(逐 task 两段 review + 整体终审 ready-to-merge)。高阶装备(重器+,`numbers.yaml treasure_drop.min_tier` 可配)掉落先播印章盖落全屏动画(墨团炸开背景 + 绛红印章 tier 题字 + 震屏 + tier 色梯度青铜/紫/金,复用 caption_ink_blob+srcIn 染色 / EquipGlyph 兜底)再进结算弹窗;reward 音效门槛化(只重器+爆品响,删主线/塔两处旧无条件 reward,realmAdvance 保留且 tower 改到动画后)。worktree **1946 测/analyze 0**。**待办**:① 真玩验收动画视觉(srcIn 染色是否 wash out / 印章爽感 / 震屏 ±5px,reviewer 两次提示)② 满意 `git merge --ff-only` 合 main ③ reward 新素材(Suno prompt `docs/suno_reward_treasure_sfx_prompts_2026-06-11.md` 已写,旧 reward 占位)。spec/plan 在 worktree `docs/superpowers/{specs,plans}/2026-06-11-treasure-drop-*`。
