...
**P1 #44 延续典故文案抽 yaml · Mac 端 wire + 红线 case spec + 红线 case 实装(默认 skip)**(2026-05-19 下午+晚,sonnet ~50min+0.3h+0.5h)。LoreContent 加 `continued_lore_obtained` / `continued_lore_boss_defeated` 双池 + GameEventService 走 LoreLoader 纯随机抽 + 占位符替换(`{source}`/`{boss_name}`/`{stage_name}`,`{equip_name}` yaml 直写)+ fallback 兜底。spec `docs/handoff/p1_44_red_line_acceptance_spec.md` + 实装 lore_loader_test.dart 加 2 case(5 strict + 1 soft):漏件 [3,5] / 占位符白名单 / 占位符语义分池 / 长度 ≤300 / 网游词黑名单 / 文风审计 warning,**默认 skip 等 DeepSeek 35 件文案落地后去 skip 启用**。test +6+2 skip case → **1117 pass + 3 skip(原 1 + 新 2)+ 0 issues**。DeepSeek 派单 spec `docs/handoff/p1_44_deepseek_continued_lore_spec.md` 待补 35 件各 2 池 3-5 条文案(预估 3-5h)。
