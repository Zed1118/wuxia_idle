...
> ✅ **2026-06-29 新档目标引导二期完成(分支 `codex/next-new-save-goal-guidance` · 待合并未 push)**:新增 `NewSaveGoalGuidance` 只读派生,从 `MainlineProgressService.availableStages` + `StageDef.dropTable/dropSkill*` 推出当前第一条自然目标。主菜单「主线」hint 显示“打哪关/取什么/为什么”;关卡列表仅当前可挑战关显示轻提示;战前情报弹窗补「当前目标」段。`DropNameResolver.itemName` 优先读 `data/items.yaml` 真名称。**边界**:不做教程弹窗/任务系统/奖励/存档字段/数值改动。验证:`flutter test --no-pub test/features/mainline/new_save_goal_guidance_test.dart test/features/main_menu/presentation/main_menu_test.dart test/features/loot_preview/stage_row_loot_wiring_test.dart test/features/loot_preview/stage_intel_dialog_test.dart` **64/64 passed**;`flutter analyze` **0 issue**。
