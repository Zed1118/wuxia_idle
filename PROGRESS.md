...
> 🌙 **2026-06-29 今晚第一梯队 8 分支评审→合并(7 合入 main · 1 暂缓 · 未 push · 未清 worktree)**:8 个独立 worktree 夜间任务,8 并行只读子代理审查(全判可合并·0 越界/0 红线破)→按 UI→domain→data 顺序 --no-ff 合并。**合入 7**:招式熟练度可视化二期 `ae6b46ca` / 离线收益回归卡 `e20c8c55`(纯展示·守在线=离线) / 商店货架需求提示 `1b84162d` / 主线重打收益路线 `65a61c06`(纯展示·非首通差异) / 装备掉落后即时处理 `d102aae6`(复用 setLocked/source) / 桃花岛产物消费+疗伤丹战后接入 `50accc12`(新 `inventoryQuantityByDefId` provider·消费锻材/开锋辅材/行囊补给·复用 ItemUseService 恢复模型) / 单人主线 01_05→06_05 平衡 `40cd3230`(stages.yaml 单人 1v1·Boss 血全降 max32000<60000)。**暂缓 1**:塔 20/25/30 Boss 机制增强(含爬塔单人路径,branch `night-tower-solo-boss-mechanics` 已 `git reset` 回退)——**质量/验证缺失**:floor30 二阶段对 on-level ceiling 仅 4/20(<80% 阈值)触发,boss 血 20000 未随机制上调致相位常打不出;破一个 pre-existing 体感诊断测(分支未跑)。待真机校 + 重调相位 HP 阈值后人工拍板再合。**合并后修测试漂移 `0c924f0e`**:taohua widget 测补 `inventoryQuantityByDefIdProvider` override;solo-balance 单人敌阵数/Boss 数值锚同步(03_05 右队 1·maxHp9000·05_05 24000/1500·06_05 32000);ch4/5/6 跨阶红线 3 人队下边界随 solo 重设计放宽(clearability 由 solo_mainline_ch1_ch6_balance_test 覆盖)。**复验**:build_runner 重生 .g.dart · `flutter analyze` **0 issue** · `flutter test --no-pub -j1` **3331 passed/1 skip/0 fail**(主仓实测)。**未 push·未清 8 worktree/分支**(含暂缓 tower 分支留待续做)。**挂账**:7 合入功能真机目检(victory 面板/商店提示/疗伤丹/回归卡排版)+ tower 暂缓项重做。
