...
> 🌙 **2026-06-29 今晚挂机任务 11 · 扫荡前收益预估完成(独立 worktree `codex/night-tier2-sweep-preview`)**:主线整章一键扫荡入口新增「扫荡前预估」紧凑面板,章级汇总主收益类型(刷装备/刷材料/练熟练度)、可能掉落、熟练度方向(秘籍/残页/敌方蓄力技)与材料缺口命中。实现为只读 `SweepRewardPreview`,复用 `MainlineReplayRewardRoute`、`StageDef.dropTable`、`MaterialSourceLookupService`、`ItemUsageLookupService`;仅在当前周目 eligible 时显示,不做逐关掉落缺口标记。**零改收益结算 / 掉落配置 / numbers.yaml / schema / saveVer**,不新增日课、体力或加速。计划:`docs/superpowers/plans/2026-06-29-night-tier2-sweep-preview.md`。验证:`dart run build_runner build --delete-conflicting-outputs`;`flutter test --no-pub test/features/sweep/domain/sweep_reward_preview_test.dart test/features/mainline/presentation/stage_list_screen_test.dart test/features/mainline/domain/mainline_replay_reward_route_test.dart test/features/inventory/material_source_lookup_service_test.dart test/features/inventory/item_usage_lookup_service_test.dart` **26 passed**;touched-file `flutter analyze` 0 issue。提交:`e9ed29b3` / `eebf68a9` / docs closeout 见 HEAD。剩余风险:未做 macOS 真机视觉目检,面板信息密度待集成后看一眼。
...
> 🛒 **2026-06-29 商店货架需求提示完成(独立 worktree `codex/night-shop-need-hints`)**:完成 backlog「商店货架需求提示」小切片。**改动**:新增 `ShopNeedHintService` 只读聚合 `ItemUsageLookupService` + `MaterialSourceLookupService` + `items.yaml` 具体品名 + 当前 active 角色;商店货架 tile 显示「当前可用」「消耗系统」「其他来源」三类提示,经验丹显示凝神丹/培元丹具体名并标出祖师可用,磨剑石/心血结晶显示装备强化/保底等消耗。**不改** `ShopService.purchase/effectivePrice`、`data/shop.yaml`、价格/刷新/折扣/限购机制。计划文件:`docs/superpowers/plans/2026-06-29-night-shop-need-hints.md`。验证:`flutter analyze` 0 issue;`flutter test --no-pub test/features/shop/shop_need_hint_service_test.dart test/features/shop/shop_screen_test.dart` **14 passed**。剩余风险:未做真机视觉目检,提示信息密度待集成后人工看一眼。
> 🌙 **2026-06-28 今晚挂机任务 06 · 主线重打收益路线完成(独立 worktree `codex/night-mainline-replay-rewards`)**:已通关主线关卡行新增「重打路线」紧凑提示,从现有 `StageDef.dropTable` / 技能掉落 / 敌方蓄力技纯派生「刷装备 / 刷材料 / 练熟练度」三类回头打价值；未通关/锁定关卡不显示,不干扰首通目标。**零改掉落结算 / numbers.yaml / schema / saveVer**,不新增体力、日课、限时奖励或在线加成。计划文件 `docs/superpowers/plans/2026-06-29-night-mainline-replay-rewards.md` 已更新恢复点。验证:`dart run build_runner build --delete-conflicting-outputs`(当前版本忽略该参数,生成 112 个 gitignored outputs)；`flutter test test/features/mainline/domain/mainline_replay_reward_route_test.dart test/features/mainline/presentation/stage_list_screen_test.dart`；`flutter test test/features/loot_preview/stage_row_loot_wiring_test.dart`；touched-file `dart analyze` 0 issue。提交:`43d23b99` plan / `ee8661a4` domain / `6b974e43` UI / progress 登记见分支 HEAD。
> 🌙 **2026-06-28 codex/night-drop-equipment-actions 装备掉落后即时处理完成(独立 worktree · 2 commits)**:胜利结算装备掉落行从纯展示扩为轻量处理块:锁定/解锁、标记常用(复用锁定保护,不增 schema)、查看来源(复用 `EquipmentSourceLookup` 真反查)、稍后处理(本地弱化无副作用)。主线 victory 流程接 `EquipmentService.setLocked`;不提供出售/分解入口,不绕过 locked/heritage/equipped 保护语义。**零碰 numbers.yaml/saveVer/Isar schema/掉落 roll**。验证:`flutter test --no-pub test/features/mainline/presentation/stage_victory_dialog_test.dart` 26 passed;`flutter analyze` 0 issue。计划:`docs/superpowers/plans/2026-06-29-night-drop-equipment-actions.md`。
> 🌸 **2026-06-29 codex/night-taohua-consumption-healing 桃花岛产物消费补齐 + 疗伤丹战后接入(独立 worktree)**:按 CLAUDE §8.0 写计划 `docs/superpowers/plans/2026-06-29-night-taohua-consumption-healing.md` 并小切片提交。**闭环**:锻材接装备强化附加消耗(+11 起,低阶 0 成本不挡新手);开锋辅材接开锋持久化扣料(槽1/2/3=1/2/3);行囊补给接清轻伤道具效果;新增战后疗伤丹面板,挂普通胜负 overlay / 主线胜利 dialog / 爬塔胜利 dialog,仅有药且有伤时显示,复用 `ItemUseService` 原子扣药与更新伤势;用途反查覆盖锻材/开锋辅材/行囊补给。**验证**:`dart run build_runner build --delete-conflicting-outputs`;`flutter analyze` 0 issue;`flutter test --no-pub -j1 test/features/equipment/application/enhancement_service_test.dart test/features/equipment/application/enhancement_persist_test.dart test/features/equipment/application/forge_persist_test.dart test/features/equipment/application/forging_service_test.dart test/features/inventory/item_use_service_test.dart test/features/inventory/item_usage_lookup_service_test.dart` **68/68 passed**。**零 schema/saveVer/在线离线收益改动**;未做大视觉验收。
