...
> ✅ **2026-06-18 续26(第七阶段·P2/P3 续作 批一·战后体验 · brainstorm→spec→plan→6 task subagent-driven TDD · 分支 `worktree-phase7-postbattle-hero-camera` · opus xhigh)**:"做 2+3" 拆三批(批一·战后体验 / 批二·Boss 机制 / 批三·队伍成长),本批=战后体验。Phase 0 摸排纠正文档 drift:**`BattleDiagnosis` 五规则三段式早已完整 wire 进 `victory_overlay`**(backlog 标 `[ ]` 误),故战后复盘本批不改;真缺口=**英雄镜头(新建)**+ 珍稀掉落触发细化。**拍板**:① 英雄镜头=立绘切入(复用现成 founder/弟子 png 无需新美术)② 触发=仅 Boss 首胜(主线章末+爬塔大Boss)③ 出镜=本场最高输出 ④ 珍稀掉落按 spec 细化。6 task:T1 `TopDamageContributor.from`(actionLog 按 actorId 聚合玩家方取 max,纯函数)· T2 `post_battle.hero_camera` numbers + `HeroCameraConfig` + UiStrings · T3 `HeroCameraOverlay` 立绘切入(AnimationController 滑入放大 + errorBuilder 退化 + once-guard 自消失/点击跳过)+ `presentHeroCamera` · T4 主线接线 + `shouldShowHeroCamera` gate 谓词 · T5 抽 `deriveHeroCameraData` 共享 helper + 爬塔对称接线 · T6 珍稀掉落细化(`pickTreasureHighlight` extraDisplayTiers + `computeFirstAcquisitionTiers` 利器首次「库存总数≤本次件数」判 + 双 flow,**重器+每次保留/min_tier 未动**)。每 task spec+quality 两阶段 review(修:印章对齐 sibling 48/六、once-guard 双向测、抽 helper 消重复、isLoaded 守卫等)。亲测全量 **2442 测 +1 skip**(baseline 2418 **+24** 零回归)/ analyze **0**。**红线全守**(§5.4 纯表现层不写 BattleState/不调伤害公式 · §5.1/§5.4 不改掉落经济/概率/min_tier · §5.5 离线走 offline_recap 不经 ceremony 天然不触发 · §5.6 yaml+UiStrings/EnumL10n · §5.7 爽感走表现层)。**deferred**:战后复盘「队伍」跳转→批三 · 技能书/残页珍稀展示→批二(走独立 skill-drop hook,真解源自章末Boss首胜与 Boss 掉落改造同批)· 材料/熟练突破展示→P4 · 场景 zoom→不做。**已知小缺口**(低风险):`computeFirstAcquisitionTiers` 无独立 isar 单测(逻辑 5 行 + 文档化 + 135 mainline/tower e2e 覆盖集成路径)。**视觉验收**:英雄镜头=立绘滑入/缩放运动,单帧截不出 → 待真机 `flutter run -d macos` 打章末Boss首胜目检;珍稀掉落利器首次可静态截。spec+plan `docs/spec/2026-06-18-phase7-postbattle-hero-camera-*`。**下一步**:合 main + push;真机目检;或批二 Boss 机制标准 / 批三 队伍成长。
