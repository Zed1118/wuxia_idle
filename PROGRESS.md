...
> 🌙 **2026-06-29 第二梯队 · 主线章节推荐刷点完成(独立 worktree `codex/night-tier2-mainline-farm-spots`)**:完成 backlog「主线章节推荐刷点」。章节页在本章 5 关全通后显示「通章刷点」,最多 2 个推荐关卡,从已合入的 `MainlineReplayRewardRoute.fromStage` 纯派生「刷装备 / 刷材料 / 练熟练度」价值;未通完整章不显示。**不改**掉落结算、收益规则、扫荡、周目回报、numbers.yaml、schema/saveVer;不追踪个人缺口,不做关卡整备条二期。计划:`docs/superpowers/plans/2026-06-29-night-tier2-mainline-farm-spots.md`。验证:`flutter pub get`;`dart run build_runner build --delete-conflicting-outputs`(当前版本忽略该参数,写 112 个 gitignored outputs);`flutter test --no-pub -j1 test/features/mainline/domain/mainline_replay_reward_route_test.dart test/features/mainline/presentation/stage_list_screen_test.dart` **15/15 passed**;touched-file `dart analyze` 0 issue。剩余风险:未做真机视觉目检,推荐排序为纯派生启发式(路线丰富度+Boss/熟练度权重),后续可按实玩微调但不影响收益。
