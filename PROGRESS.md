...
> 🔍 **2026-06-29 外部审查(WorkBuddy)分诊 + 安全批 6 项实装(独立 worktree `review-fixes-safe-batch` · 未合 main)**:对 4 份外部审查文档逐条证伪后只做低风险项,设计/平衡项另出报告待拍板。**已做 6(全量回归每条绿)**:P0-1 可选 yaml 加载区分文件不存在(静默)/解析失败(FormatException 附文件名 rethrow)`14e5050a` · P0-4 新建 `ErrorFallback` 水墨兜底替 12 屏 ~20 处 `'load error: $e'` 原始异常暴露 `6db71152` · P0-5 `_migrateSaveData` 段1(<0.18)/段3(<0.22)补版本门去隐式启动契约 `41d9319d` · P0-2 新建 `InkLoadingIndicator` 墨晕 loading 替 54 处 `CircularProgressIndicator`(含 splash) `6901e78b` · P2-3 battle_screen 3 hex 魔数色入 `WuxiaColors` `a16ce903` · P2-6 `PlaqueButton` 去 InkWell 水波纹改按下暗层 `ee9c2630`。TDD 共 +20 测,末态全量 **3391 passed/1 skip/0 fail**(worktree 实测·基线 3371→+20)。**跳过(前提为假/违原则)**:P2-7 festival(`todayFestival` 实被 encounter festivalRequired 消费非 stub) · P2-8 死枚举 · data→features 大重构(Isar 约束) · 删 pvp(违 v1.24)。**待拍板**(`docs/review_fix_triage_2026-06-29.md`):P0-3 战斗屏性能(需专会话+真机 profiling) · P1-1 AI 流派个性/P1-2 群战回血(平衡决策·撞测试) · P1-3..6 表现层增量(需目检) · P1-7 删 baike(前提为假·baike 是百科 hub·codex 是其 tab·不删)。**挂账**:P0-2/P2-6 视觉手感真机目检。外部审查多处事实错误,作 backlog 线索可用,不可顺序盲执行。
