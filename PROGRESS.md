...
**2026-05-21 晚 候选 2 Ch4「西出阳关」Phase 2.1+2.1.5+2.2 实装 ✅**(Mac+Opus xhigh ~1.5h,commit `4f7fb6d` push):**5 关 stages.yaml entry 落地 + 主线红线放开 4 章 20 关 + UI/strings 适配 + 5 test fixture 适配**。**Phase 0/1 spec 漏检披露 ⭐**:`_enforceMainlineRedLines` 硬绑 15/3 章 + ChapterListScreen `_chapters [1,2,3]` 硬绑 + 5+ test 硬绑 15 全包修(Phase 0 4 维 grep 漏 schema/UI/test fixture 维度)。改动总览:① enums.dart EncounterBiome + desert/frontier(0 switch case 漏检 — 走 byName 解析)② game_repository.dart 红线放开动态 5*chapterCount + chapterIndex 必须连续从 1 起 ③ chapter_list_screen.dart 4 章 + strings.dart chapter4Title「西出阳关」+ chapter4Hint ④ stages.yaml 5 关 entry(数值 HP 7,200→15,500 / Atk 720→1,250 / Speed 165→215 全在 §5.4 红线内 / dropTable zhongqi_qing_xu_jian 1.0 给 Ch5 起步 + baowu_jin_si_jia 0.4 Ch3 雁门 lore 串联)⑤ Isar codegen 重生 encounter_progress.g.dart index 15/16 ⑥ 测试 fixture 全 4 章 20 关 8 文件 395+ inserts。**1177 pass(原 1172 + Ch4 5 关 e2e battle test)/ 0 analyze**。narratives 留 Batch 2.3 + GDD/PROGRESS/ROADMAP 同步留 Batch 2.4 + R5 末 Boss 跨阶红线压测 case 留 Batch 2.5。

**Phase 2 剩余**:① **Batch 2.3 narratives**(opus xhigh ~3-5h,章首尾 chapter_04.yaml ~1,300 字 + 12 stages narrative ~4,840 字,**用户介入精修章首尾 + 末 Boss defeat**)② **Batch 2.4 同步**(GDD §12.5 P2 启动备注 + PROGRESS + ROADMAP_1_0)③ **Batch 2.5 收尾**(R5 红线压测 case + closeout)。
