...
> **v1.55 变更**(2026-08-25 二阶段宗门行止当前态纵切):宗门 Hub 顶部已只读接入真实当前掌门、闭关/远征/断魂庄占用、active 远征深度/战败与断魂庄关次/阶段；身份或占用异常时 fail closed。七条原路由/门控不变，不虚构疗伤/听剑占用，不恢复已拒绝的统一完成报告。这只关闭 U07 当前态摘要首缺口，U07/M6/二阶段仍开放；不改业务写入、schema、YAML、调优、奖励、经济或解锁。
>
