...
- **软可读区间（极值满 build 实战可见值 · 保读得懂）**：普通伤害典型 build 落 2,000~8,000 设计目标；满强化神物极值 build（+49 ×3.45 × 共鸣 ×1.30 × 双攻击开锋 ×1.35 × 极境修炼 ×3.0）普攻实战峰值约 5.8 万（暴击约 8.7 万），**不进十万**（与大招暴击同档），绝对天花板**不进百万**。核心是战力数字保持玩家一眼能读懂、不进百万级膨胀。守卫见 `test/balance/full_build_damage_redline_test.dart`。
