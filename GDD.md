...
> **v1.50 变更**(2026-08-25 二阶段江湖地图守城第三地点纵切):“守城试炼”从主菜单平铺迁为地图第三个生产地点；锁定与五关进度只读 `MainlineProgress`、`MassBattleService` 和生产配置，数据未决或链异常时 fail closed，原战斗守卫与 `MassBattleScreen` 去向不变。这只关闭守城第三地点缺口；其他地点、统一地点详情、U06/U14/M5/M6/二阶段仍开放，不改 schema、YAML、数值、阵型、概率、奖励、经济、解锁或叙事。
>
