...
>
> **版本**:v1.65
> **v1.65 变更**(2026-08-25 二阶段九霄塔实际参与者生产纵切):九霄塔每次挑战现从当前掌门与当代存活门人逐次选择 eligible 空闲角色，同一角色 ID 经真实 `Phase0aTowerBattleHost` exact snapshot 进入战斗；胜负 settlement 均严格绑定单一实际参与者，成长、伤势、装备战斗次数与心法使用归该角色。身份/代际、死亡、疗养、无主修、活动占用、悬空装备或错人结算均 fail closed，不回退掌门。塔层、周目、首通奖励、重打、排行榜与仪式不变。该纵切只关闭塔逐次选人与既有个人战斗账本归属子门；每角色塔层最好成绩仍因缺持久模型保持 BLOCKED，不晋升 M6/二阶段，不改 schema、YAML、调优、奖励、经济、解锁或叙事。
> **v1.64 变更**(2026-08-25 二阶段掌门支线准入生产纵切):百草岭与断魂庄的真实候选、地点详情和写事务现允许存活、空闲且有主修的当前掌门参与；掌门身份与活动占用分别消费 `CurrentLeaderResolver` 和 `CharacterOccupancyService`，在候选与事务两层复核。无效/悬空掌门、历史祖师、死亡、无主修、闭关或重复占用均 fail closed，不回退其他角色。原单人、方针、周目、补给、恢复、战斗、离线推进、结算、召回、奖励和返程不变。该纵切只关闭掌门参加两项支线的 M6 必要子门，不晋升 U08/M6/二阶段，不改 schema、YAML、调优、奖励、经济、解锁、叙事或战斗。
>
...
| **引擎** | Flutter Desktop |
| **商业模式** | 买断制（无内购、无广告） |
| **联网形式** | 纯单机（云端排行榜为未来方向；当前本地榜 + Noop 同步抽象，零网络依赖） |
| **美术风格** | 写实武侠风（金庸 / 古龙气质），全部 AI 出图 |
| **战斗形态** | Phase 0A 单角色横版水墨 ARPG：即时移动、普攻、Q/R 与数字技能；自动/离线战斗由 bot/headless 复用同一 reducer，不保留 3v3 产品模式 |
...
| 不做的功能 | 替代方案 / 设计意图 |
|-----------|-------------------|
| 传统体力系统 | **不做全局体力 / 每日次数 / 登录领取 / 付费恢复**；允许“扫荡战备 / 副本凭证 / 劳损调息”等非日课型节奏资源，详 §2.3 |
| 每日任务 / 登录奖励 / 战令 | 不制造"上线义务"，玩家想玩才玩 |
| 抽卡 | 用**武学领悟系统**自然解锁招式 |
| VIP 等级 | 买断制无 VIP 概念 |
| ~~装备分解~~（2026-06-26 推翻） | ~~装备永久保留，作为收藏品~~ → **装备可出售换银两 / 可分解成强化材料**（用户 2026-06-26 拍板推翻；理由：玩家处理冗余装备的真实需求；出售/分解为良性 sink，非氪金/留存机制。spec：`docs/spec/2026-06-26-equip-sell-decompose-inventory-design.md`）|
...
### 2.3 节奏资源边界（2026-07-08 拍板）

项目仍然禁止传统体力，但允许为经济与高价值内容设置**非日课型节奏资源**。这些资源的设计目标是限制“在线连续刷资源”或“高价值副本无限入场”，不是制造上线义务。

允许的三类资源：

| 资源类型 | 作用范围 | 设计边界 |
|---------|---------|---------|
| **扫荡战备** | 主线已手动通关关卡的一键扫荡收益 | 只限制在线连续扫荡刷银两 / 材料；不得影响首次通关、剧情推进、基础战斗、离线挂机、闭关或桃花岛生产 |
| **副本凭证** | 后期强力副本 / 高价值副本入场 | 可用“剑冢令、洞天符、名帖、残图、航海物资”等世界观资源表达；不得做成自然日“每天一次”日课 |
| **劳损调息** | 高难连续挑战 / 失败后的状态压力 | 可影响短时间重复挑战的技能爆发、开局内力、收益效率或休整选择；不得直接阻止玩家继续普通游玩 |

统一红线：
- **在线 = 离线**：恢复、累积、冷却必须按真实时间推进，开着游戏与关掉游戏等价。
- **不做日课**：不得每日重置、不得登录领取、不得绑定每日任务、不得付费购买或加速恢复。
- **可囤积**：资源上限必须允许玩家跨天积累，避免“今天不打就亏”的焦虑。
- **不拦主线首通**：首次挑战、剧情推进、新关卡证明自己打得过，不消耗扫荡战备。
- **扫荡优先**：当前阶段优先实装“扫荡战备”，用于解决主线已过关卡在线无限扫荡刷钱刷材料；强力副本以后再接“副本凭证”，不复用扫荡战备。
- **首通可读节奏独立于扫荡**（2026-07-09 实装口径）：主线首次挑战可通过 `combat.readable_first_clear` 与 `animation.readable_*` 配置拉长观看节奏，目标是展示普攻、技能、Boss 蓄力 / 破招与结算，不改变“无传统体力”的原则；扫荡与快进仍走独立快进拍长，不受首通可读节奏拖慢。
...
这里的 49 级是会改变境界门槛、装备阶、心法阶和强化上限的**真实境界层**。为了让成长更直观，每个真实层的当前经验条再均分为 10 个展示段，角色档案显示 **修为等级 Lv1～Lv490**。两者关系固定为“49 层 × 10 段”，没有独立的等级经验账。

- `Character.experience` 是唯一可写角色经验；所有来源统一交给 `CharacterAdvancementService`。
- 显示 Lv 只用于反馈，不增加生命、内力、速度或其他属性，也不参与装备/心法门槛与强化上限。
- 经验达到当前层门槛但被心魔拦截时，显示该层第 10 段并保留溢出经验。
- **当前发布边界**见头部「当前状态块」(加章 reconcile 维护,不在此处重复手写)。达到边界后只停止进入下一层，经验仍无上限累加并显示“待开放”；旧档若已超边界，保持原境界、等级与经验，不做降级，只拦后续进层。
- 当前主线规模、首通与全内容参考路线(主线、塔、轻功、群战、心魔、72h 闭关、24h 普通离线与三枚经验丹)实测锚见头部「当前状态块」。当前内容不要求玩家升满发布边界；边界以上境界层及其装备、心法定义继续保留，作为未来副本与玩法的成长空间。
- 武圣·登峰的 `experience_to_next=1,250,000` 只提供终局进度刻度；达到后显示 Lv490“修为巅峰”，不生成第 50 层或 Lv491。
- 旧存档中的独立等级字段仅作 schema 兼容，不再被生产逻辑读取或写入。
...
| 属性 | 影响 |
|------|------|
| **根骨** | 影响血量上限；根骨高于 5 时，每点使新生成的重伤时长缩短 2%，最多缩短 20% |
| **悟性** | 影响心法修炼、招式熟练度成长与武学领悟概率；高于 5 时每点成长 +2%，最多 +20% |
| **身法** | 影响出手速度与闪避；不再影响暴击率 |
| **机缘** | 影响普通奇遇触发率，并解锁少量明确标记的特殊选项；不影响商店价格与掉落倍率 |
...
| 项目 | 惩罚 |
|------|------|
| 内息状态 | 施加 6h 内息紊乱（受 12h 上限约束，不扣永久内力） |
| 原主修心法修为 | -50%（修炼度回退） |
...
| 大招暴击伤害 | 上万，不许进百万（旧 3v3 已退役诊断的历史峰值记录约 21 万，非当前守卫；见下红线分层） |
| 玩家血量 | 5,000 ~ 20,000 |
| Boss 血量 | 上限 60,000（2026-06-14 由上限 50,000 上调；更高须先与人类讨论，任何情况不进 1M；当前最高 stage_15_05=59,500 头寸已用尽） |
| 内力 | 500 ~ 15,000 |
| 真气 | 基础气海 100（心法构筑最终限于 80~140） |
...
> **2026-07-12 真气循环复校补充**：上述心魔机制镜像的 `0.75` 继续缩放装备攻击；05/06 另以配置化总输出 `0.52` 避免永久内力伤害形成硬墙，07 不套总输出折减，以保留限时生存路线压力。
...
> 注：2026-07-12 内力/真气拆分后，内力不再撑高血量；旧曲线改由境界层级基础血量承接，根骨与装备轴不变，仍守 20,000 硬红线。

### 5.7 战斗体验原则（爽感主旋律 · 2026-07-15 用户拍板）

战斗以**参与感 + 即时打击爽感**为产品主旋律。核心交互：**点按选招、立即出手**，不做拖放，也不做“排到下回合再放”的延迟等待。单体招式先点技能进入待发，再点目标立即出手；若敌方只剩唯一合法目标则直接释放。群体招式点击即对全体释放。长按技能方块查看简介；再点同一技能、点击空白或按 ESC 取消待发。
...
| 类型 | 命中范围 | 目标选择 |
|------|---------|---------|
| **single 单体** | 单一目标 | 点技能后点敌方人物指定 / 唯一合法目标直接释放 / AI 选对面 HP 最低者 / 破招锁定蓄力中的敌人 |
| **aoe 群体** | **全体存活敌人** | 无需指定，点击技能即对全体触发 |
...
- aoe：点击技能即对全体触发，**无需指定落点**。
- single：点击技能进入待发，再点击具体敌方人物指定目标；唯一合法目标时直接释放。
...
|------|------|
| **战斗掉落** | 所见即所得（敌人用什么掉什么） |
| **江湖商店** | 固定货架·无刷新（P1 激活，售炼器材料磨剑石/心血结晶为固定标价；守 §5.1 无每日刷新/限购/机缘定价，守 §5.5 银两离线=在线）。原「每日刷新+机缘定价」设计 2026-06-21 拍板废除。**P2 激活经验丹/秘籍用途**（2026-06-21）：经验丹 3 档加角色经验推境界（守升层锁），秘籍解锁对应秘传招（仅掉落不上货架，守 §5.7 江湖偶得绝学的磨砺感）；经验丹小/中上货架可买，大档/秘籍仅掉落。**经验丹采用「ETL 恒定兑换率动态标价」**（2026-06-24 拍板授权，balance T3/T6 校准）：标价 = 祖师当前单层所需经验(ETL) × `price_layer_fraction`，随境界推进上涨以**锁定银两↔经验兑换率恒定**（buyRatio=5），防低境界囤丹套利推境界。此为**进度锚定**而非 §5.1 废除的「机缘定价」——后者按玩家「机缘属性」变价制造留存焦虑，前者兑换率恒定无套利空间，性质不同。仅经验丹动态，材料类仍固定价 |
...
挂机 / 探索时累积各类触发进度。条件满足后按**悟性**计算武学领悟概率，命中时弹出“灵光一现”事件；不另设可积攒的“机缘值”。普通奇遇仍按**机缘**计算概率，少量选项会明确显示机缘门槛，未达标时保持可见但不可选择。
...
**开放式闭关**:玩家选择地图后直接入定,不再预选 1/4/12 小时。闭关会一直持续,直到玩家主动收功;启动或回到前台不自动结算。

**真实时间机制**：使用**系统真实时间**计算，关闭游戏继续计时。

**两段结算**:前 72 小时按所选闭关地图完整结算;超出部分按同境界普通挂机收益线性累积,不设第二上限,两段时间不重叠。

**装备机缘**:前 72 小时内每满 12 小时进行一次独立判定,最多 6 次,不设保底。节点越后,当前阶与越阶装备权重越高;随机种子由存档、闭关会话、开始时刻和节点组成,重启不可刷结果。越境界装备可入包,但未达境界不可装备。

品阶基准取 `max(地图基准阶,开始时可用阶-1)`,权重为:
...
**约束**:闭关期间可查看背包、心法等非战斗页面,但角色**不可出战**;战斗入口引导玩家先前去收功。

**设计理由**:把现实时间编织进游戏,但不要求玩家在固定时点回来。长时间离开仍持续有收益,而地图特色与高阶装备机缘保留收功仪式感。
...
| 装备 | 80 件（覆盖 7 阶） |
| 心法 | 49 本（覆盖 7 阶 + 3 流派） |
| 通用 / 战斗招式（skills.yaml） | 221 招 |
| 奇遇专属武学领悟招式（encounter_skills.yaml） | 40 招 |
| 招式总池（SkillDef） | 261 招（221 + 40） |
| 心法相生组合 | 12 |
| 师徒角色 | 3（祖师 + 大弟子 + 二弟子） |
| 典故文案 | 50-80 段 |
| 主线剧情字数 | 3,000 ~ 7,000 字 |

> **招式计数口径**（2026-08-21 Phase 0A Q/R 过渡战术技 +2）：`261` 是
> `GameRepository.skillDefs` 总池口径，不是 `data/skills.yaml` 单文件口径；
> 其中 `skills.yaml` 221 招，`encounter_skills.yaml` 40 招。Q/R 两招
> `source: special`仅作 Phase 0A 过渡战术槽真实身份，不进心法成长池。
> 外部审查若只数 `skills.yaml` 会误判为 GDD 漂移。
>
...
- 二周目玩家可**跳过引导直接开档**，避免老玩家重复承受基础门槛。**当前状态（2026-07-11）**：已实装。任一现存存档完成首周目后，空槽祖师创建页可选「老江湖开局」；该模式只解除第一章的心法/闭关引导门槛，主线、首通奖励、角色塑形与初始资源仍从头开始。默认仍为「循序入门」，不替玩家自动跳关。
...
| 排行榜 | 本地榜 + `LeaderboardSyncService` 抽象（当前 Noop；云端 Supabase 为未来方向） |
...
> **v1.18 导语更新**：本节原为 Demo 期「不实现、留接口」清单；1.0 内容周期已把多数系统实装（各行内有 ✅ 实装注）。**已实装**：心魔（§12.1）/ 帮派门派（§12.2）/ 江湖恩怨+声望（§12.1/§12.2，P1.2）/ 轻功对决+群战（§12.3）/ 第二条主线 Ch4-6（§12.4）/ 节日 encounter 内容层 / 门派事件（P3.4 比武·危机·任务，`numbers.yaml sect_event` + `lib/features/sect/`，2026-07-02 审查订正：原误列「不启动」）/ **桃花岛养成支柱一期+二期**（saveVer 0.30，藏卷阁 Hub，`lib/features/taohua_island/`，2026-06-27/28 合 main，2026-07-02 审查补记——此前本文档 0 记载）。**已切除 / 不启动**：PVP（不保留入口、service、UI、玩法配置;仅保旧档 schema 兼容）/ 婚姻后代 / 节日活动系统级框架 / MOD / §12.5 长期愿景——动这些前必须先回到本文档讨论。

### 12.1 剧情与世界

- **江湖恩怨系统**：NPC 之间的关系网，杀某人会被其门派追杀。**已实装 ✅**(2026-05-25 P1.2 100% 闭环 · 与声望系统联动 · 详 `docs/handoff/p1_2_jianghu_full.md`)。
- **心魔系统**:高境界突破前需面对心魔关卡,剧情化的内心战斗。**v1.10 1.0 P2.2 Batch 2.1-2.5 全收尾 ✅**(2026-05-23):7 关 `stage_inner_demon_01..07` 拦截 wuSheng 7 层突破；镜像队伍受红线约束并在高层配脆弱窗口。**2026-07-12 失败语义升级（历史）**：不再扣永久内力或写“心魔余毒”，改为施加有上限的「内息紊乱」并保留主修修炼度惩罚；有效战斗每场减轻一次，闭关/离线按同一真实结算时长恢复，历史设计源见 `docs/superpowers/specs/2026-07-12-inner-force-qi-cycle-design.md`。**2026-08-24 G0 决议 + M5 Batch1 生产迁移 ✅（取代上述主修惩罚）**：失败时不再扣主修修炼度，只保留有上限的内息紊乱；退役 `failure_penalty` 配置被显式拒绝，摘要不报告未发生的内力/修炼度/伤势变化，普通 Boss 路径不变。详 `docs/dispatch/phase0a_overhaul/decision_registry.yaml` 的 `INNER-DEMON-CULTIVATION-01` 与本批审计。
...
- **群战 / 守城战**:5v5 或更大规模的特殊关卡。**v1.13 1.0 P3.2 Batch 2.1-2.5 全收尾 ✅**(2026-05-24 worktree `feat/p3_2_mass_battle` · 5 commit · 1268 pass / 0 analyze · ~2h opus xhigh / spec 估 6-7h):5 关 `stage_mass_battle_01..05` 跨 yiLiu(qiMeng/jingTong/dengFeng)+ jueDing(qiMeng/jingTong)2 Tier · wave 2-4 / enemy 5-7「以少胜多」· `MassBattleStrategy` 组合委派 `DefaultGroundStrategy`(tick/stepOne/runToEnd 均已接 wave 推进与 formation/terrain 烘焙 · formation modifier 烘焙**仅 leftTeam** · clamp ≤0.95 防红线破 · `_intermission` HP+IF preserve / actionPoint+cd reset / result 清空)+ `Formation` enum 3 项(yanXing/baGua/fengShi)+ `numbers.yaml mass_battle` 段 50 行 + 平行支线**不接管 wuSheng 突破链**(`isLayerLocked` 无 massBattle 路径)+ unlock_triggers 链 `stage_06_05 → mass_battle_01 → 02 → 03 → 04 → 05` + narrative ~2.2k 字「不躁/不乱/不溃/不让/不忧」五处试炼。**2026-07-08 状态**:旧“UI tick wiring 留 Batch 3.x”已完成；`stage_mass_battle_05` 当前压线通过，后续 AI/装备数值改动需优先回归。详 `docs/handoff/p3_2_mass_battle_closeout_2026-05-24.md`。
- ~~**生死状 PVP**：异步 PVP，挑战其他玩家阵容（基于 Supabase）。~~ **已切除**（2026-06-27）：不保留玩家入口、路由、provider/service、占位 UI 或玩法配置；旧存档相关 schema 仅兼容忽略。

### 12.4 内容与节奏

- **第二条主线**：承接前三章的后续剧情（Ch4～6）。当前发布版玩法数值统一在三流内，原一流～武圣标注只保留为历史创作强度记录；有效境界与掉落以 §8.1 和生产 YAML 为准。
...
| **战斗形态** | 1(历史地面队伍战) | **1**(Phase 0A 单角色 ARPG 核 × 主线/塔/扫荡/远征/断魂庄内容适配) | 同核扩内容 | PVP 已切除；轻功/群战/心魔作为内容与规则适配保留，不再构成独立旧引擎形态 |
...
### 12.4.2 二阶段 G0 决议索引（2026-08-24）

用户按推荐方案批准 G0；细节真相源为 `docs/dispatch/phase0a_overhaul/decision_registry.yaml`。主线可见重打/前台 bot 可选 eligible 空闲角色，无人值守 headless/扫荡固定当前掌门，记录、成长与伤势归实际参与者。连续 `MainlineRun` 整段锁同一参与者，关间可换装并生成版本化新快照，仅当参与者下一关不再可战时中断。随行听剑单关占用，与闭关、远征、断魂庄、疗伤均互斥；只成长主修招式熟练度，比例与每关上限仍为 `TUNING`。心魔失败取消主修修炼度扣减，只保留有上限的内息紊乱。

七心魔 AI 选择“先补 canon / narrative / behavior / test 四列矩阵再逐名冻结”，未冻结前保持现有 AI；塔、轻功、差遣、群战、远征、断魂庄与心魔的渐进解锁选择逐模式形成 current → target 迁移表，未签模式保持现 gate。六类地域锚与 M2 Ch1 山匪纵切冻结，其余逐关生态按章节/塔任务包分批审核并只实现已签批次。断魂庄不开放前台可见 bot，保留完整首通后的 headless 重刷。

Build 双方案、章节回顾/收藏百科一级入口、Boss 技能预兆图标、统一归来事件小结与轻功专属收益继续不重开；失败原因诊断只部分放开事实性失败/返程/伤势展示，建议型诊断与配装建议 UI 继续禁止。20 项 `TUNE-*` 只授权生成候选，未经 YAML、红线、自动模拟、适用的 Mac/Windows Profile 与真人试玩不得标为 frozen 或接入生产。G0 登记是设计授权，不代表代码、数据或 UI 已上线。

生产迁移进度（2026-08-25 最新切片）：G1 生产合同、G2 黑风岭纵切、第一章连续首通、权威持久结算及其中互动奇遇/Boss 招降的 U04 持久 FIFO 已验收；可见真人重打现已按实际参与者贯通生产 Host 与归属结算，但 `U01` 仍因听剑生产接线和 headless/扫荡等全模式一致性未闭环而不得标记完成。U05 四个一级入口均已经独立 READY：“继续江湖”直达当前首推关，“武学与行囊”“宗门”“江湖纪事”收拢原生产路由；资源、设置与退出已归入角落工具区，心魔只保留角色突破入口，本地排行榜已归位九霄塔且保持 Noop 同步。“当前要事”的闭关、伤势与突破摘要现绑定真实 active roster 角色并在点击时复核身份，异常不再回退角色 1。U06 已建立同卡片次级江湖地图入口，九霄塔、轻功试炼、守城试炼、断魂庄、百草岭远征与江湖恩怨已成为六个生产地点，均保持原准入和去向；当前六个地点均已有统一详情首纵切。轻功/守城详情对多根、汇合、环、截断配置 fail closed，守城仍保留原阵型选择和周目流；断魂庄和百草岭现允许存活、空闲且有主修的当前掌门与合格门人从原生产页逐次选人，并在 service 事务内复核身份和占用，原补给/方针、周目、恢复/离线推进、战斗、结算、召回、奖励与返程不变；江湖恩怨详情只读六门派、稀疏声望、七阶与变化来源，仍进入既有声望面板且不猜测当前 NPC 仇敌身份/数量；江湖商店保持主菜单条件入口。U07 已有首个生产当前态摘要：宗门 Hub 顶部只读显示真实掌门及其状态、门人闭关/远征/断魂庄占用、active 远征与庄局进度，异常 fail closed；不包含已拒绝的统一完成报告，也不虚构疗伤/听剑占用。U08 两个生产入口已移除旧全局三席编辑并只读当前掌门、当代门人和真实活动状态，但完整差遣策略仍开放。其他未迁移野外内容、U07 其余开放语义与 U14 仍开放；当前六地点的首轮详情覆盖不等于 U06 完成。U04 不外推到塔/特殊模式全局队列；本切片不晋升 U05/U06/U07/U08 整体状态，M2、M5、M6、M3/M4 及其他里程碑仍开放；断魂庄仍不开放前台 bot 或代选奖励，百草岭仍不新增亲战、前台 bot、多人或新自动化，本次不改奖励、解锁、调优或其他业务规则。
