...
> - 发布上限:绝对境界层 **47**(武圣·圆熟 = 武圣段中章;真相源 `data/numbers.yaml` `progression.release_cap.max_absolute_realm_level`)
> - 主线规模:**20 章 100 关**(真相源 `data/stages.yaml` `stageType: mainline` 计数)
> - 实测锚:100 关首通 **Lv113**(累计 5074 经验);全内容参考路线 **Lv127**(由 `progression_release_budget_test` 维护,实测锚不入自动校验)
...
| 装备 | 80 件（覆盖 7 阶） |
| 心法 | 49 本（覆盖 7 阶 + 3 流派） |
| 通用 / 战斗招式（skills.yaml） | 218 招 |
| 奇遇专属武学领悟招式（encounter_skills.yaml） | 40 招 |
| 招式总池（SkillDef） | 258 招（218 + 40） |
| 心法相生组合 | 12 |
| 师徒角色 | 3（祖师 + 大弟子 + 二弟子） |
| 典故文案 | 50-80 段 |
