...
> **v1.19 变更**(2026-07-09 扫荡战备 + 首通可读节奏收口):① §2.3 的“扫荡战备”进入实装校准口径:只限制已通关主线重复扫荡,首通/普通战斗/离线收益不消耗;② 主线首通可读节奏采用配置化拍长与胜利保留时间(`readable_action_interval_ms` / `readable_victory_min_ms`),配合首通敌方 HP/攻击与自动技能倍率微调,目标是让新关卡从“1-3 秒结束”转为可读的普攻-技能-结算节奏,而不是靠传统体力或无限抬血制造拖沓;③ 记录当前素材缺口:战斗大招与 Boss 蓄力起手 SFX 仍为临时转用素材,不得误判为最终专属资产。
...
- **心魔系统**:高境界突破前需面对心魔关卡,剧情化的内心战斗。**v1.10 1.0 P2.2 Batch 2.1-2.5 全收尾 ✅**(2026-05-23,10 commit `e666e4c → b15d34d` 全 push origin/main):7 关 `stage_inner_demon_01..07` 拦截 wuSheng 7 层突破(qiMeng → ruMen → ... → dengFeng → 飞升前置)+ `InnerDemonService.buildMirrorEnemyTeam` 深拷贝 playerTeam ×(1+10-25%) clamp §5.4 cap(`mirror_caps` HP ≤20k/IF ≤15k/Attack ≤6k=3×§5.4 单件 2000)+ 机制化心魔攻击乘 0.75 + `isLayerLocked` 拦截 hook 接 advancement_service.applyExperience(EXP 留账 §5.1)+ 3 callers wire(seclusion/tower/mainline)+ 失败 = 内力 ×0.85 / 主修修炼度 ×0.9 + 「心魔余毒」debuff 闭关 8h 清（失败惩罚已由 `BattleResolutionService.resolve` 在心魔战败分支调用 `InnerDemonService.applyFailurePenalty` 接入）+ 22 narrative ~3,900 字(Tier wuSheng「湛然/寂照/圆融/化机」+ 7 主题贪/嗔/痴/慢/疑/空/真)+ **UI reactive 三态**(InnerDemonScreen cleared/available/locked + main_menu _MenuButton 入口)+ R1-R5 28 测。2026-07-08 调优后 `_07` 镜像 buff 为 +25%、05/06/07 脆弱窗口承伤 0.16/0.16/0.14，双镜像架构不动。挂账 1.0 P3+:BreakthroughBlocker 集成 character_panel + 7 enemy 立绘。详 `docs/handoff/p2_x_inner_demon_final_closeout_2026-05-23.md`。
