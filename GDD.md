...
>
> **版本**:v1.46
> **v1.46 变更**(2026-08-25 二阶段主菜单角落工具区纵切):资源总览与设置退出一级玩法区域，和既有退出统一置于主菜单右上角工具区；资源、设置、退出继续进入原生产页面、面板和确认流程，四个一级入口与现有门控不变。这只关闭冻结方案的角落工具区缺口；U06 地图迁移、排行榜归位、U05/M6/二阶段仍开放，不改 schema、数值、概率、奖励、解锁、经济或叙事。
