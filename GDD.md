...
> - 发布上限:绝对境界层 **45**(武圣·熟练 = 武圣段首章;真相源 `data/numbers.yaml` `progression.release_cap.max_absolute_realm_level`)
> - 主线规模:**19 章 95 关**(真相源 `data/stages.yaml` `stageType: mainline` 计数)
> - 实测锚:95 关首通 **Lv109**(累计 4569 经验);全内容参考路线 **Lv124**(由 `progression_release_budget_test` 维护,实测锚不入自动校验)
...
- 当前共 **95 个关卡**；Ch1-6（30 关）累计 492 经验、Lv1 首通实测至 Lv44；95 关首通累计 **4569** 经验、实测至 **Lv109**；全内容参考路线（+塔/支线/闭关 72h/离线 24h/三丹）实测 **Lv124**（2026-07-28 Ch19 扩章后重校·纯展示级不影响红线·逐值实测终拍。旧值 75 关/2815/Lv93/Lv112 是 Ch15 口径，Ch16-18 三章连续漏更，本批一并订正）
...
| 装备 | 80 件（覆盖 7 阶） |
| 心法 | 49 本（覆盖 7 阶 + 3 流派） |
| 通用 / 战斗招式（skills.yaml） | 217 招 |
| 奇遇专属武学领悟招式（encounter_skills.yaml） | 40 招 |
| 招式总池（SkillDef） | 257 招（217 + 40） |
| 心法相生组合 | 12 |
| 师徒角色 | 3（祖师 + 大弟子 + 二弟子） |
| 典故文案 | 50-80 段 |
