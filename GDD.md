...
>
> **版本**:v1.45
> **v1.45 变更**(2026-08-25 二阶段“江湖纪事”一级 Hub 纵切):主菜单散列档案入口收拢为一个“江湖纪事”，提供章节卷轴、人物、地点、敌手、装备典故与待处理江湖事六路。章节/人物/敌手/装备复用既有生产 Screen；地点只展示主线已踏足或当前可达关卡，不泄露锁定名称；待处理事项只读现有 settlement journal typed FIFO，并经原 `runStageFlow` 恢复。敌手与装备保留原隐藏门控，异常 fail closed。这只关闭 U05 四个一级入口中的第四项；资源角落工具、排行榜归位、U05/M6/二阶段仍开放，不改 schema、数值、概率、奖励、解锁、经济或叙事。
> **v1.44 变更**(2026-08-25 二阶段“宗门”一级 Hub 纵切):主菜单原先散列的角色面板、闭关修炼、桃花岛、门派事务与江湖远行收拢为一个“宗门”入口，内部复用角色档案、门人调度、闭关、疗伤、远征、桃花生产和门派事务生产 Screen。角色入口绑定 active roster 第一位角色并在身份未决时 fail closed；闭关、远征、桃花与门派门控保持原值，活跃闭关状态仍由主菜单常驻 banner 展示。这只关闭 U05 四个一级入口中的第三项，“江湖纪事”、资源角落工具、U05/M6 或二阶段仍开放；不改业务写入、schema、数值、概率、奖励、解锁阈值或叙事。
>
