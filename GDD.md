...
> **v1.47 变更**(2026-08-25 二阶段心魔入口归位纵切):落实 §11.2，心魔不再出现在主菜单；原 Ch6 门槛锁定/满足时均只由角色突破阻断区提供入口，并继续进入既有 `InnerDemonScreen`。心魔解锁、战斗、失败与奖励语义不变。这只关闭心魔入口归位缺口；U06 地图迁移、排行榜归位、U05/M6/二阶段仍开放，不改 schema、数值、概率、奖励、经济或叙事。
>
