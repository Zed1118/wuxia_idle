...
>
> **版本**:v1.52
> **v1.52 变更**(2026-08-25 二阶段江湖地图百草岭远征第五地点纵切):“百草岭”作为地图第五个生产地点；可见性只读既有 `jianghuJourneyUnlocked` 隐藏门，进行中状态只读 `activeExpeditionProvider`，去向仍为 `ExpeditionOverviewScreen`。宗门 Hub 的远征派遣/管理入口保留。这只关闭百草岭第五地点缺口；其他地点、统一地点详情、U06/U14/M5/M6/二阶段仍开放，不改 schema、YAML、调优、数值、参与者、占用、奖励、经济、解锁或离线推进。
> **v1.51 变更**(2026-08-25 二阶段江湖地图断魂庄第四地点纵切):“断魂庄”从主菜单平铺迁为地图第四个生产地点；可见性只读既有 `jianghuJourneyUnlocked` 隐藏门，进行中状态只读 `activeGauntletProvider`，去向仍为处理新建/恢复的 `GauntletLoadoutScreen`。这只关闭断魂庄第四地点缺口；其他地点、统一地点详情、U06/U14/M5/M6/二阶段仍开放，不改 schema、YAML、调优、数值、参与者、奖励、经济、解锁或叙事，不开放前台 bot 或代选奖励。
>
...
生产迁移进度（2026-08-25 最新切片）：G1 生产合同、G2 黑风岭纵切、第一章连续首通、权威持久结算及其中互动奇遇/Boss 招降的 U04 持久 FIFO 已验收；可见真人重打现已按实际参与者贯通生产 Host 与归属结算，但 `U01` 仍因听剑生产接线和 headless/扫荡等全模式一致性未闭环而不得标记完成。U05 四个一级入口均已经独立 READY：“继续江湖”直达当前首推关，“武学与行囊”“宗门”“江湖纪事”收拢原生产路由；资源、设置与退出已归入角落工具区，心魔只保留角色突破入口。U06 已建立同卡片次级江湖地图入口，九霄塔、轻功试炼、守城试炼、断魂庄、百草岭远征分别作为前五个生产地点，均保持原进度、准入和去向；断魂庄与百草岭继续使用既有隐藏门和 active 状态，分别复用原整备/恢复页与远征总览页；宗门 Hub 远征派遣/管理入口保留。其他地点、统一地点详情与 U14 仍开放。U04 不外推到塔/特殊模式全局队列，U05 的排行榜归位、U06、M2、M6、M3/M4 及其他里程碑均仍开放；断魂庄仍不开放前台 bot 或代选奖励，本次不改业务规则、奖励、解锁或调优值。
