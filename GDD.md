...
>
> **版本**:v1.33
> **v1.33 变更**(2026-08-24 二阶段 G0 推荐方案拍板 · 设计合同收口):冻结主线 replay/连续 run、随行听剑对象与占用、心魔失败惩罚、生态分批审核边界及断魂庄可见 bot；七心魔 AI 与渐进解锁进入有安全默认的矩阵/迁移表流程；五项旧否继续不重开，失败原因仅放开事实展示；20 项调优只授权候选生成，未批准生产值。详 §12.4.2 与 decision registry。
...
- **心魔系统**:高境界突破前需面对心魔关卡,剧情化的内心战斗。**v1.10 1.0 P2.2 Batch 2.1-2.5 全收尾 ✅**(2026-05-23):7 关 `stage_inner_demon_01..07` 拦截 wuSheng 7 层突破；镜像队伍受红线约束并在高层配脆弱窗口。**2026-07-12 失败语义升级**：不再扣永久内力或写“心魔余毒”，改为施加有上限的「内息紊乱」；有效战斗每场减轻一次，闭关/离线按同一真实结算时长恢复。**2026-08-24 G0 设计决议**：取消失败时主修修炼度 10% 扣减，只保留有上限的内息紊乱；当前 `data/numbers.yaml` 与生产服务仍保留旧扣减，必须由 M5 独立迁移和回归后才算上线。详 `docs/dispatch/phase0a_overhaul/decision_registry.yaml` 的 `INNER-DEMON-CULTIVATION-01`。
