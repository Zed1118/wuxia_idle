...
### 12.4.1 二阶段待决索引（不等同于已批准合同）

以下事项沿用《二阶段优化方案》的状态标签，须经 G0 明确改写、保留或暂缓后，才能进入生产合同：主线重打/前台 bot/headless/扫荡参与者，连续 `MainlineRun` 的锁定、换装与伤势中断，随行听剑占用，七类心魔 AI 行为映射，以及心魔失败是否扣主修修炼度（均 `PROPOSED`）；随行听剑成长比例与每关上限（`TUNING`）。稳定事实（如主线 21 章/105 关）不因这些未决项改变。

### 12.4.2 1.0 P2 内容总量表[v1.5 正式拍板,2026-05-22 用户审稿过]
