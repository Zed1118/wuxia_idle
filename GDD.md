...
>
> **版本**:v1.36
> **v1.36 变更**(2026-08-24 二阶段 G1 生产合同收口):G1 的作用域、参与者、输入、失败/奖励等既有合同保持，两个真残项完成生产迁移：玩家与敌方 Phase 0A 技能冷却分别以显式秒值为权威，不再随攻击间隔隐式连动；扫荡开跑前由玩家显式选寻隙/强攻/稳守，三者仍共用同一 command/reducer/headless 核。这不扩大主线前台 bot 准入，也不冻结 Ch1 候选怪潮数值；production catalog、黑风岭纵切和 G2 仍是后续门禁。
