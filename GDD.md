...
>
> **版本**:v1.77
> **v1.77 变更**(2026-09-01 二阶段 M5 轻功/守城自动化矩阵):轻功与守城仍要求真人首通；已通关后，可见重打按全局自动战斗设置选择真人或前台 bot，快速推演走 `direct + playerBot + headless + replay`，原差遣保持 `dispatch + playerBot + headless + offlineResume`。三条通道使用玩家本次选定的 exact participant；守城 headless 在开跑前选择并持久化阵型，轻功拒绝阵型。既有 live Host、Phase 0A headless runner、durable receipt、占用与共享 settlement 仍是唯一 owner。M5 固定工程矩阵由 `34/42` 推至 `36/42`，顶层 M5、M0–M9 仍分别 `0/1 BLOCKED`、`1/10`；真人桌面/Windows 验收继续挂账，不改 schema/saveVersion、YAML、TUNING、数值、技能、奖励、经济、解锁或战斗规则。
...
守城实际参与者结算报告纵切（2026-08-25）已关闭 M6 必要生产子门：地点详情仅展示 eligible 人数，挑战时逐次选择实际参与者，复用既有阵型、真实 `Phase0aMainlineBattleHost` 和共享结算/报告路径；M6 与 Phase 2 顶层状态不变。
