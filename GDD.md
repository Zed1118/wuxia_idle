...
> **v1.42 变更**(2026-08-25 二阶段“继续江湖”直达纵切):主菜单第一入口现按生产 21 章数据动态解析唯一待首推当前关，并经既有战斗守卫直接进入连续首通 `runStageFlow`；状态与目标提示不再只扫描前 15 章。进度未决或全通时回退章节地图，保留重打。这只关闭 U05 四个一级入口中的第一项，不关闭其余 Hub、U05/M6 或二阶段；不改 schema、数值、概率、奖励、解锁或叙事。
>
