...
- **当前发布边界**为绝对境界层 24（一流·熟练）。达到边界后只停止进入第 25 层，经验仍无上限累加并显示“待开放”；旧档若已在 Lv171+，保持原境界、等级与经验，不做降级，只拦后续进层。
- 当前 40 关主线从 Lv1 出发首通实测至 Lv57；当前主线、塔、轻功、群战、心魔、72h 闭关、24h 普通离线与三枚经验丹组成的全内容参考路线实测至 Lv91。当前内容不要求玩家升满发布边界；二流·精通及以上境界层及其装备、心法定义继续保留，作为未来副本与玩法的成长空间。
...
| 装备 | 80 件（覆盖 7 阶） |
| 心法 | 49 本（覆盖 7 阶 + 3 流派） |
| 通用 / 战斗招式（skills.yaml） | 213 招 |
| 奇遇专属武学领悟招式（encounter_skills.yaml） | 40 招 |
| 招式总池（SkillDef） | 253 招（213 + 40） |
| 心法相生组合 | 12 |
| 师徒角色 | 3（祖师 + 大弟子 + 二弟子） |
| 典故文案 | 50-80 段 |
