...
> **v1.58 变更**(2026-08-25 二阶段轻功统一地点详情纵切):轻功试炼地点先展示连续生产进度、下一关推荐境界、地形、敌方生态、掉落/修为、真实当前掌门、亲战方式与预计占用，再经原战斗门禁进入关卡列表。非法解锁图、身份或 provider 异常 fail closed 且无 CTA，五路全通保留重打。该纵切不新增角色选择、派遣、自动化、持久占用、渐进解锁或 U14 参与规则，只关闭轻功地点详情首缺口；其他地点详情、U06/U14/M5/M6/二阶段仍开放，不改 schema、YAML、调优、奖励、经济或战斗。
>
