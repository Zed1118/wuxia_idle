...
> **v1.34 变更**(2026-08-24 二阶段 M5 Batch1 · 心魔失败语义生产迁移):G0 `INNER-DEMON-CULTIVATION-01` 已完成代码、配置、UI 与回归收口。心魔失败不扣主修修炼度，不回退境界/永久内力/装备且不新增物理伤势，只施加有上限的内息紊乱；战败摘要只陈述角色与内息紊乱，入场前既有伤势不得冒充本次失败后果。退役 `failure_penalty` 配置 fail-closed，普通 Boss 旧失败语义保持。
> **v1.33 变更**(2026-08-24 二阶段 G0+G7 · 设计合同与证据口径收口，0 改玩法数值):冻结 G0 推荐方案产品合同；追加将 13.5–21 万明确标为已随旧 3v3 删除的历史诊断记录，不再冒充当前守卫。当前可重跑证据拆为满 build calculator 探针与 2310 次 Ch1 起手画像 Phase 0A 真实 reducer 路径，并登记“Phase 0A 满 build 真实路径极值探针”后续缺口。G0 细节见 §12.4.2 与 decision registry。G7 是同日证据订正，按本批 archive 冻结边界并入 v1.33，不新开版本号。
...
生产迁移进度（M6 Batch1）：主线叙事去阻塞与章节卷轴搬迁、断魂庄完整首通后的 headless 重刷准入已经实现并验收；`U01/U04/U05`、完整连续五关、全模式自动化、G2 黑风岭纵切以及 M3/M4 仍未关闭。断魂庄准入不开放前台 bot、不代选奖励，也不改变数值、解锁或存档格式。
