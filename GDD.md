...
>
> **版本**:v1.38
> **v1.38 变更**(2026-08-24 二阶段第一章连续首通生产切片):第一周目当前可挑战关已接入同一段非递归 `MainlineRun`：第一章真实五关按关结算后由玩家选择继续或返回，参与者锁定为 run 起点掌门，关间只为同一角色重新生成版本化战斗快照；参与者不再可战或快照/后继关系异常时 fail closed。该实现不增加强制关间剧情、不改变重打/特殊模式，也不冻结听剑成长比例/cap 或任何 `TUNE-*`。持久崩溃恢复与重复结算键、听剑生产发放、replay/manual/auto/headless/扫荡一致性、U04/U05、M2/M6 仍开放，不能由本切片外推为二阶段完成。
