...
>
> **版本**:v1.60
> **v1.60 变更**(2026-08-25 二阶段断魂庄统一地点详情纵切):断魂庄地点先展示生产庄局进度、基础推荐境界、三关敌方生态、首通/失败奖励、断魂帖/补给、可用候选人、亲战方式与会话占用；进行中庄局显示真实参与者并保留原恢复入口。隐藏门、配置/引用、参与者或 provider 异常 fail closed 且无 CTA；CTA 仍进入原 `GauntletLoadoutScreen`，选人、补给、周目、恢复、战斗、结算和奖励选择不变。该纵切不新增差遣、前台 bot、自动化、参与者 policy、渐进解锁或 U14 规则，只关闭断魂庄地点详情首缺口；其他地点详情、U06/U14/M5/M6/二阶段仍开放，不改 schema、YAML、调优、奖励、经济或战斗。
> **v1.59 变更**(2026-08-25 二阶段守城统一地点详情纵切):守城试炼地点先展示连续生产进度、下一关推荐境界、阵型、波次/敌数、敌方生态、掉落/修为、真实当前掌门、亲战方式与预计占用，再经原战斗门禁进入关卡列表。非法解锁图、关卡配置、身份或 provider 异常 fail closed 且无 CTA，五关全通保留重打；原阵型选择、周目和战斗流不变。该纵切不新增角色选择、派遣、自动化、持久占用、渐进解锁或 U14 参与规则，只关闭守城地点详情首缺口；其他地点详情、U06/U14/M5/M6/二阶段仍开放，不改 schema、YAML、调优、奖励、经济或战斗。
>
...
生产迁移进度（2026-08-25 最新切片）：G1 生产合同、G2 黑风岭纵切、第一章连续首通、权威持久结算及其中互动奇遇/Boss 招降的 U04 持久 FIFO 已验收；可见真人重打现已按实际参与者贯通生产 Host 与归属结算，但 `U01` 仍因听剑生产接线和 headless/扫荡等全模式一致性未闭环而不得标记完成。U05 四个一级入口均已经独立 READY：“继续江湖”直达当前首推关，“武学与行囊”“宗门”“江湖纪事”收拢原生产路由；资源、设置与退出已归入角落工具区，心魔只保留角色突破入口，本地排行榜已归位九霄塔且保持 Noop 同步。“当前要事”的闭关、伤势与突破摘要现绑定真实 active roster 角色并在点击时复核身份，异常不再回退角色 1。U06 已建立同卡片次级江湖地图入口，九霄塔、轻功试炼、守城试炼、断魂庄、百草岭远征与江湖恩怨已成为六个生产地点，均保持原准入和去向；九霄塔、轻功、守城与断魂庄已有统一地点详情首纵切，轻功/守城详情对多根、汇合、环、截断配置 fail closed，守城仍保留原阵型选择和周目流，断魂庄详情仍保留原选人、补给、周目、恢复、战斗与结算流；声望地点复用第一章末关门槛与既有声望面板，江湖商店保持主菜单条件入口。U07 已有首个生产当前态摘要：宗门 Hub 顶部只读显示真实掌门及其状态、门人闭关/远征/断魂庄占用、active 远征与庄局进度，异常 fail closed；不包含已拒绝的统一完成报告，也不虚构疗伤/听剑占用。其他地点详情、其他未迁移野外内容、U07 其余开放语义与 U14 仍开放。U04 不外推到塔/特殊模式全局队列；本切片不晋升 U05/U06/U07 整体状态，M2、M6、M3/M4 及其他里程碑仍开放；断魂庄仍不开放前台 bot 或代选奖励，本次不改业务写入、奖励、解锁、调优或其他业务规则。
