...
>
> **版本**:v1.37
> **v1.37 变更**(2026-08-24 二阶段 G2 后最小整合):黑风岭 `stage_01_03` 已完成 G2 八项正式生产验收，并作为后续二阶段唯一整合基线；其结论只覆盖黑风岭纵切，不外推为第一章连续五关、U01/U04/U05、M2/M5/M6 或整个二阶段完成。远征死亡角色入口与散功活动占用/stale tuple 两个权威事务缺口已分别 fail closed；不改变奖励、解锁、存档 schema、玩法数值或候选调优状态。20 项 `TUNE-*`、听剑比例/cap、七心魔 AI、渐进解锁及其余章节/塔生态分配仍保持原状态。
> **v1.36 变更**(2026-08-24 二阶段 G1 生产合同收口，状态快照已由 v1.37 取代):G1 的作用域、参与者、输入、失败/奖励等既有合同保持，两个真残项完成生产迁移：玩家与敌方 Phase 0A 技能冷却分别以显式秒值为权威，不再随攻击间隔隐式连动；扫荡开跑前由玩家显式选寻隙/强攻/稳守，三者仍共用同一 command/reducer/headless 核。这不扩大主线前台 bot 准入，也不冻结 Ch1 候选怪潮数值；当时 production catalog、黑风岭纵切和 G2 仍是后续门禁，现以 v1.37 为准。
> **v1.35 变更**(2026-08-24 二阶段 M6 Batch1 · 已冻结叙事/断魂庄自动化合同生产迁移):105 个主线关不再自动弹 opening/victory/defeat 阅读器，252 个既有叙事 ID 保留并经 manifest 进入现有章节卷轴可选阅读；Boss 战败事实损失与特殊模式叙事保持。断魂庄继续禁止前台可见 bot 与 headless 首通，只开放 exact gauntlet 完整首通后的 headless 重刷，并在奖励选择态停止。本批不关闭完整连续主线或 G2。
>
...
生产迁移进度（G1 收口）：M6 Batch1 叙事去阻塞/断魂庄自动准入与 G1 全部 21 项生产合同已验收；其中技能冷却秒权威与三战术扫荡消费链已闭环。`U01/U04/U05`、完整连续五关、全模式自动化、Ch1 production catalog、G2 黑风岭纵切以及 M3/M4 仍未关闭。断魂庄准入不开放前台 bot、不代选奖励，也不改变数值、解锁或存档格式。
