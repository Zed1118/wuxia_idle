...
| 装备 | 80 件（覆盖 7 阶） |
| 心法 | 49 本（覆盖 7 阶 + 3 流派） |
| 通用 / 战斗招式（skills.yaml） | 210 招 |
| 奇遇专属武学领悟招式（encounter_skills.yaml） | 40 招 |
| 招式总池（SkillDef） | 250 招（210 + 40） |
| 心法相生组合 | 12 |
| 师徒角色 | 3（祖师 + 大弟子 + 二弟子） |
| 典故文案 | 50-80 段 |
| 主线剧情字数 | 3,000 ~ 7,000 字 |

> **招式计数口径**（2026-07-10 drift 订正·2026-07-20 Ch9 +1）：`249` 是
> `GameRepository.skillDefs` 总池口径，不是 `data/skills.yaml` 单文件口径；
> 其中 `skills.yaml` 210 招（含 Ch8 灰袖回风 + Ch9 沉沙一诀 + Ch10 止水诀），`encounter_skills.yaml` 40 招。
