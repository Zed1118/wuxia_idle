...
| **战斗形态** | 半横版队伍战 3v3，自动战斗 + 手动放技能（⚠ 2026-08-19 漂移指针：终态已拍板 = Phase 0A 单角色 ARPG 替换（§7.4 ADR 路线 C），本行口径同步随 ADR 执行批，详 `docs/audit/living_doc_drift_2026-08-18.md`） |
...
> ⚠ **漂移指针（2026-08-19）**：战斗终态用户 2026-08-18 拍板 = Phase 0A 单角色 ARPG 替换旧 3v3（不两模式并存）；§7.4 ADR 2026-08-19 拍**路线 C 终态替换·前置排程**（硬前提 = 6 人 Gate + Windows 实机 + Phase 1 纵切 + 共享层/headless 内核安置先行）。本节及 §1.1/§8/§12.3 相关措辞的改写随 ADR 执行批落，禁单独改；拆除范围事实底座 `docs/audit/legacy_3v3_removal_scope_2026-08-18.md`，drift 清单 `docs/audit/living_doc_drift_2026-08-18.md`。
...
| 装备 | 80 件（覆盖 7 阶） |
| 心法 | 49 本（覆盖 7 阶 + 3 流派） |
| 通用 / 战斗招式（skills.yaml） | 219 招 |
| 奇遇专属武学领悟招式（encounter_skills.yaml） | 40 招 |
| 招式总池（SkillDef） | 259 招（219 + 40） |
| 心法相生组合 | 12 |
| 师徒角色 | 3（祖师 + 大弟子 + 二弟子） |
| 典故文案 | 50-80 段 |
| 主线剧情字数 | 3,000 ~ 7,000 字 |

> **招式计数口径**（2026-07-10 drift 订正·2026-07-22 Ch13 +1）：`253` 是
> `GameRepository.skillDefs` 总池口径，不是 `data/skills.yaml` 单文件口径；
> 其中 `skills.yaml` 213 招（含 Ch8 灰袖回风 + Ch9 沉沙一诀 + Ch10 止水诀 + Ch11 鎏金诀 + Ch12 绵里藏针 + Ch13 一览众山），`encounter_skills.yaml` 40 招。
