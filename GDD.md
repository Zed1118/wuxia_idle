...
> **v1.49 变更**(2026-08-25 二阶段江湖地图轻功第二地点纵切):“轻功试炼”从主菜单平铺迁为地图第二个生产地点；锁定与五关进度只读 `MainlineProgress`、`LightFootService` 和生产配置，数据未决或链异常时 fail closed，原战斗守卫与 `LightFootScreen` 去向不变。这只关闭轻功第二地点缺口；群战等其他地点、统一地点详情、U06/U14/M5/M6/二阶段仍开放，不改 schema、YAML、数值、概率、奖励、经济、解锁或叙事。
>
