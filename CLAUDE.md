...
> **2026-06-14 红线语义收口（用户拍板分两层）**：原「普通伤害 ≤8000 不得突破」与现实分裂——满强化神物极值 build 实战普攻实测约 5.8 万（暴击 8.7 万，4-7× 越 8000），且 `test/data/p1a_redline_test.dart` 早已自承「≤8000 是设计层指南，极值已越界」。故红线分两层：**配置基础表值=硬约束**（schema 拦截）；**极值满 build 实战可见值=软可读区间**（不进百万膨胀）。
...
| 项目 | 区间 |
|---|---|
| 普通伤害 | 典型 build 设计目标 8,000；满强化神物极值 build 实测普攻 ~5.8 万（暴击 ~8.7 万），**不进十万**，绝对天花板**不进百万** |
| 大招暴击 | 几万，不许进十万 |

**理由**：玩家一眼能读懂——核心是**不进百万级膨胀**，不是钉死每个极值 build ≤8000。配置基础表值（装备攻击/血量/内力/招式倍率）yaml 写完 schema 校验拦下越界；实战可见值由「满 build 普攻不进十万」软红线测兜底（`test/balance/full_build_damage_redline_test.dart`）。装备派生有效攻击（强化×共鸣×开锋连乘）远超 2000 是**有意终局爽感**，不是越界。
