...
| 10 | 师承遗物规则层(4 子项)| v1.5 决议(2026-05-16):① 传递时机:武圣飞升时自动传(GDD §7.1 原意);② 多徒弟归属:玩家进选件界面逐件分配;③ 累代叠加:只取当代不叠加(数值不爆炸 + UI 可显传承链路但 buff 不叠);④ 同部位冲突:自动卸下原装入背包 + 新遗物入槽。已落 `numbers.yaml inheritance.heritage_items` 加 4 规则字段(`transfer_trigger=ascend_to_wusheng` / `multi_disciple_allocation=player_pick` / `stack_across_generations=false` / `conflict_slot_resolution=auto_swap`)。**v1.14 P2.3 已实装 ✅**(2026-05-24,Batch 3.1-3.3 4 commit):① + ② 在 `AscendService.performAscend`(`lib/features/ascension/application/ascend_service.dart`)真消费 — `transferTrigger` 锚定唯一 trigger(LineagePanel 飞升按钮入口)+ `multiDiscipleAllocation=player_pick` 真消费 AscensionScreen DropdownButton 逐件分配 UI;③ + ④ Demo 仅一代飞升不验证(`stackAcrossGenerations=false` enforce + `conflictSlotResolution=auto_swap` 大弟子飞升前空槽 YAGNI · P5+ 多代师徒升级实装)。详 `docs/handoff/p2_3_ascension_closeout_2026-05-24.md`。 |
