...
> **版本:v1.84**
>
> v1.84 变更摘要(2026-08-25 二阶段整改整合 · 0 改玩法数值):一个权威 Gate 作为唯一主 WIP，允许其必要子门在同一受控链串行收口；分开孤立集成债与 main 发布债，固化用户合并决策队列、四项效率读数、无人值守保险丝和一页核心整改报告。Gate 权重仍待用户批准，未冻结前不报权威百分比。
>
...
- 每完成一个有意义、可独立验证的恢复点再 commit，并同步更新计划文件；不为固定时钟或每个文件制造保险丝提交。硬停止时如必须用提交保证可恢复，使用明确的 `[WIP]` / `[PARKED]` 标记，不计为 READY 或进度。
...
### 8.4 二阶段结果驱动门禁（2026-08-25 整改整合）

**Gate 层级与主 WIP**:
- 权威 Gate 只来自用户批准的二阶段方案、GDD 完成定义和本文件；子任务、页面、测试或 READY 不得自行升格为 Gate。M0–M9 粗粒度分母/权重待用户批准；批准前只报可核验的 `x/y Gate` 或状态变化，不报权威百分比。批准后的分母固化在本操作真相源，不让 `docs/dispatch/` 或单次报告变成第二套权威。
- 同时只推进 1 个权威 Gate。一个 Gate 需多个必要子门时，可在同一受控候选链内串行继续；前一子门必须已被该链吸收、通过风险匹配回归、记录真实状态并保持 clean。子门 READY 不晋升上级 Gate，也不得转去启动无关主 WIP。

**两类债务与合并授权**:
- `孤立集成债` = READY 但未进入当前受控候选链的子门；新增债务上限为 2，超限时禁止新切片，只能集成、回归、修复、验收或报阻塞。2026-08-25 前的历史 READY 分支先做归类/处置盘点，不伪报为 0，也不与新增债务混算。
- `main 发布债` = 已关闭权威 Gate、通过候选链最终回归，但尚未获用户授权进入 `main` 的候选。每关闭 1 个 Gate 即进入待批队列；收工时最多提交 3 个是非题。累计 2–3 个已关闭 Gate 仍未决策时，暂停下一权威 Gate 并请求受控合并决策。每次进 `main` 仍须用户单独明确授权。

**测量、无人值守与文档**:
- 每个可比 Gate 的效率读数固定为四项：Gate 状态变化、结果合同到验收的耗时、一种事先约定的主成本单位、集成返工次数。生产路径、风险匹配验证、受控集成和 clean 是必备验收证据，不算可选指标。
- 无人值守只接高确定性、范围锁定、不需新 schema/saveVersion/迁移决策的任务，必须预设硬停止时刻和可恢复点。可恢复点不强制每 30 分钟造 commit；硬停止时需提交则标 `[WIP]` / `[PARKED]`。
- 复盘、整改和审计文档的问题、根因、措施和验证标准默认压缩到约 60 行核心结论，证据复现、历史和参考放附录或链接；高风险证据不因页数限制删减。`PROGRESS.md` 首屏只保留分母/当前 Gate、两类债务、Top-3 blocker 和待批决策，代码/提交/测试量下压为证据。
