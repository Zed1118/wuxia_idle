...
买断制、写实武侠挂机游戏，**发布目标 Windows**（开发与验收在 macOS）。Flutter Desktop，3v3 自动战斗 + 离线挂机（⚠ 2026-08-19 漂移指针：战斗终态已拍板 = Phase 0A 单角色 ARPG 替换，§7.4 ADR 路线 C，口径同步随 ADR 执行批）。Demo 里程碑已达成（§8.4 14/14），当前处于 **1.0 长线打磨期**（质量优先，不设上线时间压力，见 §7）。
