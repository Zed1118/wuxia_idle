...
步号:
  ...
  7  C·一#18 死链扫描器定位(打印 P10 状态 + 摘录 README)
  8  C·P12   远端 4 分支清理(现算独有 commit 数)
...
step2() {
  ...
  echo "╚══════════════════════════════════════════════════════════════╝"
  echo "编号说明:BACKLOG 一区无 #19 条目;本项实为 P11 登记的「视觉档位化」,"
...
step6() {
  ...
  echo "╚══════════════════════════════════════════════════════════════╝"
  echo "要定什么:30 关主线在高熟练度态的难度曲线是否需微调。"
  echo "选项:(a) 维持现状(mean +8.3pt 全过)/(b) 微调具体关卡"
  echo
  echo "玩哪段:30 关主线,分别用 floor(零熟练)/ceiling(满熟练)态玩,"
  echo "重点看 01_05 floor(变易)、05_05 ceiling(fresh 0%→满熟练 76%)。"
  echo
  launch_game "高熟练度试玩"
  echo "已有证据:"
  echo "  波B 全表 sweep(docs/spec/playability_phase2_backlog.md:58):"
  echo "    30 mainline × floor/ceiling × uses{0,800} × 25 seed,mean +8.3pt 全过"
  echo "  常驻测:test/tools/balance_simulator_test.dart:146(容噪 10pt 单调断言)"
  echo
  echo "拍完之后:若选 (b) 改对应关卡数值(stages.yaml/numbers.yaml),守红线与测。"
  echo
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
list_steps() {
  ...
  echo "  7  C·一#18 死链扫描器定位(3min)"
  echo "  8  C·P12   远端 4 分支清理(3min)"
  echo
  echo "用法:"
