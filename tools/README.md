...
| `doc_link_scan.py` | docs/ 内部引用死链扫描器:跑 `python3 tools/doc_link_scan.py` 出「存活/死链/ignored」分类底账(`--json` 供 diff,`--rows`/`--ignored` 看明细;只用标准库+`git ls-files`/`git check-ignore`,任何 worktree 结果一致;详 `docs/dispatch/reports/2026-08-07_L1D_link_scanner.md`)。**当前定位=可试用,非终审事实源**:`tools/test_doc_link_scan.py` 的 10 类固定样例只覆盖解析层(采集→清洗→跳过→分类)且 mock 了 `git_ls_files`/`git_check_ignore`,**未做真实语料标注,全仓 precision/recall 未测**;引用其输出时须标「未经标注验证」。补验路径见滚动池 P6 |
