...
Vector2 _scriptedHero(double t) => Vector2(
  640 + 140 * math.sin(t * 0.5),
  500 + 60 * math.sin(t * 0.23),
);
...
void main() {
  ...
  test('attack telegraphs stay bounded and strikes are followed by retreat', () {
    ...
    while (t < 40) {
      ...
      maxConcurrentTelegraphs =
          maxConcurrentTelegraphs > telegraphing
              ? maxConcurrentTelegraphs
              : telegraphing;
      observedRetreat =
          observedRetreat ||
          sim.enemies.any((enemy) => enemy.state == DraftEnemyState.retreating);
      t += dt;
    }
    expect(
      maxConcurrentTelegraphs,
  ...
  test('same seed and hero script replay identical trajectories', () {
    ...
    for (var index = 0; index < first.strikes.length; index++) {
      ...
      expect(
        first.strikes[index].position,
        second.strikes[index].position,
      );
    }
  ...
  test('slow field reduces closing speed without changing target selection', () {
