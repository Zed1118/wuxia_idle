...
W15 三销账(#35/#36/LoreLoader)基础上,本会话再推 5 大块:**EquipmentDetailScreen 显化 lore**(opus)+ **Codex round3 dialog 视觉验收 6/6 PASS** + **DeepSeek 34 招 narrativeInsightId 映射 22 交付 13 留空** + **Codex 装备详情屏 6/7 PASS 1 WARN** + **#37 23 orphan events 部分挂回 6 条**。WARN 触发像样货 lore 段数审计发现 W15 #35 实际 70 段不是 closeout 自报的 75 段,挂账 #38。tag `v0.5.2-w15` 已 push,627/627,analyze 0 issues。
...
7 张截图 6/7 PASS + 1 WARN + 0 FAIL。三层反馈:信息卡 chip+三围层级清楚 / tier 颜色映射明显 / 段间分隔稳定 / Navigator 过渡自然 / EnhanceDialog initialTab 分流正确 / scroll 无卡顿。

**1 WARN** `weapon_xiangyang_gang_dao` 只见 1 段:Mac 端 `grep -c text:` 核实**不是 UI bug,是 DeepSeek 文案 1 段交付**。
...
神物   5 × 3 = 15
────────────────────
实测   70 段(closeout 自报 75 段,自数错)
```

GDD §6.6 [50-80] 区间内仍合格,70 段 OK,但**像样货 5 件全只 1 段**是 W15 #35 派单 §4.2 体例"像样货~利器 2 段"的偏差。DeepSeek 漏配。**挂账 #38** 留 DeepSeek polish 20-30min 补 5 段。
...
### 4.4 UI WARN 反推数据 bug

Codex 装备详情屏 04_xiangyang_gang_dao WARN(派单预期 2 段,实际 1 段),Mac 端不是先怀疑 UI bug,而是 grep 数据 → 发现 DeepSeek 文案漏配。**视觉验收 WARN 优先核数据 / yaml,UI bug 排在第二位**(UI 已有 widget test 覆盖)。
...
| 候选 | 推荐档位 | 工作量 | 涉及端 | 阻塞? |
|---|---|---|---|---|
| **#38 像样货 5 件 lore 补第 2 段** | DeepSeek 派单 | Mac 0 / DeepSeek 30min | DeepSeek | 无,可立刻派 |
| **encounter_skills 35 招 description 补文案** | DeepSeek 派单 | Mac 0 / DeepSeek 1-2h | DeepSeek | 无,与 #38 可同批 |
| **memory 更新**(schtasks fallback / 红线测试演进 / closeout 数字 grep) | 零代码 | 5-10min | Mac | 无 |
...
### 5.5 DeepSeek polish 派单提示词草案(可直接发,合并 3 项)

```
项目:挂机武侠 (F:\Projects\wuxia_idle)

git pull --rebase --autostash 拉最新代码(应到 HEAD 5a85479 或更新),
读 docs/handoff/week15_full_closeout_2026-05-15.md §5.5 与本提示词,
完成 3 项 polish:

任务 A · 像样货 5 件 lore 补第 2 段(挂账 #38)
- 文件:data/lore/weapon_xiangyang_gang_dao.yaml / weapon_xiangyang_chang_jian.yaml
  / weapon_xiangyang_jiu_jie_bian.yaml / armor_xiangyang_pi_jia.yaml
  / accessory_xiangyang_yin_jie.yaml
- 每件在现有 default_lore 段后追加第 2 段(对齐 W15 #35 派单 §4.2 体例:
  「像样货~利器 2 段」),文学体例对齐好家伙 2 段、利器 2 段
- 不改第 1 段、不改 id / name 等元数据
- 不在文案里写数值(GDD §5.6 红线)

任务 B · encounter_skills.yaml 35 招 description 补文案
- 文件:data/encounter_skills.yaml
- 35 招 description 全部 TODO_NARRATIVE,需要补成 1-2 句武学描述
- 体例对齐 skills.yaml 现有招式 description
- 不改 id / name / type / 数值字段 / narrativeInsightId 映射
- 已有 narrativeInsightId 映射的 22 招可参考对应 insight 的 description
  保持主题统一

任务 C · "翳"字 polish(可选,非强制)
- 文件:data/events/xiao_zhen_wen_yi.yaml
- title「小镇问翳」中"翳"字过于生僻(Codex round3 OCR 误读为"翁")
- 可选改更常见字(保持音和意),或保留不动(文学性强)
- 你决定,改不改都行

校验:
- yaml 解析(可选 python -c "import yaml; yaml.safe_load(...)")
- 不动其他文件

完成动作:
- 3 项可一个 commit 或分 commit(你判断)
- closeout 文件:docs/handoff/deepseek_w15_polish_closeout_2026-05-15.md
- push 即结束,不联系派单方
...
- **#38 像样货 5 件 lore 缺第 2 段**(下波 DeepSeek polish)
- **Pen-only T64 test fail**(`.dart_tool/build` cache stale 推测)
- **encounter_skills 35 招 description**(下波 DeepSeek polish 合并 #38)
