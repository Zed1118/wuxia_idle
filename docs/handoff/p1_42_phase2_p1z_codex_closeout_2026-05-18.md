...
DeepSeek combat_advanced.md 8/8 满收口后,Mac 端 opus 同会话延续 P2:11 现成 lore md 全入 CodexIndex(2026-05-10 W18-A3 已落),机制百科 8→19 条。spec `p1_42_phase2_p1z_p2_spec.md` + reality check `p1_42_phase2_p1z_p2_reality_check_2026-05-18.md`。

### 10.1 产出

HEAD `bf286dd`,P2 主 commit `8f85fd4`(10 文件 +274/-56);test fix `d4009ba`(P1.z 主 spec 漏写"档 8 交付后 fix"步骤,事后补);文档沉淀 `f1d982a`(+484 行)。1086 pass + 1 skip + 0 issues。

### 10.2 改动

- `codex_category.dart`:加 `lore` 值,`step` getter → `int?`,新增 `isMechanic` / `isLore`
- `codex_entry.dart` / `codex_index.dart`:`step` 全改 `int?`;entries 8→19(8 档 + 4 A 组 + 7 B 组 lore)
- `game_repository.dart`:`_enforceCodexRedLines` step 唯一性废除(A 组同档共存),保留机制 step ∈ [1,8] / lore step==null / 字数 200-550 / 段非空
- `codex_providers.dart`:`unlockedCodexCount` 改 `step.clamp(0,8)`(已解锁机制档数)
- `codex_tab.dart`:分两段渲染,lore 永远 unlocked,chip 分母固定 8;`_LoreSectionHeader` inline
- 测试:codex_tab_test 重写 4 旧 + 加 3 P2 新(viewport 800x3000);codex_loader_test +1 lore graceful;codex_entry_test +6 P2 case

### 10.3 spec vs 实测

opus xhigh ~55min vs spec sonnet 1.5-2.25h,**~2× 加速**。memory `feedback_opus_xhigh_interactive_duration` 第 8 次锚点。

### 10.4 vs spec 2 项调整(教训)

1. **红线 step 唯一性废除**:spec 写"≤ 2/档",未考虑 enhancement 档下 A 组 2 条 + P1.z 首批 1 条 = 3 条;Phase 0 reality check 漏算 A 组挂同档分布
2. **chip 分子语义改"档数"**:spec 写"条目数"max 12 > 分母 8,玩家困惑;改 `step.clamp(0,8)` 对齐 GDD §10.1 8 档叙事

### 10.5 19 条最终归属

| Category | 条目 | 数量 |
|---|---|---|
| combat(1) | realm, equipment_tiers | 2 |
| enhancement(2) | resonance, strengthening, weapon_forging | 3 |
| techniques(3) | techniques_and_styles, lost_techniques | 2 |
| schoolCounter(4) | three_styles_detail | 1 |
| seclusion(5) | retreat | 1 |
| lineage(6) | master_disciple | 1 |
| encounter(7) | encounter_system | 1 |
| advanced(8) | combat_advanced | 1 |
| lore(null) | hidden_weapons, battle_taboos, jianghu_medicine, jianghu_rules, jianghu_ranks, major_sects, famous_battles | 7 |
| **合计** | | **19** |

### 10.6 下波候选

候选 1+2 ✅ 已收口(DeepSeek combat_advanced + P2 扩段);剩 ③ §10 P1.x 5.2 扩段(DeepSeek 30-60min)/ ④ #44 延续典故抽 yaml(双端 4-7h)/ ⑤ 美术 PoC(用户主导 6-10h)/ ⑥ 挂账冲刺(sonnet 3-5h)。

**§10 闭环 + P2 扩段双里程碑达成**:三方式 100%(P1.x/y/z)+ 机制百科 8→19。P1 #42 内核全收口。
