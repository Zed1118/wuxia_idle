...
Codex R2 验收环境:macOS app,窗口 1280x720。先按派单从 hub 进入 `battle_drag_live`;为避开 hub push 动画抢占首帧,补充临时使用 `VISUAL_ROUTE=battle_drag_live` 直达包截交互帧。验收结束后已重跑 `./tool/build_acceptance.sh`,恢复 hub 验收包(commit `271eb61a`)。

8. 无法触发 `drag_live_08_guide_line.png` - 多轮 fresh launch 后用鼠标合成长按主控 ready single 技并拖动,未稳定捕捉到从按钮锚点到指针的拖招引导线;截图主要为自动战斗推进帧。
9. 无法触发 `drag_live_09_enemy_hover_highlight.png` - 合成拖动到存活敌头像区域时未观察到绛红 hovered 光晕;可见伤害/红色受击区来自自动战斗帧,不能作为拖拽命中态证据。
10. 无法确认 `drag_live_10_single_target_hit.png` - 画面可见主控 `崩山式` 结算,但未伴随 8/9 的引导线与 hover,且目标与拖拽下发因果不可确认;按约定不记代码 FAIL。
11. 无法触发 `drag_live_11_charging_glow.png` - 未观察到拖招下发后我方出招者头像持续存在的流派色蓄势光晕;截图中主要为自动结算/受击后的队伍状态。
12. 无法确认 `drag_live_12_rush_to_actor.png` - 未能把战斗加速到拖招角色出手的体感与合成拖拽建立因果;战斗仍按自动节奏快速推进。
13. PASS `drag_live_13_aoe_tap.png` - 点一下主控大招 `万钧裂空` 后立即直发,出现大招题字/群体连线/多目标伤害,按钮进入 `冷却5`,不需要拖到头像。
14. 无法确认 `drag_live_14_drag_to_empty_noop.png` - 合成拖到空白区域松手时未取得稳定拖招态;后续仍有自动战斗行动,无法证明 no-op/不消耗与空白拖放的关系。

手感观察(Codex best-effort):`battle_drag_live` 开屏提示条正确,无需翻 toggle,敌人确为高血耐久配置,比 R1 的普通 Ch1 关卡更适合作为真玩入口。Codex 鼠标合成长按拖仍不稳定:可点触触发 aoe,但 single 拖拽链路的引导线、hover、蓄势和拖后快进均未能稳定截到;该部分建议按派单交给用户真玩判手感。
