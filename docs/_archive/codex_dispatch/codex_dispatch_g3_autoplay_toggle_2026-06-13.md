...
R1 你建议的现代图标问题已采纳:`smart_toy/touch_app` 换成**绛红方印「自/手」朱文 glyph**(红框 + 浅红底 + 红字,自动=「自」/ 手动=「手」)。暂用现有字体单字(真小篆需篆体字体,12px 下篆体笔画繁复难辨,楷体「自/手」更清晰 → 留后续美术 polish)。R2 顺带看:
- 主线 `stage_list_autoplay`:01_01「自」印 enabled、01_03/04「自」印灰显 disabled、01_02「手」印 enabled,印章在 1280×720 下是否清晰、红度是否合宜(别太刺)。
- 第 1-6 项功能行为已 PASS 不必重验,只看新印章观感 + 第 7' / 8' 爬塔 dialog。
