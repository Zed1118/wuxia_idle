...
- [ ] 测试 `setUpAll` 样板专项重构(**两批累计迁移 21 文件,2026-07-10**):新增 `test/support/test_data.dart` 与 `isar_test_support.dart`,统一生产配置加载和 Isar Core 幂等初始化;首批迁 8 个代表性文件并有 helper 自测,第二批聚焦 `test/data` 迁 13 个生产配置测试(含 broken-loader 红线测试,52 测全绿)。其余文件按触达增量迁移,不做一次性机械改写。
