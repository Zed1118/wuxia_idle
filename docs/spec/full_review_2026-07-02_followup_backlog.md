...
- [ ] 测试 `setUpAll` 样板专项重构(**GameRepository 三批累计 43 文件 + Isar 首批 13 文件,2026-07-10**):新增 `test/support/test_data.dart` 与 `isar_test_support.dart`,统一生产配置加载和 Isar Core 幂等初始化;配置加载首批迁 8 个代表性文件并有 helper 自测,第二批 `test/data` 13 文件(52 测全绿),第三批 battle 22 文件(146 测全绿),同类 loader 样板 83→61;Isar 首批聚焦装备/背包/商店 13 个持久化测试(94 测全绿),直接 Core 初始化 95→82。其余文件按触达增量迁移,不做一次性机械改写。
