...
## 2026-09-10 已批准增量（当前实现）

用户“按推荐执行”批准替换此前前台只更新时间的策略：无主动闭关时，前台、后台、关闭按相同配置产率累计普通经验与磨剑石，整数即时入账，尾数持久保留。产率锚点不改，现有普通挂机不设时间上限；下方 6 月历史条款中的 72h cap、仅离线发放、单一时间字段不再代表当前实现。

- 独立 `passiveLastSettledAt` 负责收益；`lastOnlineAt` 保留 presence / 原有伤势恢复用途。前台心跳与收益边界不恢复伤势，启动/恢复焦点才沿用原恢复行为。
- 经验到达层级阈值的时刻切换后续产率，长段与每分钟分段使用同一积分规则。主线解锁、丹药、战斗发奖、传位、开始闭关前先结旧账，再读取角色并应用新状态，均在原事务内。
- 收益以持久 `founderCharacterId` 指向的当前掌门为准；缺失或死亡不猜对象、不推进收益时刻。经验尾数留给原角色，材料尾数属于存档。
- active 闭关占有其完整时段（含闭关上限后的既有普通尾段），不叠加第二份普通收益；收功在同一事务重启普通锚点，保留闭关前尾数。
- 海岛已初始化时，普通挂机的真实跨阶时刻同步分割其旧生产窗口；外部成长与传位在改变境界/掌门之前推进旧窗口。进入海岛、收取及切方先同步普通账，避免先后顺序导致提前解锁生产。未开岛不自动初始化；不把每次普通心跳改成海岛生产 tick。
- 新掌门创建时建立时间锚点。旧档只有已建立 presence 的时段可结算；未建立时段不历史补发。0.50 迁移与缺字段 NaN 修复见 `data_schema.md` 的本日增量。

下方内容保留为历史设计记录；本增量优先适用于上述冲突项。自动验证不替代正式玩家验收。
