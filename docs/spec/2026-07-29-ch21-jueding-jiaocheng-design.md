...
| 6 | **机制层** | 21_04 vuln 0.16 + chargeCounter 两相位;21_05 vuln 0.10 + cycleVulnerability 2:0.05 + 两相位 + **`surviveTicks: 10` 主线首用** | 承 Ch20(0.18/0.12/0.06)逐章收一档;终章换胜负轴=段级拍板 B;0.05 = §5.4 承伤乘子**下界本身**,收官到底不再有下一档;ticks **已探针校准**详 §9 |
...
## 9. surviveTicks 探针校准实录(2026-07-29 · 15750 场)

**先修了一处探针失真**:`progression_battle_probe.dart:130` 此前未透传 `stage.winCondition`,
凡配 surviveTicks 的关,探针测的都是「打死 Boss 要多少拍」而非真实战斗时长。已修(未配该
条件的 104 关行为不变)。

**关键机制认识**:`default_ground_strategy.dart:101` 既有语义是「击败」与「撑满」**任一即胜**
(同心魔 07 · CLAUDE §5.4 明载)。故**调高 ticks 不会让战斗更长,只会让更少人走到「守住」结局**
—— 而本章题眼正是「不必赢」。原设 18 正踩这个反向。

纯击败耗时实测(未灌 winCondition·50 seed/档):

| 档 | min | 中位 | avg | max |
|---|---|---|---|---|
| undergeared | 15 | 20 | 19.64 | 24 |
| standard | 9 | 13 | 12.44 | 19 |
| nearMax | 6 | 6 | 6.12 | 7 |

候选值 → 各档走「守住」路线占比:

| ticks | undergeared | standard | nearMax |
|---|---|---|---|
| 10 | **100%** | **96%** | 0% |
| 12 | 100% | 68% | 0% |
| 18(原设) | 72% | 2% | 0% |

**取 10**:只有刻意满配的玩家会把少年打趴(守 §5.4「满配秒杀=有意爽感」),正常玩家都拿到
题眼结局。灌入 winCondition 后复测兑现:undergeared 10.00 / standard 9.96 / nearMax 6.12 拍,
胜率全 100%。

**附带发现**:undergeared 档玩家终局残血由 Ch20 的 0.9466 **掉到 0.6124** —— 终关成为主线
最吃力的一战(打不穿只能守满),但胜率仍 100%,既守 §12 定调 A(不做 undergeared 战败威胁),
又让收官有分量。这是全段唯一一处「机制真的构成压力」的关卡。
