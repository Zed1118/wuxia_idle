...
- 专属立绘：`elder_grey` → `battle_elder_grey`（布帽、灰须、竹杖的笑面老者，避免与白发宽袍祖师撞型）；`shaonian` → `battle_shaonian`（轻灰练功服、拳掌架势的春水堂青年）。
- 新增覆盖：2 种敌人 / 2 次出场；累计覆盖 44 / 79 种、85 / 120 次出场。
- 技术验收：两图均为 RGBA、alpha `0–255`、四角透明、完整头脚；脚底比例已写入战场锚点。
- 待完成：同章 `guntou`，以及 `stage_02_02` / `stage_02_03` / `stage_02_04` 的真实整队双视口验收。
