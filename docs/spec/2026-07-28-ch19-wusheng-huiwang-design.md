# Ch19 章级设计 spec — 武圣段首章 · 回望弧开篇 · cross-tier · 已拍板并实装
...
**状态**:**六项全 A 已拍板(用户「按推荐开」)· 本批已实装**

> **实装期两处订正(照实记录,不修饰)**
> 1. **`cycleVulnerability` 语义我在 §4 #4 写错了**。它的 key 是**周目数**(schema 强制 `>=2`·`stage_def.dart:405`),是**二周目收窄窗口**,不是我写的「战斗内周期开窗」。段级 spec 原文「cycleVulnerability 周目加压」是对的。实装按真语义做:19_05 配**基础窗口 0.15(管首通)+ 周目 2 收窄 0.08(管二周目)**两件,首通仍有实档,机制新台阶也成立。
> 2. **Boss 位仍是 {4,5}**。§1.3 原写「19_04 接关人 / 19_05 守镜人 由地理决定」,实装期把西行链完整倒推后确认:东返序为 西凉城下 → 沙海深处 → 黑风口 → 门户关城 → 黑石戈壁,两个复出人物天然落在 {4,5},Boss 位体例不必动,阳关外驿道留 Ch20 开篇。
...
## 3.5 探针实测(2026-07-28 本批 CSV 重生成 · 14251 行 · Ch19 750 行)

undergeared 剖面,`hp_end/hp_start`:

| 关 | ticks | 残血 | 动作行 |
|---|---|---|---|
| 19_01 | 7.56 | 0.7860 | 9.02 |
| 19_02 | 8.48 | 0.6851 | 11.18 |
| 19_03 | 5.30 | 0.9353 | 6.96 |
| 19_04(章中 Boss) | 7.94 | **1.0000** | 13.28 |
| 19_05(末 Boss) | 8.04 | 0.9550 | 14.14 |

**末 Boss 落在历代段首带内**(13_05 0.9117 / 16_05 0.9294 / 15_05 0.9379),Ch18_05 的 0.4178 是段末收官刻意尖峰,不作段首对标。终章动作行 14.14 > 门槛 8 ✓。

**逮到一条跨章系统性趋势(非本章引入,须单独拍板)**:章中 Boss 残血逐章单调变软 —— 13_04 **0.4993** / 14_04 0.5124 / 15_04 0.4773 / 16_04 **0.1882** / 17_04 **0.9099** / 18_04 0.9437 / 19_04 **1.0000**。**拐点在 Ch17**,早于本章两章。根因:敌攻自 Ch18 起封顶 2000、Boss HP 逼近 60000 硬线,而玩家每加一章就变强一截。**关键推论:机制层救不回这条**——`vulnerability` 只减少 **Boss 受到**的伤害(拉长战斗),不增加**玩家承受**的伤害。故段级 spec §4「难度 100% 走机制层」只成立一半:机制能拉长,不能构成威胁。Ch20/21 设计前建议先拍这条。

## 4. 拍板 6 项(2026-07-28 全 A · 已实装)
