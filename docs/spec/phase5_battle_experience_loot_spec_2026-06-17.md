...
## 3. 主线二 · 普攻节奏 + 即放打击感 + 首通门控（风险高 · 实装升 xhigh）

- 批次 2.1 诊断：balance_simulator 加「普攻 vs 技能 击杀/伤害占比」探针 → 实测当前 → 证伪「普攻过强」（不凭体感）。
- 批次 2.2 数值：据诊断调普攻（拆普攻系数或降倍率），模拟器跑多境界×周目×build；红线测兜底（不进百万 + 首通节奏区间 普通20-40s/精英45-75s/Boss90-150s 折算 ticks）。
...
| 主线 | 批次 | 状态 |
|---|---|---|
| 一 战斗 UI 表达 | 1.1-1.4 | 待开工 |
| 二 普攻+即放+门控 | 2.1-2.5 | 待开工（2.3 升 xhigh） |
| 三 掉落传闻 | — | 待开工 |
