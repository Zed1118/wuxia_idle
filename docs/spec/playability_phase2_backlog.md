...
- **仍需用户拍板/实玩校值**:残页集齐数量、战斗节奏真机校值、音频转用素材听感复核、音频专属真素材生成与筛选、桃花岛正式场景美术。
- **仍是候选且未见已合入证据**:祖师开局塑形 7 项 + 待定姓名/门派名、装备出售/分解后的匠人委托、长线平衡审计任务、材料来源反查、终局装备目标追踪、爬塔层数结构复核、新手前 30 分钟体验打磨。
- **已由既有批次覆盖,不应重复开工**:2026-06-29 下一阶段 13 任务批次、第 4 梯队 10 任务批次、招式熟练度可视化/离线收益结算明细等旧候选。
...
- [ ] **开局祖师流派三选一**:玩家开新档时选择刚猛、阴柔、灵巧,决定主修心法、起手招式、属性倾向和角色气质。
- [ ] **三份命盘候选**:新档创建时一次生成 3 份命盘供玩家选择,遵守现有单项 1-10 / 总点 16-24 规则,兼顾随机感与反刷数值膨胀。
- [ ] **祖师出身选择**:选择山野散人、镖局旧徒、书院弃生、药庐门人等出身,影响少量初始资源、属性倾向或心法起点。
- [ ] **祖师创建页**:替代静默 seed 流程;新档首次进入时先完成祖师创建,再进入主菜单。
- [ ] **属性效果预览**:创建祖师时即时说明根骨、悟性、身法、机缘对血量、修炼、速度/暴击/闪避、奇遇/掉落等方向的影响。
- [ ] **祖师履历写入角色档案**:把玩家选择的流派、出身、命盘结果写入角色面板/传记,后续可回看这一代祖师从何而来。
- [ ] **开局模板数据化**:把祖师候选流派、出身、命盘权重、属性倾向放入 yaml,后续扩展祖师路线不需要改 Dart 逻辑。

#### 待定

- [ ] **祖师姓名与门派名创建**:新档创建祖师时同步输入祖师名、门派名,替代默认「祖师 / 我的门派」。
...
1. **装备链路一组**:材料来源反查 → 终局装备目标追踪 → 匠人委托。装备筛选与锁定优化已完成,后续先补可查性,再加长期目标与新委托系统。
2. **反馈可读性一组**:新手前 30 分钟体验打磨。招式熟练度可视化与离线收益结算明细已完成,早期体验可作为下一步候选。
