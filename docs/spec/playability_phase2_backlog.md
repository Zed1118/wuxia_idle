...
- [ ] **桃花岛产物消费补齐**:把锻材、开锋辅材、行囊补给等尚未形成终端用途的产物接进现有系统,让桃花岛生产链更闭环;不改在线=离线结算原则。
- [ ] **单人主线 01_05→06_05 平衡调参**:祖师单人线前 4 关已修,后续 26 关仍可能单人 1v3;按关卡敌阵、Boss、掉落节奏做实际体验优化。
- [ ] **爬塔单人路径打磨**:塔只按境界解锁,不按弟子入队门槛;针对高境界单人爬塔补敌阵/机制/提示层优化。
- [ ] **塔 20/25/30 Boss 机制增强**:优先补阶段控场、多目标压力、反制窗口等机制杠杆,避免只靠堆数值制造挑战。
- [ ] **招式熟练度可视化二期**:在既有藏经阁可见性基础上,继续统一战报、角色页、技能详情中的熟练度收益与当前效果说明。
- [ ] **离线收益回归卡优化**:隐藏 0 值项,按来源分组展示经验、材料、装备、熟练度等回归信息;只解释在线=离线结果,不引入加速。
- [ ] **疗伤丹战后接入**:战斗结束后可直接用疗伤丹处理伤势,消耗桃花岛丹房产物,减少有药但用不上的断裂。
- [ ] **主线重打收益路线**:在已通关章节里突出刷装备、刷材料、刷熟练度的不同重打价值,让玩家知道为什么回头打。
- [ ] **装备掉落后即时处理**:胜利结算里对新装备提供锁定、查看来源、标记常用、稍后处理等轻量操作,减少仓库堆积感。
- [ ] **商店货架需求提示**:商店物品显示当前谁用得上、哪个系统会消耗,降低经验丹、材料、秘籍购买决策黑箱。
