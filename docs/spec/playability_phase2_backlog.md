...
- [x] **P2 队伍成长与三人协同**(master spec §四 / §13 P2):**协同深度 + ①渐进解锁 + ④控制倾向已实装**(第六阶段破绽窗口 `2026-06-18-phase6-coop-break-window-*` · 第七阶段批三 `disciple_join_service` 开局单人→收徒扩队 · `LineageRole.senior/junior` + senior 破防倾向)。**续作待拍板(非漏做)**:② 出战编成/换人 UI(✅ v1.39 出战编成屏 2026-07-15 已实装,本条 stale 勾销)③ Boss 协同窗口 ✅ 第八阶段(敌方协同)2026-08-06 全五切片收官(掩护-蓄力+护法合击·塔 42 首实例·真机目检待用户后关账)——P2 全子项闭环。
...
- [ ] **转用素材听感复核**:battleUlt(realmAdvance_v2_01 裁 2.4s)/ battleChargeStart(defeat_v2_02 负向预警)是转用,真玩听感不合再重打专属 prompt。**2026-07-14 用户拍板挂账**:专属素材生成暂不做,借用态(temporaryBorrowed)继续可用不阻塞任何批次;prompt 已存档(`docs/_archive/suno/suno_dedicated_battle_sfx_prompts_2026-06-29.md` 每槽 3 条)随取随用,择期生成+听选后按 manifest(`docs/handoff/dedicated_audio_asset_manifest_2026-06-29.md`)流程落位并翻 `finalAsset`。
...
- [x] **战斗节奏真机校值**:A+C 已实装(spec/plan `2026-06-23-battle-pacing-readability-*` · 合 main)。初值 `action_interval_ms=1000`/`key_moment_hold_ms=400`/`damage_popup_ms=1000`(旧记 700 已 drift,以 numbers.yaml 现值为准)待真机 `flutter run -d macos` 看常速战斗手感调定(纯 numbers.yaml 配置·连带同步 `AnimationNumbers.defaults`)。**2026-07-14 并入 §十三 #2 T4**:三初值建议维持;新增首通展示帧三停顿(900/800/700,animation.first_clear_*)同场一并手感;**2026-07-14 勾账**:随 §十三 #2 T4 真机验收确认三初值维持(1000/400/1000),首通三停顿(900/800/700)竞态修复后真机呈现 PASS。
...
- [x] **开锋槽 3 专属技内容化**(2026-06-29 合 main `64706eb9`/`b2bbb993`):skills.yaml +21 招(7 阶×3 流派·max 倍率 7000≤8000·三系锁死守)+ equipment.yaml 36 武器重指向;装备 build 深度已由 `codex/next-special-skill-slot3-content` 覆盖。
- [x] **桃花岛场景化主屏**(2026-06-29 合 main `64706eb9`/`57f1986d`):场景 hub + 7 建筑热区点击切详情卡;正式美术仍归「桃花岛主屏场景化画面」挂账,本项布局底座/热区架构已完成。
- [x] **多存档体验二期**(2026-06-29 合 main `64706eb9`/`769a11ed`):重命名/删除二次确认/卡片信息增强(祖师/主线/问鼎/最近游玩 badge)。未改 saveVer/schema,复用既有字段。
...
- [~] **音频专属素材补齐**(2026-06-29 合 main `64706eb9` 仅完成元数据/manifest/prompt):battleUlt / battleChargeStart 已标 `temporaryBorrowed` 并生成专属素材清单,但真音频仍待外部生成/筛选后接入;不重复开代码分支。
- [x] **资源总览页**(2026-06-29 合 main `64706eb9`/`790a46cb`):五类库存只读聚合+来源/用途,主菜单入口;明示「不在此消费/结算」。后续二期见第 4 梯队 `220eac37`。
- [x] **装备典故阅读体验升级**(2026-06-29 合 main `64706eb9`/`7555a451`):装备详情典故区升级为「器物志」阅读层;预设典故可展开,动态延续典故以「持有人记忆」兼容展示。
- [x] **主菜单状态摘要二期**(2026-06-29 先暂缓评审,后合 main `8b7fca48`):只读摘要 provider + 主菜单摘要面板已接入;用户拍板后「当前要事」红线争议已收口。
- [x] **关卡列表章节轴重做**(2026-06-29 合 main `64706eb9`/`d1e4d56d`):竖向 timeline 轴路标;不做地图地域化、不加新收益。真机目检细节仍按计划挂账,不等同未实现。
- [x] **技能详情页表现层升级**(2026-06-29 合 main `64706eb9`):秘本纲要 4 块(流派/破招/熟练收益/典型用途),提升武学详情质感。
- [x] **设置页二期**(2026-06-29 合 main `64706eb9`/`b7719631`):战斗播放速度(只缩放播放节拍守在线=离线)+文字密度+减少闪烁,SharedPreferences 持久化。
- [x] **战斗 UI 术语统一整理**(2026-06-29 合 main `64706eb9`):`CombatTerm` 枚举集中术语,蓄力/破招/真气/御体/相位/重伤单一来源;不新增教程弹窗。
- [x] **桃花岛生产队列可读化**(2026-06-29 合 main `64706eb9`/`a086f5c2`):二分探测下一件/满仓时间只读派生,建筑卡内嵌生产情报,守在线=离线。
...
- [x] **桃花岛建筑手册**(2026-06-29 合 main `8aa41f25`):选中建筑卡加入「建筑志」,说明产出/消耗/协同/去向;纯说明层,不改产量。
- [x] **装备掉落后详情强化**(2026-06-29 合 main `3c225116`):胜利结算新装备详情强化来源、可用角色、境界门槛、锁定建议;延续即时处理,不做装备目标追踪。
- [x] **商店货架分层展示二期**(2026-06-29 `codex/fourth-shop-shelf-groups-v2` 合 main `12518584`):商店按购买意图分组,并显示库存已有量;固定货架,不做刷新/折扣/购买逻辑改。
- [x] **开锋槽 3 专属技展示升级**(2026-06-29 合 main `e60b6809`):装备详情以「器物绝招」展示触发条件、流派、适合角色;不改候选数量和数值。
- [x] **离线回归卡叙事化分组**(2026-06-29 第 4 梯队余 7 合入 `af360f1d`;计划 `2026-06-29-fourth-offline-recap-narrative-groups.md`):active/passive 回归卡按既有收益字段做叙事分组;不改收益。
- [x] **闭关中状态牌**(2026-06-29 第 4 梯队余 7 合入 `af360f1d`;计划 `2026-06-29-fourth-seclusion-active-status-card.md`):闭关页内部显示地点、时长、预计收获类型;不放主菜单催促。
- [x] **桃花岛产物去向标签**(2026-06-29 合 main `a7c448d7`):产物列表标「用于疗伤 / 用于开锋 / 用于强化 / 可出售」等只读标签;不新增消费。
- [x] **问鼎九霄进度条美化**(2026-06-29 第 4 梯队余 7 合入 `af360f1d`;计划 `2026-06-29-fourth-tower-progress-polish.md`):爬塔页增加进度条、当前可挑战层、最高进度与 Boss 节点标记;不改楼层结构/奖励。
- [x] **秘籍获得仪式小升级**(2026-06-29 第 4 梯队余 7 合入 `af360f1d`;计划 `2026-06-29-fourth-skill-acquire-ritual.md`):获得秘籍/残页改用「得卷」小型展示层;不改掉落概率、不做残页来源聚合。

### 2026-06-30 backlog 真实剩余摘要(夜间 worker 1 审计)

- **仍需实玩校值**:残页集齐数量、战斗节奏、音频听感与专属素材筛选、桃花岛正式场景美术。
- **仍属明确暂缓**:装备出售/分解后的匠人委托、终局装备目标追踪;不得作为普通候选自动开工。
- **已由既有批次覆盖,不应重复开工**:祖师开局塑形与命名、材料来源反查一期、爬塔结构复核、长线平衡审计、新手前 30 分钟体验,以及 2026-06-29 的 13 任务/第 4 梯队批次。

### 2026-06-29 祖师开局塑形待办

> **✅ 2026-07-03 drift 订正 + 收官**:本段 7 项及 line 146「仍是候选未见合入证据」是 **living-doc-state-drift**——Phase 0 探查证实 **6/7 早于 `c967a959`(saveVer 0.33.0·2026-06-27)已实装**(流派三选/三份命盘/出身四选/交互式创建页/属性预览/履历写档案)。唯缺第⑦「姓名与门派名输入」于 2026-07-03 补齐(创建页名号段 + `founder_names.yaml` 掷名 + 主菜单门派横幅·不改 schema·全量 3598 绿)。spec/plan `docs/superpowers/{specs,plans}/2026-07-03-founder-sect-naming*`。**本段整体收官**。

> 用户拍板:本批次按 Codex 推荐加入待办。目标是把生产新档的「固定祖师模板」升级为开局可选择的祖师塑形流程;不增加战队人数,不做出战编成 / 换人 UI。未选的「单次摇点」和「手动调点」已迁入 `rejected_task_registry.md`。

- [x] **开局祖师流派三选一**:玩家开新档时选择刚猛、阴柔、灵巧,决定主修心法、起手招式、属性倾向和角色气质。
- [x] **三份命盘候选**:新档创建时一次生成 3 份命盘供玩家选择,遵守现有单项 1-10 / 总点 16-24 规则,兼顾随机感与反刷数值膨胀。
- [x] **祖师出身选择**:选择山野散人、镖局旧徒、书院弃生、药庐门人等出身,影响少量初始资源、属性倾向或心法起点。
- [x] **祖师创建页**:替代静默 seed 流程;新档首次进入时先完成祖师创建,再进入主菜单。
- [x] **属性效果预览**:创建祖师时即时说明根骨、悟性、身法、机缘对血量、修炼、速度/暴击/闪避、奇遇/掉落等方向的影响。
- [x] **祖师履历写入角色档案**:把玩家选择的流派、出身、命盘结果写入角色面板/传记,后续可回看这一代祖师从何而来。
- [x] **开局模板数据化**:把祖师候选流派、出身、命盘权重、属性倾向放入 yaml,后续扩展祖师路线不需要改 Dart 逻辑。

#### 已完成补项

- [x] **祖师姓名与门派名创建**:新档创建祖师时同步输入祖师名、门派名,替代默认「祖师 / 我的门派」。
...
- [x] **招式熟练度可视化打磨**(2026-06-29 合 main `ae6b46ca`;旧候选由「招式熟练度可视化二期」覆盖):统一战报、角色页、技能详情中的熟练度收益与当前效果说明。
- [x] **长线平衡审计任务**(2026-07-01/05 两轮完成):已复核掉落、银两、强化材料、心法修炼、离线收益、终局极值 build 与周目回报;七维全在带内,实战峰值 213,015,守住不进百万。详 `docs/audit/long_term_balance_audit_2026-07-05.md`。
- [x] **装备筛选与锁定优化**(2026-06-28/29 合 main `1cfda7ed`):背包/仓库已支持流派、锁定、保护态筛选;出售/分解保护沿既有规则,无 schema/saveVer。
- [x] **离线收益结算明细**(2026-06-28 合 main `847d5a46`;2026-06-29 `e20c8c55` 继续优化):回归卡已展示经验/材料/熟练度等来源明细并隐藏 0 值项;只解释在线=离线结果,不引入加速。
- [x] **材料来源反查一期**(2026-07-06 合入):`MaterialSourceSheet` 已接强化、开锋、分解、商店等 5 个入口;资源总览折叠区复用同一 sheet 属独立薄接线候选,不应重开一期。
- [ ] **终局装备目标追踪(历史候选 · 本轮暂缓)**:允许标记一件装备作为长期养成目标,聚合强化、共鸣、开锋、材料缺口与可行动入口。
- [x] **爬塔层数结构复核**(2026-06-28/07-01 完成):30 层结构、Boss 节点、掉落曲线与难度坡度已有专项审计和机制收口。详 `docs/audit/tower_structure_review_2026-06-28.md`。
- [x] **新手前 30 分钟体验打磨**(2026-07-01 实装,07-10 复验):完成创建说明、失败诊断、首胜整备、重打门控,并以 macOS 真窗口复验新手/主线/离线流程。详 `docs/audit/runtime_onboarding_acceptance_2026-07-10.md`。
...
- [x] **#2 战斗爽感二期(表现层)**:首通脚本化展示帧(开局亮相/首技慢镜/Boss 蓄力提示/破招题字,07-09 自审已给方向)+ 技能伤害占比调参(现 38.4%,普攻 61.6% 主导观感)+ battleUlt/battleChargeStart 两个借用 SFX 专属化(整合 §七 听感复核、§九 节奏真机校值两个既有开放项)。**2026-07-14 实装批(`feat/combat-feel-phase2`)**:T1 展示帧四拍(纯表现层,整场各一次)+ T2 占比校准 39.9%→47.1%([balance] 首通血线 3.0/1.5,探针证据见 plan)+ 展示帧题字验收路由已合入分支;**T3 SFX 素材 2026-07-14 用户拍板挂账**(借用态可用,prompt 已存档随取,§七 承载,不再阻塞本条);**2026-07-14 T4 完成(bg 会话代跑·用户授权)**:真机首通验收发现展示帧生产入口竞态并修复(`0a09e9e2`,详 PROGRESS 当日条+plan 恢复点),复验四拍生产呈现;§九 三值维持。PR #37 已合 main `d4724d04`,**本条勾账**(SFX 素材延续 §七 挂账,不属本条)。
...
- [x] **#4 出战编成/换人 UI(2026-07-14 已拍板:做,C 完整编成屏)**:三拍板=C 完整编成屏 / 纯编成不动成长(inactive 成长渠道留后续独立拍板,追赶补助仍 rejected)/ 并入 PR #36 观察①(心法主辅修选择流收窄版)。spec 见 `2026-07-14-team-lineup-screen-design.md`(取代 07-05 brief)。**2026-07-15 实装批完成(`feat/team-lineup-screen`,PR 待审合)**:编成屏+LineupService+择路流全落;实装期补 spec §2 两处订正(战斗态=路由结构性保证 / 加入者须已修主修硬前置,详 spec 订正块);Codex 真机目检走 visual_route `team_lineup`。
