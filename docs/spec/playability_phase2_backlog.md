...
- [ ] **装备出售/分解后的匠人委托**:冗余装备回收后积累材料与委托进度,解锁修复残器/定向开锋/铁匠请求;必须守买断制与非日课,不做限时刷新。
- [x] **招式熟练度可视化打磨**:2026-06-29 二期完成。复用 `SkillProficiencyFormatter` + `StageProgressRow` + `skill_proficiency` 配置,统一藏经阁/战报/角色页/心法页/武学详情中的当前效果、下阶效果与战斗放招来源说明;不复制公式、不改数值。
...
- [ ] **终局装备目标追踪**:允许标记一件装备作为长期养成目标,聚合强化、共鸣、开锋、材料缺口与可行动入口。
