# Ch20「东入阳关」章级设计 spec — 武圣段中章 · **六项已经用户复核确认(2026-07-29)**
...
## 0. 实装订正块(2026-07-29 用户复核批补 · **读本 spec 前必先读本节**)

> ⚠️ **下文 §2 表中 3 项在实装阶段已被推翻,本 spec 当时未回写**。实装(PR #95)才是真值,
> 差异如下;用户 2026-07-29 复核后**确认维持实装版**,不回滚。

| # | §2 原拍(已作废) | **实装真值(现行)** | 改因 |
|---|---|---|---|
| 2 | 关名 …关门 / **关册** | …**接程** / **孤城开** | 末关名与真解同名收束,「接程」对 16_01「送关」 |
| 3 | 末 Boss = 新人物**记关人**(lingQiao) | 末 Boss = **守关老将**(Ch15_05 复出·**yinRou**·沿 Ch15 既定) | Phase 0.5 核 canon 逮到 `chapter_15:16`「守的就是这一开」是 5 章前埋的伏笔,复出兑现 > 新编人物 |
| 4 | 真解「**阳关有故人**」lingQiao | 真解「**孤城开**」`skill_gu_cheng_kai`·**yinRou**·mult 7400 | 绑 #3;与 Ch15 掉的「孤城闭」成对,canon 自生长 |

**副作用留档**:#3/#4 从 lingQiao 改 yinRou,使 §2 #3 原理由「lingQiao 平衡三章 school 分布」失效,
末 Boss school 连续四关 yinRou。已实测确认并转为 Ch21 硬约束,详段级 spec §13。

**其余 3 项(章名 / 神物第二批 4 件 / 机制层三值)实装与 §2 一致**,机制三值已于 2026-07-29 经
15000 场探针校准**确认维持不调**(证据见段级 spec §13)。
