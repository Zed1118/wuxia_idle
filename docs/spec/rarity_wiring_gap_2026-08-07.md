...
## 三、[待拍板] rarity 是「出生标签」还是「当下标签」

> ⚠ 2026-08-07 订正:本节初稿写成「关键设计判断:rarity 必须是 computed」,把一个
> **GDD 未settle 的产品语义问题当成了技术正解**,属未经拍板的倾向性结论。改写如下。

GDD §4.1(`GDD.md:224`)原文:「**不可重 roll**。出生即命运,但奇遇可微弱后天弥补
(**每个角色整个生涯内**最多 +3~5 点)」。两句话对本题给出的信号相反:

- 「出生即命运」→ 档位是**出生烙印**,后天不变(存储字段,创建时定死)
- 「奇遇可弥补」+ 六档表是**总点数**的映射 → 总点数长了,档位机械地就该跟着变(派生值)

GDD 没有明说档位本身是否随奇遇升档,**这是产品语义决策,需用户拍板,不是工程取舍**。
