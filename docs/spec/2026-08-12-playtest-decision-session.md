...
> **⚠ 本节原文有误,协调者 2026-08-12 复核订正**。交付方原写「BACKLOG.md 一区未见 #19 编号条目(一区实际编号:17/18/15/16/4/5/6)」——**两处均不成立**:`git show origin/main:BACKLOG.md` 现查,一区**第 14 行确有 `| 19 | 资质视觉档位化`**,且一区真实编号是 **17/18/19/4/5/6**(#15/#16 早已销账删行,不在一区)。交付方的**结论**(该决策项仍开着、只剩视觉档位化未拍)经代码注释独立验证**是对的**,错的是支撑论据。下文保留其对该项来龙去脉的梳理,仅删去失实的编号断言。

本项即 `BACKLOG.md` 一#19「资质视觉档位化」,与 `docs/dispatch/pool/README.md` 的 **P11 滚动池条目**同源:
...
1. ~~**一#19 编号错位**:BACKLOG 一区无 #19 条目~~ **该声明本身失实,协调者已订正**:一#19 条目确实存在(`BACKLOG.md` 一区第 14 行),一区真实编号 17/18/19/4/5/6。决策项真实成立这一结论不变,详见上文「编号说明」的订正块。
1b. **协调者抽查记录(2026-08-12)**:本清单的代码引用逐条现查抽验通过——`wuxia_tokens.dart:97` 的 `entryJianghu = 'assets/ui/mj/entry_city_defense_01.png'`、`lineage_character_detail_screen.dart:303-304` 的「待视觉终拍」注释、`main_menu.dart:532` 的 `thumbnailPath: WuxiaUi.entryJianghu`、`main_menu.dart:184-194` 的桃花岛解锁门控、`battle_standee_fusion.dart` 三常量(`:92`=125.0 / `:96`=195.0 / `:104`=0.85)、以及 `~/Desktop/Projects/挂机武侠素材/桃花岛美术候选_20260807/` 下六张候选与复检图,**全部逐值/逐字符对上**。唯一失实项即第 1 条。
