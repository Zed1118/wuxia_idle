# 武圣段(Ch19-21)设计 spec — 主线终段 · 已拍板

**日期**:2026-07-28 · **载体**:main 直落(bg 会话·纯文档)
**境界阶**:武圣(wuSheng·绝对层 43-49·终极境界)· 主线 Ch19-21 三章 · 承 Ch18「阳关故人」
**状态**:**已拍板(2026-07-28 用户「全 A」· §6 九项全取 A 案)** — Ch19 实装批前置解除
**前置实测**:HEAD `cec7cd1a` · analyze EXIT=0 · 全量 4712/0 · 18 章 90 关 · cap 42
**拍板前复验**(2026-07-28 拍板会话现 grep · 非转抄):cap 42(`numbers.yaml:206`)/ 主线 90 关 · 六段各恰 3 章 requiredRealm 递进 / **tier7 显式 drop·fragment·gauntlet = 0 门**(全仓 tier7 只 4 招且全 `special`)/ tier7 技能招 21 且仅 `yinrou_chuanshuo_fang_*` 一组已用 / 神物 11 件除 equipment.yaml+lore 外全仓零引用 / 飞升条件③ `wuSheng·dengFeng`(`numbers.yaml:1830-1832`)—— **§1 实测面全部复验通过**
...
## 6. 段级拍板项(2026-07-28 全部拍定)

> **拍板结果:九项全取 A 案**(用户回「全 A」)。下表「推荐」列即最终决议,无一项走 B/C。
> 连带确认:③ 飞升 Ch21 收官即开 → **抬 cap 到 49 是本项目传承/二周目循环首次真正对玩家开放**;⑤ 真解三章各新写 tier7 mainline_drop 一门(倍率须落在 ≤8000 硬线内,宗师段真解 6400 是唯一参照);⑦ 机制层复合档位**不预先钉值**,由 Ch19 实装批探针实测校准。
...
- **Ch21**(cap 47→**49 封顶**):**主线终章** · 真解 ×1 · 神物第三批 · 末 Boss 复合机制 + 新胜负条件 · **飞升三条件在此全满足**,章尾 hook 指向传承/二周目而非下一段(没有下一段了)。**硬约束(2026-07-29 复核新增)**:末 Boss school **必须 lingQiao 或 gangMeng,不得再取 yinRou**——见 §13。
...
## 13. Ch20 探针校准结论 + 末 Boss 流派约束(2026-07-29 用户复核批 · 现测)

**探针** `progression_playtest_diagnostic_test`(100 关 × 3 档 × 50 seed = 15000 场·EXIT=0)· undergeared 档逐关实测:

| 关 | 机制配置 | ticks | 玩家残血 | 胜率 |
|---|---|---|---|---|
| 19_04 | 仅 chargeCounter(无 vuln) | 7.94 | **1.0000** | 100% |
| **20_04** | **vuln 0.18** + 两相位 | **8.84** | **0.9296** | 100% |
| 19_05 | vuln 0.15 + cycle 0.08 | 8.04 | 0.9550 | 100% |
| **20_05** | **vuln 0.12** + cycle 0.06 | **8.16** | **0.9466** | 100% |

**结论:六项拍板 #6 的三个机制值维持不调**。同流派对照方向全对——20_04 加 vuln 比 19_04(无 vuln)拉长 +0.90 tick、残血从毫发无伤 1.0000 掉到 0.9296,兑现「章中复合化台阶」;20_05 收紧一档比 19_05 再长 +0.12 tick、残血再降 0.0084;三档胜率全 100%,未引入意外战败 = 守 §12 定调 A。**口径局限**:`progression_battle_probe.dart:150,168` 玩家 school 固定 gangMeng,被克的 yinRou 敌关(20_02/20_05)天然偏快,**不可据此断言「末关比章中关软」**,只有跨章同流派对照有效。

**新约束(数据支撑)**:末 Boss school 实测 18_04/18_05/19_05/20_05 **连续四关全 yinRou**,gangMeng 玩家可连吃四章末 Boss ×1.25 克制(20_05 nearMax 仅 3.30 tick)。Ch20 已被叙事绑死(守关老将沿 Ch15 既定,不改),**修正窗口留给 Ch21**:末 Boss 必须 lingQiao 或 gangMeng。
