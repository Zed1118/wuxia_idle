...
> **2026-06-18 修订（Phase 0 补漏）**：开工后发现 battle_screen 已有屏震基建 `_shakeCtrl` + `screenShakeOffset`（整 SafeArea，暴击固定振幅触发 `_spawnPopup:623`）。故 Task 6 **复用 `_shakeCtrl` 并分档振幅**，不新建 CameraShake；**删除 Task 5 建的 `camera_shake.dart`**（防双抖 + YAGNI）。`ScreenFlashOverlay`（Task 5）保留。详 spec §4 修订。
