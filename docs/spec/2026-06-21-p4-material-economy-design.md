...
### 4.1 银两货币（新）
- 存储：`SaveData.silverTaels`（int 字段），**saveVer 0.27→0.28**，老档迁移补 0（迁移逻辑 `lib/data/isar_setup.dart:160-195`）。
- 来源接线（全走 numbers.yaml，§5.5 离线=在线）：
  - 敌人掉落 / 关卡通关 / 爬塔首通：战斗结算 `BattleResolutionService` 路径加银两产出。
  - 闭关产出：`seclusion_service` 沿 `mojianshi_per_hour` 体例加 `silver_per_hour`。
- 不做卖出。

### 4.2 江湖商店（新 · sink）
- 主菜单入口，解锁条件 = 首次获得银两（`silverTaels > 0` 曾为真，沿兵器谱解锁谓词体例）。
...
| Phase | 范围 | 红线风险 | 交付独立性 |
|---|---|---|---|
| **P1 经济核心** | 银两货币（SaveData.silverTaels + saveVer 0.28 + 来源接线）+ 江湖商店（只卖磨剑石/心血结晶，已有用途）+ 材料背包查看 | 低 | 自洽闭环，无死钱包、无无用货品，可独立 ship |
...
- `SaveData` 加 `silverTaels` int 字段 → saveVer 0.27→0.28 → 全仓同步硬编码版本断言（见 memory `feedback_version_bump_test_assert_sync`，grep 禁 `| head` 截断）。
- 新增 `data/shop.yaml`（货品 id + 标价 + 货架分组）+ schema 校验。
- numbers.yaml：银两各来源产出系数（掉落/通关/首通/闭关 `silver_per_hour`）。
- 经验丹/秘籍 item def（P2）：经验丹给多少经验、秘籍对应哪门心法。

## 8. 测试要点

- 银两迁移：老档加载补 0、saveVer 往返。
