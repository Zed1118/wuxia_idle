...
> 消费方：下一片 reducer + 根应用纯 Flutter 战斗表现层（实装排期在 Qoder deterministic simulation core / input adapter 冻结之后，见接线审计 §4）。本契约只定义**语义事件**，不锁 Dart API、不绑定未实装类名、不含伤害/CD/半径等平衡数值（数值一律归根配置层 yaml + 红线 validator）。probe 不再改，仅作手感/性能对照；正式验收对象 = 根应用真实入口。
> 参考模型：probe `feedback_events.dart` / `feedback_cues.dart`（语义事件 + 事件→cue 纯函数映射，静音契约）；本契约沿用其「事件只携带语义、不携带数值表现参数」的原则。
...
- 顺序/去重：同一 `actor` 相邻两次 attack_started 之间必有 hit_landed 或被取消语义；按 `seq` 去重。
...
- payload：`actor`、`target`、`move_kind`、`is_critical`、`is_ultimate`（均布尔/语义档，无伤害数值）。
...
- 音频：`battleDeath` 槽位预留未接线（`audio_assets.dart:47,55`）；接线前回退 `battleStagger.mp3` 或静音（选用见 manifest 音频组）。
...
- payload：`actor`；applied 另带 `affected`（受影响单位语义 id 列表，有序）。
...
- payload：`slot`（技能印语义位）、`availability`（五态枚举：`ready` / `cooldown` / `qi` / `casting` / `down`）。
