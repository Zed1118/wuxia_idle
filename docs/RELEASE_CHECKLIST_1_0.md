...
- [~] 内容文案最终 polish(typo / 古风一致性 / 主线叙事流畅度)— **2026-06-02 Phase 0 复扫:文案整体已很干净**(元信息穿帮 0 / 标点 0 真问题〔ASCII 命中全 YAML flow 结构逗号〕)· 本轮修 12 处硬错(11 对话/引语半角引号→直角「」 + 网游词「传说级」→「传说神功一脉」§5.2 · `6fc123c`)· 强调引号 `"词"`(~58 处)用户拍**保持现状**(作者有意「词强调 vs 「引语」」两级 · jing_hong_shen_fa 同行并存为证)· 续 typo 抽样
...
- **v1.14**(2026-06-02)窗口最小尺寸 Pen 实机验收闭环:`3db46b2` WM_GETMINMAXINFO 锁 1280×720 经 Pen Codex **3/3 PASS**(门1 真实鼠标拖窗锁死缩不下去 / 门2 锁定点无 RenderFlex overflow 主菜单完整 / 门3 默认窗口正常 · 日志 0 exception)。**验收有效性已证**:Pen git pull 卡住、基线 `f0d2e5a` + 手动 apply patch,但 blob 校验(`f0d2e5a:win32_window.cpp` == `3db46b2` 父版本 · `72366d6` == `3db46b2`)→ Pen 编出 native runner 逐字节 == `72366d6` 发布版。**§12.9 上线门达成,1.0 闭环唯一「待验证」工程项消除**。0 代码改动纯状态对齐(纯 doc)。
