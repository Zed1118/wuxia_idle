...
## 六、修法方向(已派单 E 实装 · 含一条被本人推翻的初版建议)

> **推翻记录(留痕,防后人重走)**:本文档初稿建议「把 `-R` 换成全屏截图 + 按几何裁剪」,理由是全屏截图锁屏时实测是活的(实验一 `LOCK=YES` 下 23,626,735 B 成功)。**该建议随后被自己推翻,不要采纳**——`-R"0,0,W,H"` 能对上画面靠的是 `:376` 那次 osascript 把窗口摆到左上角;锁屏时 osascript 操不了窗口,几何前提本就不成立,按 (0,0,W,H) 裁全屏会裁出**错内容**。而主路径(窗口截图)锁屏时是活的 ⇒ **一旦走到 fallback,就说明窗口是真的没找到/截不出,此时并不存在"正确的画面"可抢救**。产出一张看似成功、内容却错的图,比明着失败更糟(本仓有过 720 版整张错拍的事故)。

定稿方向 = **让失败可诊断、可定位、且立刻停住**,不抢救出图:

1. **截图失败时输出锁屏态与分模式结果**:会话是否锁屏(判据 `ioreg -n Root -d1 -w0` 的 `IOConsoleUsers` 字典是否含 `CGSSessionScreenIsLocked`;**未锁时该键整个不存在**,只能判键的有无,不能比值)、`window_id()` 究竟返回了什么(空/`-1`/具体 id,现状完全没留痕,是排查时最缺的一条)、窗口截图的退出码与字节数。
2. **`-R` fallback 保留**(未锁时实测有效,仍是有用兜底),但**它也失败时立刻非零码结束该路由采集**,并打印人能读懂的结论;**不再往下走到 `crop_window_content.py`** —— 现状让用户看到的是一个 PIL 的 `FileNotFoundError`,与真实原因隔了三层。
3. `capture_visual_window` 的状态串扩充成能区分「窗口截图成功 / 走了区域 fallback 且成功 / 全失败(带锁屏态)」,写入位置与既有 `VISUAL_CAPTURE:` 前缀不动(下游与 4 份终拍日志按此解析)。

派单包:`docs/dispatch/2026-08-12_E_visual_capture_lock_diagnostics.md`。
