...
M5 不能从历史纵切 READY 或已经关闭的 M6 工程接线直接推导为完成。按二阶段方案 M5 的六模式 × 七项合同重新建立固定分母后，当前 main 可证明 `34/42`，仍有 `8/42 BLOCKED`，因此顶层 M5 工程 Gate 保持 `0/1 BLOCKED`。

本轮没有发现需要在既有 owner 内做一两行修补的局部回归；缺口分别涉及个人记录持久作用域、尚未实现的自动化通道和百草岭首次关键节点的手动接管。为避免假绿，本轮不新增“把现状写成常量再断言”的集成测试，也不改变现有产品语义。
...
| 轻功 | PASS `LightFootScreen → runStageFlow` | PASS `LightFootService` 路线链 | PASS 已通路线 durable dispatch | PASS 共享 stage/durable receipt | PASS 通用伤势归 exact participant | PASS `MainlineProgress` 路线通关 + 个人战斗账本 | **BLOCKED** durable dispatch 已有，但前台 bot 与即时 headless 未接生产入口 | 6/7 |
| 守城 | PASS `MassBattleScreen → runStageFlow` | PASS `MassBattleService` 关卡链 | PASS 已通关 durable dispatch | PASS 共享 stage/durable receipt | PASS 通用伤势归 exact participant | PASS `MainlineProgress` 守城通关 + 个人战斗账本 | **BLOCKED** durable dispatch 已有，但前台 bot 与即时 headless 未接生产入口 | 6/7 |
...
合计：`5 + 6 + 6 + 6 + 6 + 5 = 34/42`。

## 八个真实阻塞

1. 九霄塔缺每角色塔层/最好成绩记录；既有 `TowerProgress` 是每存档单行，旧审计已明确不能拿 Boss 纪念替代。
2. 九霄塔缺首通后的当值/差遣 durable owner；当前只冻结并实现 sweep。
3. 轻功缺已首通后的生产前台 bot 与即时 headless 入口；现有 durable dispatch 不能外推为两条通道。
4. 守城缺已首通后的生产前台 bot 与即时 headless 入口；现有 durable dispatch 不能外推为两条通道。
5. 心魔通关事实是存档全局 `clearedStageIds`，未形成角色个人心境进度。
6. 断魂庄缺完整首通后的差遣/离线 durable owner；现有 headless replay 是 direct，不占用真实世界时长。
7. 百草岭没有首次关键里程碑必须手动的接管边界。
8. 百草岭没有 `routeId + milestoneId` 的自动化解锁记录，因而也无法证明“首次手动、以后自动”。

其中 1、2、5、6 很可能需要新的或扩展的持久 owner；7、8 还需要生产 UI/恢复状态机与离线遇门自动返程。它们不是本次“集成守卫”可以诚实补齐的测试缺口。
...
2. 再补短通道：轻功/守城前台 bot 与即时 headless 复用现有 mapper、bot adapter、headless runner 和 settlement，不新增内核。
3. 再补长时 owner：塔当值/差遣与断魂庄差遣先冻结 durable session/占用/奖励选择边界，再做迁移。
4. 最后补百草岭手动里程碑接管；它跨离线推进、可见战斗、返程报告和恢复，是八项中风险最高的一项。
