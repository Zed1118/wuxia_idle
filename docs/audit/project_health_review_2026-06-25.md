...
| 3 | **40 招池仅 29 挂 encounter**，11 招未被任何 encounter unlockSkill 引用 | 可能预留 / 未接线 | **值得拍板确认**：11 招是有意预留还是漏接线 |
| 4 | `encounter_skills.yaml` 头注自称「35 招」实测 40 | stale 注释 | 顺手订正 |
| 5 | `techniques.yaml:18` 头注引用已退役的 DeepSeek 文案流程 | stale 注释 | 下次动该文件删一行 |

## 7. 风险评估

- **高**：无（红线/数据/接通/测试全绿）。
- **中**：无 CI（#1）——全绿依赖本地手动跑，重构期回归无自动兜底。
- **低**：11 招未接线（#3，需拍板）；4 处文档 drift（#2/#4/#5，纯卫生）。
