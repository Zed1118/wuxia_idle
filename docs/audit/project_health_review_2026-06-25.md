...
| # | 项 | 性质 | 处置建议 |
|---|---|---|---|
| 1 | 无 CI | 降级为「非必要」 | 2026-06-25 Windows 真机验收实测平台风险清零（test 2905 逐位同 Mac + release exe 成功，详 `windows_acceptance_2026-06-25.md`），且证明 **Windows 验收可按需 SSH 自驱**（不依赖常驻 CI）。CI 边际价值低，1.0 后再评估，非隐患 |
| 2 | ~~CLAUDE.md §7「仍然不做」仍列 PVP，但 PvpService 已实装~~ | ✅ **已订正(2026-06-25·用户拍板)** | §7/§9 把 PVP 从「仍然不做」移到「已在 1.0 周期实装」(P3.3·本地 mirror 对战 + ELO + UI·ELO 持久化留 Phase 5+·不违 §5.1)。CLAUDE v1.21→v1.22 |
...
| 5 | ~~techniques.yaml:18 头注引用已退役的 DeepSeek 文案流程~~ | ✅ **已订正(2026-06-25)** | 改「description 已就地全填(Mac 单端维护)」,清 DeepSeek 退役引用 + 过期占位说法 |

## 7. 风险评估（2026-06-25 更新）

- **高**：无（红线/数据/接通/测试全绿）。
- **中**：无。
- **低**：#1-#5 全部收口/降级。Windows 发布目标平台已实测验证（test 逐位同 Mac + release exe 成功 + 启动不崩），平台风险清零。
- **唯一人工兜底项**：GUI 动态画面 / 战斗手感 / 中文渲染目检（SSH 无图形回传，启动不崩 ≠ 画面正确）——ship 前人工实机一次。
