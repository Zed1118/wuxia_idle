...
按 §8.3 读作 WIP,不可评不可合。
**2026-08-27 订正**:本文初版把这写成「就绪标记机制的一个洞」,该判断**错误,现予撤回**。
`CLAUDE.md` §8.3 明写「就绪与否**以 tip 前缀为准**」——后续任何 commit 让旧状态失效
是**有意的 fail-closed 设计**,不是缺陷:旧 Gate 是在旧代码树上跑的,
改成「扫描最后一个带标记的 commit」会把旧 Gate 结论错误套用到已经变化的树上。
正确处置是 **merge 后重跑 Gate,再追加一个绑定当前 HEAD 的新状态 commit**。)
...
### 4.3b 两处定性订正(2026-08-27 用户复核后)

1. **T2 被排除在当前 G2 之外是正确的授权处置,不是代拍。**
   本文初版曾把 codex「T2 排除是正确的」判为代拍,该判断**过重,现予收回**。
   当前生效授权仍是 A,T2 自身状态是 `[BLOCKED]`,
   因此不把它并入当前候选是**照章办事**。真正属于用户的问题是
   「`2/1/1/0` 是否永久不可用」——这一条 codex 没有替谁回答,本文也不应替谁回答。
   用户 2026-08-27 已裁决:**保留为独立实验候选,需要时再做真人 A/B 试玩**。

2. **测试计数不是 codex 算错,是两边口径不同。** 本文与盘面初版称其「绝对基数错误」,
   **该措辞收回**。四种口径并列如下,增量在任一口径下都是 **+2 文件 / +12 用例**:

   | 口径 | `1cf7df37` | `g2-human-ready` | 增量 |
   |---|---|---|---|
   | `test/` 下全部 `.dart` | 820 | 822 | +2 |
   | 仅 `*_test.dart` | 803 | 805 | +2 |
   | 宽口径调用 `(test\|testWidgets)\(` | 5526 | 5538 | +12 |
   | 严格缩进 `^[[:space:]]*(test\|testWidgets)\(` | 5505 | 5517 | +12 |

   上表四行均本会话实测。codex 报的宽口径为 5525/5537,与我实测恒定差 1
   (正则细节差异),**增量一致**;文件数 820/822 与我实测完全一致。

   **教训**:引用计数必须同时写明分母口径,否则两个都对的数字会被误判成一方出错。
...
一直 `[BLOCKED]` 无人拍板(2026-08-27 用户裁决:T2 保留为独立候选,不纳入当前 G2,也不永久否决)。这不是 codex 的问题,是决策留痕机制有洞。
