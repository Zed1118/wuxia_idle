# P2-M6-U06 江湖地图第三地点（守城试炼）纵切

- taskId: `P2-M6-U06-JIANGHU-MAP-MASS-BATTLE-LOCATION`
- milestone: `M6`
- owner: `codex_root`
...
延续 U06 生产地图架构：把“守城试炼”从主菜单平铺项迁为江湖地图第三个生产地点。地点锁定和进度只从现有 `MainlineProgress` + `MassBattleService` + production config 派生；点击仍经 `guardBattleEntry` 进入既有 `MassBattleScreen`。

## 非目标

- 不改群战 Ch6 解锁、五关链、周目、阵型、战斗、奖励或记录语义。
- 不同批迁移断魂庄、远征、商店/声望或其他地点。
...
- `docs/audit/phase2_m6_u06_jianghu_map_mass_battle_location_2026-08-25.md`
...
1. 红测证明主菜单仍平铺守城，地图没有守城地点。
2. 转绿后主菜单不再平铺守城；地图在新档显示原 Ch6 锁定语义，解锁后显示 production 五关进度。
3. 地点点击仍经既有 battle guard 进入 `MassBattleScreen`。
4. 覆盖 1280×720 / 1440×900，运行主菜单、地图、群战相邻回归、纸面对比审计和两层 analyze。
5. 独立语义复核与全量通过后才同步审计/真相源并生成 clean READY。

## 停止条件

若迁移必须改变 Ch6 门槛、群战链、阵型、准入或战斗去向，立即记录精确 `BLOCKED`；不在地图复制第二套解锁真相源。
