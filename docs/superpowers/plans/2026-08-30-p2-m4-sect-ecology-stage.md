...
- 当前分母：M4-A `0/1`；只有上述 6 项全部进入受控候选才变为 `1/1`。M4 父 Gate 仍为 `0/1`，不用一个生态纵切冒充六生态完成。
...
- 状态：WIP，生产纵切已闭包，等待破坏证红与批末工程门。
- 最后完成：`stage_02_02` 已接入 25 人/10 active 的四角色门派生态；typed catalog、stage-local verified refs、runtime bundle、production host、角色显示名与 25 目标完成链均已转绿。
- 下一步：冻结候选后做显示名消费与 stage assignment 双向破坏证红，精确还原；随后跑 analyze、整仓 format、持锁全量、macOS Debug build 和 gate。
- 已跑验证：display-name schema 相关 100/100；Ch1 相邻生产链 + Ch2 纵切 19/19；`test/data/phase2` 52/52；catalog loader/validator 66/66。fresh worktree 已完成 `pub get --offline` 与 build_runner 128 outputs。
- 阻塞项：无新产品决策阻塞；真人目检按用户授权统一延后。
