...
- Task 8 收口门禁绑定收尾前 `HEAD=ea45ff27125771a7d2162d4153613280456fbfe2`：format 1118/0 changed；analyze 0；工作树 `git diff --check` 与整分支 `git diff --check "$(git merge-base HEAD main)"..HEAD` 均无输出；十文件定向集 JSON reporter 非隐藏 43 success/0 failure/error；全量 JSON reporter 非隐藏 3933 success/0 failure/error，1144 hidden testDone records（597 loading + 547 setUpAll/tearDownAll）未计入 pass；macOS debug build 成功，仅有 `audioplayers_darwin` Swift actor isolation 和 Xcode 脚本 output dependency 告警。一次性未提交 debug harness 复用生产 widget/repository/临时 Isar，完成前期、心魔溢出、战后反馈、Lv490 四画面 @1280×720/1440×900 共 8 张真窗口原尺寸目检，全部 PASS；临时 harness 已删除，截图路径与 SHA-256 见审计报告。未新增 P0/P1，P2-1/P2-2 不变，`lib/` / `data/` / `numbers.yaml` / schema / save version 未修改。
- 下一步：分支已由当前 `[READY]` tip 冻结；合并和推送等待后续明确授权。
...
最终实现位于 `test/tools/progression_playtest_diagnostic_test.dart`：先在内存生成
1800 行并验证固定 header、行数、字段、30 stages、3 profiles、seed 0～19 与组合
唯一性；默认把同一内容写入 `Directory.systemTemp` 的本次临时目录，验证后清理，
并逐字节及 mtime 断言 tracked CSV 未变。只有
`UPDATE_PROGRESSION_PLAYTEST_EVIDENCE=1` 时才用目标同目录临时文件 + flush +
rename 原子更新 committed CSV，并打印 `updated evidence`。诊断不再断言
`ticks < 240`；共享 runner 已负责拒绝真正未完成的 tick-cap，本轮 max tick=24 只打印
为软观察。平均 ticks 统一使用 Dart `toStringAsFixed(3)`。
...
Expected: 两种模式均 PASS；默认模式不改 CSV 的 SHA-256/mtime，显式更新后生成内容
与现有 1801 行 CSV byte-identical 且 `git diff` 无输出。
