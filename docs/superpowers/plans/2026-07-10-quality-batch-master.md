...
- 状态：四份实施计划已完成并通过占位、类型和规格覆盖自审。
- 最后完成：16 项任务已映射到批次 A-D；存档恢复设计已由用户批准，规格提交 `d85ecabc`。
- 下一步：执行批次 A 的 Task 1，先写恢复 domain/file-ops 失败测试。
- 已跑验证：设计文档 `git diff --check` 通过；分支推送成功。
- 阻塞项：无。
