...
- 状态：批次 A 功能实现完成；批次 B Isar 迁移执行中。
- 最后完成：非 feature Isar 初始化 14 文件迁移，直接调用 66→52。
- 下一步：迁移 `test/features/` 余下 52 文件；批末统一做 A 的 macOS 视觉验收。
- 已跑验证：批次 A 定向 analyze 0、47 测；Isar 迁移第一批 104/104。
- 阻塞项：无。
