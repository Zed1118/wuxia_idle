...
|---|---:|---|---|---|---|---|---|---|---|---|
| 普通主线 `battle_boss_phase` | ~20 min | 待执行 | 待采样 | 不适用 | 待观察 | 待观察 | 待观察 | 待观察 | 待判 | 待填 |
...
- 状态：进行中——Ch7/Ch8 原单已于 `230952cb` 第一次 `[READY]` 冻结；追加动态观察目标已登记，tip 将回到 WIP。
- 最后完成：只读盘点 battle46 已知风险与既有真机 route；确认主线/塔可分别用真实 `stage_01_05` 与 floor 30 的长战 route，三种特殊模式也有既有 route。
- 下一步：先跑普通主线约 20 分钟，记录窗口帧、route log 与同一进程 RSS 前后采样。
- 已跑验证：原单 42/42 + 日志 + Git 对账已完成并冻结；追加单尚未开始计时。
- 阻塞项：无。
