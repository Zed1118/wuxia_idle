...
- Gate 稳定性修复：仅为该测试库增加 2 分钟时间预算；不改断言、不 skip、不改生产实现，并保持 test diff 纯新增、零删除。现场 `pmset` 证明系统已有防休眠断言，后续长停顿落在另一条 Isar 事务内，故根因记为整仓并发下的 Isar 原生 I/O 阻塞，而非产品回归或系统休眠。
- 修复后全量：默认并发、持锁执行 `flutter test --no-pub`，`5734/5734`、末行 `All tests passed!`、退出码 `0`；墙钟 `38:43`，其中存在跨文件 Isar 长阻塞，后续作为测试基建债单列，不冒充性能正常。
- 最终独立 Gate：待最终 READY tip 后记录。
