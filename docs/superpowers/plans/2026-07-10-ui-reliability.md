...
- [ ] **Step 1: 定义关键位清单**：主菜单背景、章节/关卡封面、角色头像、装备详情、商店商品和桃花岛地图；装饰纹理、印章允许静默收起。
- [ ] **Step 2: 写失败审计测试，读取关键文件并要求对应 `WuxiaImage` 调用包含显式 `errorBuilder`。**
- [ ] **Step 3: 每个命中点复用现有业务 fallback（图标、文字或 `ErrorFallback`），不新增占位资产。**
- [ ] **Step 4: 运行图片、商店、主菜单、章节、角色和桃花岛定向测试。**
