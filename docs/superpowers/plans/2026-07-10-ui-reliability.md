...
- [x] **Step 3: 运行 shared widget 测试与 1280x720 标题栏视觉路由。**
- [x] **Step 4: 提交**：`git commit -m "Unify title bar navigation actions"`。
...
- [x] **Step 5: 缺失资产尺寸与诊断由 fallback widget/audit 测试锁定；macOS 双尺寸实机截图确认正常资产路径容器稳定。**
- [x] **Step 6: 提交**：`git commit -m "Harden critical image fallbacks"`。

## 当前恢复点

- 状态：Task 1-2 实现、提交和批末视觉验收均完成。
- 最后完成：main_menu、chapter_list、equipment_detail_screen 在 1280x720 与 1440x900 下完成真实窗口截图目检。
- 下一步：进入 PR CI。
- 已跑验证：标题栏/shared widget 13/13；图片共享组件 8/8；关键页面 141/141；全量测试 3790/3790；相关 analyze 0 问题；六张截图均 READY、无 overflow/exception。
- 阻塞项：无代码阻塞，按批次顺序等待。
