...
- **下一步 C2.1（入场扣帖+补给托管·单 writeTxn·未开工）：** 新建 `application/gauntlet_service.dart` `enter(characterIds, supplyDefIds, now)`。**Phase 0 已摸清（下会话零重复起手）**：① 模板 = `ExpeditionService.dispatch`（`expedition_service.dart:42`）+ 测模板 `expedition_dispatch_test.dart`（Isar setUp 见 `test/support/isar_test_support.dart`·本 worktree 已验 Isar 事务测可跑·libmdbx OK）。② **占用服务已认 bossGauntletRun**（`character_occupancy_service.dart:38`）→ 建会话后占用自动生效，**无需改 occupancy**。③ 断魂帖 `item_duanhuntie`(ItemType.ticket) 扣减走 `inventoryItems.getByDefId().quantity--`；补给 `item_liaoshangdan`(hp 0.30)/`item_xingnang_buji`(qi 0.20)。④ `enter` 单 writeTxn：占用校验(`CharacterOccupancyService(_isar).snapshot().isCharacterOccupied`)+ 扣一张断魂帖 + 补给移入 `escrowItemDefIds/LoadedQty/UsedQty`(UsedQty=0·普通库存 quantity 减) + 建 `BossGauntletRun`(inBattle/stage 1/members 快照/seed)。校验(仿 dispatch)：1-3人/无重复/非祖师/未占用/已修主修/每存档≤1 active/断魂帖≥1/补给≤3(supplyCap)。⑤ **seed**：无 `gauntletRunSerial`(异于远征)，用 `saveId` 派生定值存 `run.seed`，每关组合层再混 currentStage(仿 `ExpeditionSeed.forNode`)。⑥ 守恒测：补给移入后普通库存不再持有(扣量==loadedQty)。⑦ 失败/认输不退帖(§5.1)——close 只返补给(C2.2)。
