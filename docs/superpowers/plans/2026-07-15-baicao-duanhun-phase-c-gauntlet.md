...
- **下一步 C2.3（崩溃恢复到关次开打前整备态·§5.6/§10·未开工）：** 检查点只两类时刻（每场战斗结束进整备页 / 整备页每次用药后）——二者均已是 `put(run)` 持久点。关闭重进 → 恢复当前关**开打前整备态**重打该关；断魂帖不重扣·已用补给不返不复制·已击败关次进度/队伍/暂存奖励保留；生命/真气/冷却/阵亡为该关开打前检查点原值不回满；同一稳定随机流不重抽（seed 已存 run·每关混 currentStage）。配置损坏：第一关前不可恢复 → 关会话退帖；已开战 → 认输关闭。**Phase 0 关键待摸**：谁**驱动一场战斗**？= load run → 从 members 建 playerTeam（`buildPlayerTeamForCharacters` + 用 members 的 currentHp/qi/cd 覆盖满血基准）→ `runStage`（seed 混 currentStage）→ `advance` → 持久化。此「战斗驱动 service 方法」**尚未建**（C1.3.3 只给纯 runStage/advance）——可能并入 C2.3 或先做独立切片；C2.3 崩溃恢复本质是「不重复驱动已完成关 + 从检查点续」。
