...
- **下一步 C2.2（庄内整备页用药·会话托管消耗·守恒·未开工）：** 整备页(interlude/awaitingRewardChoice 前)用药 → `escrowUsedQty++` **不碰普通库存**（§5.1）；疗伤丹恢复 1 名存活角色 30% maxHp、行囊补给恢复全体存活 20% maxQi（§5.1 表·`item_liaoshangdan.gauntletHpHealPct`/`item_xingnang_buji.gauntletQiRestorePct`）；战斗中不可用药（仅关次间）；不能复活倒下者/不清持久伤势。close/结算时 `装入−已用` 原子返还普通库存（§5.1·归 C2.2 或 C2.5）。Phase 0 待下会话摸（模板 = enter 的 escrow 反向消费 + 整备页状态机）。
