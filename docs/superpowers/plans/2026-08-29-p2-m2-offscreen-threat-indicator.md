...
- 正式全量两次稳定失败于
  `phase0a_focus_nav_test.dart:234`：旧断言以角色的屏幕 y 位移证明 W 键
  冒泡。75% 玩家跟随 camera 会主动抵消角色屏幕位移，该口径已不再成立。
- 初版处置把旧屏幕坐标断言替换为 world-state 断言，整仓全量恢复 5664/5664；
  但独立 gate 正确检出 32 条测试删除，最终判决 `FAIL: test_deletions`，未合并。
- 最终处置恢复所有基线测试原文，只新增 camera-aware `stage.updateCameraCenter`
  见证；生产绘制层让 standee key 精确落在脚点，并为跟随镜头下的移动保留
  固定 4px、下一渲染帧清除的步向摆势。伤害数字上浮距离由 34px 增至 64px，
  保证 75% camera 下缩放入场仍净向上移动。
- 修正后 base→工作树测试 diff 的 `^-[^-]` 计数为 0；focus 8/8、屏外
  11/11、Stage 7/7、整屏 28/28 均通过。该提交之后需重新绑定双向破坏
  证红与完整九步证据。
