...
- 生产输入路径未失效；测试已改为在技能印持有焦点时检查真实
  `controller.state.player.position.y` 下降，仍直接证明 W 从子焦点冒泡到屏幕
  handler 并驱动 world-state 上移。
- 修正后 focus 8/8、屏外 11/11、整屏 28/28 均通过。该提交之后需重新绑定
  双向破坏证红与完整九步证据。
