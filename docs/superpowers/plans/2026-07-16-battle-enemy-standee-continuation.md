...
- **状态**：进行中。
- **最后完成**：`31386b9c` 已提交 `bandit_head` / `qingshan`，早期主线五名过渡敌人全部专属化；继续完成第二章 `elder_grey` 竹杖灰须老者与 `shaonian` 春水堂拳掌青年的生成、抠图、alpha 技术检查和生产映射，覆盖提升至 44 / 79 种、85 / 120 次出场。
- **下一步**：运行第二章映射回归并提交小切片；继续补 `guntou`，随后集中跑第一、二章和低层塔真实冻结路由双视口验收。
