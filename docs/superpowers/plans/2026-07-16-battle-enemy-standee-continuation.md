...
- **状态**：进行中。
- **最后完成**：在前三张专属画像基础上，完成 `bandit_head` 独眼宽背刀匪首与 `qingshan` 青色束额青年剑客的生成、抠图、alpha 技术检查和生产映射；早期主线五名过渡敌人已全部专属化。
- **下一步**：运行映射回归并提交早期主线资产收口切片；随后用真实 `stage_01_02` / `stage_01_03` / `stage_01_04` 与 tower 2 / 3 / 8 冻结帧集中做双视口体量验收。
