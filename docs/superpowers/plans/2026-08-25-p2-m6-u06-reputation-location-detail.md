...
- 状态：独立 worktree 已建立，生产合同与精确白名单已登记，尚未写红测或生产实现。
- 最后完成：确认现入口门槛为第一章末关，去向为 `ReputationPanelScreen`；生产数据来自六门派、七阶配置和 playerId=1 稀疏声望行；NPC 关系当前 source 身份不可由入口可靠推导。
- 下一步：提交登记；安装依赖/生成后写 provider、详情屏和地图路由红测，保存真实编译失败。
- 已跑验证：只读代码/配置审计；未运行本切片测试。
