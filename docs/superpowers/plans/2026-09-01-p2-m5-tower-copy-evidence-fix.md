...
5. 证据口径：M5 审计记录 final-tip `5840/5840`、Gate PASS 与原始本机证据边界；外部审查校正将 U11/U12 标为 `UNPROVEN`。
6. 红线：零 schema/saveVersion、YAML、数值、技能、奖励、经济、解锁、战斗规则改动；中文 UI 文案只进入 `UiStrings`。
7. 收口：定向测试、analyze、整仓 format、锁保护全量和 Gate 全 PASS；最终 diff 符合范围且工作树 clean。
