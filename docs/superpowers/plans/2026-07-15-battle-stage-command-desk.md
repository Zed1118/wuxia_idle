...
- **状态**：持续推进，尚未完成。正式 `stages.yaml` / `towers.yaml` 共 79 种敌人、120 次出场，目前 24 种 / 65 次已接入透明全身战斗立绘。
- **最后完成**：补齐江湖客、名门执事与爬塔 5/10/15/25 层 Boss 的战场展示映射，先消除纸底半身像；爬塔四名 Boss 当前为过渡复用，未视作专属画像定稿。
- **下一步**：按身份、武器与阵营辨识度处理剩余单次敌人；优先重做过渡复用 Boss 的专属透明全身图，并对真实主线/爬塔关卡逐一执行 1280×720 与 1440×900 目检。
- **已跑验证**：`character_avatar_test.dart` 14 项通过；正式数据覆盖脚本确认 24/79、65/120；`flutter analyze --no-pub` 零问题；`git diff --check` 通过。
