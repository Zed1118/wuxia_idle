...
- 实时基线：六模式中断魂庄已有 typed automation policy，百草岭已有 typed dispatch request，心魔已有本人 manual-only typed admission；塔、轻功、守城仍没有玩家可达的自动 runner 与 typed production admission。
- 当前关键阻塞：冻结矩阵允许的塔/轻功/守城首通后 bot/headless/差遣路径尚无权威 production runner owner；纯新增声明表和单元测试不会证明生产接线。
...
- 现有定向合同与地点展示回归 `92/92 PASS`，只能证明已存在路径没有回归，不能补出三个缺失的 typed production admission。
- 后续解阻必须先为塔、轻功、守城建立经授权的真实 production runner/admission 纵切，再由 U14 做一张穷举允许矩阵和入口全状态复核；不得先写一张无人消费的“真值表”。
