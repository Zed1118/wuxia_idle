...
本切片建立三份不可变 typed def，与既有叙事 `EncounterDef`（encounter_def.dart）明确分名、零字段共享：
...
- **显式 migrationState**：`CombatStageEncounterAssignment` 逐 stage 显式声明 `legacy | migrated`（无默认路由）；`migrated` 必须恰有一个 encounterId，`legacy` 必须无 encounterId（both/neither fail closed），镜像 E05 `Phase0aEncounterMigrationState` 语义。
...
- **内容中立 objective 原语引用**：`CombatObjectivePrimitiveRef` sealed 八类（defeatTargets / destroyAnchors / reachCheckpoint / touchMarkers / surviveDuration / defendEntity / pursueTarget / defeatCommander），镜像 O01 八原语；只携带 kind + 显式参数（id 集合非空唯一干净、ticks 为正），不硬编码任何 stage/内容 ID，不选择黑风岭胜利条件。
...
- 状态：实现完成，验证通过，准备提交。
- 最后完成：三份 typed def + 三份单测落盘；与 O01 八原语 kind 对齐（读 O01 worktree `encounter_objective.dart` 核对）。
- 下一步：提交实现 + empty READY commit（`[READY][PI][P2-G2-S01] 完成战斗内容类型合同`）。
...
- 阻塞项：O01 原语文件未合入本 worktree（并行切片），S01 引用形态按 O01 已读源码对齐、不 import 其类型；loader（L01）与主控整合时需 bridge。
