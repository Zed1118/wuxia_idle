...
3. 心魔失败只增加受上限约束的内息紊乱；不得产生主修修炼度、境界、永久内力、装备或伤势的惩罚性回退，正常战斗计数等通用结算保持不变。
...
4. 普通 Boss 的零层回退分支仍使用修炼度摘要字符串，因此本批保留该字符串。
...
5. 回写 CLAUDE/GDD/PROGRESS、任务/决策登记、审计、真实提交号和测试计数，最后生成唯一 READY 恢复点。
...
- 除本决议明确退役的 `inner_demon.failure_penalty` 外，还需要修改其他生产配置、存档 schema 或数据迁移。
