...
- [x] Bug C 修法红绿轨迹:先加翻红测(钉 4 条形态中可测者)→ 实装复绿
- [x] 破坏证红:反向补丁还原 Bug C 清洗 → 新测精确红 → 还原复绿
- [x] 扫描器测试全绿(解析层 19 + git 交互层 20)
- [x] 新基线实测:dead 373 → **87**(翻活 4 + audio 排除 36 + 归档吸收 246,
      逐项吻合;archival 593 → 828)
...
- **状态**:实装完成,收账中
- **最后完成**:README 定位升级(`57010a58`)· 破坏证红已做 · 新基线 dead 87
- **commit 链**:`e82b340a` RP0 → `1c7b2965` 红测 → `1a106610` 实装 →
  `57010a58` README 定位升级
- **下一步**:merge main → BACKLOG 销 #18/#20 → PROGRESS 登记 → push → CI
- **阻塞项**:无
