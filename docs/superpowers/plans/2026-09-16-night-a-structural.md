...
- 状态：A-1、A-2、A-3、A-4 READY；目标验收完成，仓库内收据已更新。
- 最后完成：A-4 三场景生产装配事件流 golden；完整套件 6935 项通过，伤害变异触发 3 失败，已精确还原。
- 下一步：无待实施代码目标。收工提交后的最终 tip 验证与外置收据由 `../logs/final_*`、`../receipt.yaml`、`../summary.md` 记录，不再改动代码与此恢复点。
...
- A-4 READY：`b97184705ab89a1064518a33dc477fd12b9cad0e`；收口执行 `date`：`Thu Sep 17 01:36:46 CST 2026`，未触发围栏，全部目标已完成。
- 在该 READY 提交后复跑代码单双向破坏证红：移除主线掉落写入 `00:06 +16 -10: Some tests failed.`；塔层数归零 `00:01 +3 -2: Some tests failed.`，退出 1，10/2 失败；两次原字节恢复且工作区干净。证据归档 `../logs/A-4_receipt_break_*`。
- 仓库内收据更新为 A-4 最后代码与测试 READY，使用本目标全量、分析、格式及已执行双向破坏证红；提交收据会产生新的最终 tip。依照协调者明确授权，外置 `../receipt.yaml` 覆盖为该最终 tip 的实测收据，`../summary.md` 同步覆盖。
- 最终 tip 上仍执行完整套件、分析、格式及提交后的双向破坏证红，结果仅写外置日志和收据，避免 SHA 自引用。外置收据的本地解析对撞不等于协调者独立复测。
- 待拍仅有 A-1 生产 null/缺项配置的后续治理，详残留登记；本单无未解除授权阻塞。范围外 138 点只登记。
