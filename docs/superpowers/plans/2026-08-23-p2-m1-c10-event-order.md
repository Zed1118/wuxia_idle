...
本片只新增三白名单文件。生产接线、现有 `Phase0aEvent` 迁移、UI/VFX/SFX、聚合算法和最终数值留给 G1 审核及后续任务。此次复核补齐了 feed 输入顺序与非表现字段约束；主审保持 feed entry 的非空字段类型，投影先固化输入快照，所有 ID 必须是已 trim 的非空字符串。仍未接生产。
