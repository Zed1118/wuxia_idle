...
**先攒批,再停。** 命中下列任一项时**不要立刻停下**:先把本单其余待拍板点、以及所有不依赖该答案就能查清的事实全部排查完,攒成**一份清单一次性问**。只有当"不知道这个答案就无法继续排查"时才立即停。

依据:2026-08-29 实测,`00:43:11` 与 `00:49:11` 连着两个 `[BLOCKED]`(屏外提示视觉方案、屏外可见性架构)间隔仅 6 分钟,本可一次问完;当晚等拍板累计 6 小时 15 分,占总跨度 46%,是整体吞吐的头号瓶颈——**不是执行速度慢,是往返次数多**。

清单格式:每项写「问题 / 我的推荐方案 + 一句话理由 / 备选 / 你不回复我会怎么做」。
...
## §11 预授权决策(用户 2026-08-29 批准,免停直接执行)

下列三类你**不必停下**,按你自己的推荐方案直接做,做完在 plan 文件里记录「依据 §11.x 预授权执行 + 推荐方案是什么 + 为什么选它 + 有哪些备选」。

1. **表现层视觉呈现方案** — 布局、图形语言、动效、层级、颜色取自既有 token 的呈现方式。前提:不改数值、不改 schema/存档、不新增或改写 `lib/shared/strings.dart` 的玩家可见文案。
2. **实现路线选择(A/B 型)** — 同一需求的两三条技术路线之间选一条。前提:不改数值、schema/存档、公共 API 契约,且所选路线不扩大既有玩家可见语义。
3. **范围分段** — 把一个总项拆成"当前生产链可达的前段"+"依赖未满足的后段"。前提:**总项不得标记完成**,后段必须写明前置条件与归属单号,且必须是分批而非削减(见 §2.8:分批实装合法,切掉范围塞 backlog 不合法)。

**预授权失效的情形——回到 §10 停下问:**

- 你想采用的方案**偏离你自己给出的推荐**(预授权授的是"按推荐执行",不是"随便选")
- 触及 §10 任一红线,或 §7 任一禁区文件
- 三类之间有交叉且你判断不清属于哪类
- 该决策会改变已冻结的内容(例如已拍板的 camera 规则、威胁分类、既有玩家可见契约)

依据:2026-08-29 昨晚三次停下(屏外表现方案、屏外架构路线 A、聚合伤害两段式范围)用户全部批准了你的推荐方案,故将这三类转为预授权。
