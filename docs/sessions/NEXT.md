...
| # | 任务 | 模型 | 预估时长 | 备注 |
|---|------|------|----------|------|
| 0 | **拍 BACKLOG 一#15：老存档角色资质永久停留「标准」（推荐）** | 用户 + opus | 10min 拍板 + 30-60min 实装 | 🔴 上一会话「出生锁死」拍板的直接后遗症：加载时不重算，故 08-08 前的存档角色永久带旧写死值。**比下面几项更该先看**，因为它影响已有玩家的可见状态；三个选项与各自误差已写在 BACKLOG。⚠ 属读码推断，未拿真档验证 |
| 1 | 拍 P7 扫描器修复合不合 | opus | 5-10min | **补丁已写好并全验证**（死链 940→909、工作树漂移 17→0、既有 10 类样例 10/10、总数守恒），提案在 `docs/dispatch/reports/2026-08-08_scanner_fp_fix_proposal.md`，用户点头即可合，是所有候选里性价比最高的 |
