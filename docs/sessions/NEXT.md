...
| # | 任务 | 模型 | 预估时长 | 备注 |
|---|------|------|----------|------|
| 0 | 拍 BACKLOG 一#15：老存档角色档案页资质停留「标准」 | 用户 + opus | 10min 拍板 + 30-60min 实装 | 上一会话「出生锁死」拍板的后遗症，但**二次审计后严重性已下调=纯展示零数值影响**：`character.rarity` 唯一读取点是角色档案页，招募卡是现算不读存档，战斗/成长/掉落层零消费。三选项与误差见 BACKLOG。⚠ 读码推断未拿真档验证 |
