...
| 12 | 战报格式化片段是否列为第 4 合法集中 sink | 设计拍板 | 2026-07-25 门禁实装时暴露:`damage_calculator.dart` 的伤害明细片段(`'(凝甲,原…)'`/`'(弱点/抗性)'`/`' + 震伤 …'`)与 `damage_popup.dart` 解析用正则 `r'^暴击 (\d+) 伤害$'`,性质同已正名的 `battle_log.dart`(集中格式化 ≠ 散写),但不在 CLAUDE §5.6 的三 sink 名单。拍板点=列为第 4 sink(改扫描器豁免·6 条出 allowlist)/ 视为散写(迁 UiStrings)/ 维持现状(留 allowlist) |
