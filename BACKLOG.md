...
2026-09-06 实测缺陷准入，优先顺序 7 → 9 → 8；本批只取证，不把它们记为已修复。

| # | 项 | 生产证据与收口条件 |
|---|---|---|
| 7 | 自动战斗近身停滞（M2 主 WIP） | 2026-09-06 同一合法暗器档/黑风岭/seed：生产 Bot 300 秒仍 ongoing；测试指令进入射程停止折返并精确瞄准后 30.8 秒全胜。先修移动与瞄准配合，再覆盖五武器、边缘与三战术；不改射程、伤害或战斗规则。证据：`docs/audit/phase2_blocker_verification_2026-09-06.md`。 |
| 8 | 减少闪光未接通受击表现（M8） | 同一真实 Boss tick 47 命中时，开关均为 alpha 0.76、100ms 有/600ms 无。需接通生产消费并证明命中、伤害及领域状态一致；前置 M2–M6 正式依赖仍不因该修复关闭。证据同上。 |
| 9 | 密集战斗原生音频任务悬挂与性能（M4） | 两视口带声/静音 Profile 均未过帧门；真实 AudioPlayersBackend 带声反复出现 Swift continuation misuse。先隔离播放器池并发与内存增长，再做双视口 3×60 秒及长时复验；不能以 MP3 解码/假后端通过替代，不默认升级 SDK 或依赖。长时结果见同上报告。 |
