...
| 10 | numbers.yaml `inheritance.unlock_rules.can_take_disciple_at` 归属 | 配置消费/拍板 | 5A I 段：`disciple_joins` 在 Ch6（sanLiu）早于 yiLiu 故原 I-A 守卫必红；但 tutorial step 6 收徒门禁硬编码 `RealmTier.yiLiu`（`tutorial_service.dart:100`）是真门禁。推荐 I-A′=该 hook 改读本键（值不变）；选项表见 `docs/audit/numbers_unused_keys_pending_decision_2026-09-17.md` 末节。 |

## 二 · 已解锁可派

2026-09-06 修复候选：#7 自动战斗近身停滞、#8 减少闪光生产消费已接通并定向验证；#9 的原生音频悬挂/内存增长已完成真实带声十分钟复验，密集场景帧耗继续保留。集成记录见 `docs/audit/phase2_blocker_fixes_2026-09-06.md`，正式里程碑不因这些缺陷修复自动关闭。

| # | 项 | 生产证据与收口条件 |
|---|---|---|
| 9 | 密集战斗帧耗未达标（M4） | 当前代码 `57f95bc7f` 带声十分钟 RSS 285→320 MiB、原生告警 0，原悬挂/增长反例闭合；1280/1440 帧门仍红，静音对照也红。下一步用 CPU/raster 证据区分生产表现与密度夹具反复重建成本，再对双视口 3×60 秒复验；不降负载/放宽门槛，不默认升级 SDK。真人听感、物理 Windows 与 72h 仍开放。 |
