...
> Codex 启动必读。本文件用最小篇幅让你立刻能在本项目中正确工作。
...
> 注：装备攻击系数（早期 ×8）/ 血量内力系数（早期 ×5）/ 根骨系数（早期 ×500）均为 Phase 1 平衡前口误值，平衡 + P0.1 #38 方案 D 后以 `numbers.yaml` 为准（装备攻击 1.0 / 内力 0.5 / 根骨 400）。详 GDD §5.3/§5.6。
...
| 端 | 工具 | 写什么 | 不写什么 |
|---|---|---|---|
| Mac | Codex + Opus 4.7 | `lib/`、`data/*.yaml`（根目录单文件）、`test/`、`GDD.md` | `data/narratives/` `data/lore/` `data/events/` 下任何文件 |
| Windows | Codex + DeepSeek | `data/narratives/` `data/lore/` `data/events/` | 任何 Dart 代码、yaml 数值 |
...
> 以下条目在 GDD v1.0 中尚未明确。Codex 实现到这些位置时**必须停下来问人类**，不要凭训练数据自行补脑。
> 决策定下后从本表移除并同步进 GDD。
