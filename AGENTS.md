...
买断制、写实武侠挂机游戏(发布目标 Windows,开发/验收在 macOS)。Flutter Desktop,3v3 自动战斗 + 离线挂机(⚠ 2026-08-19 漂移指针:战斗终态已拍板 = Phase 0A 单角色 ARPG 替换,§7.4 ADR 路线 C,口径同步随 ADR 执行批)。当前阶段:**1.0 长线打磨期**(质量优先,不设上线时间压力 · 详 CLAUDE.md §7 / GDD §1)。
...
- **数值红线**:配置基础表值硬守（玩家血 ≤20,000 / Boss 血 <1M 且当前红线 60,000+ / 内力 ≤15,000 / 装备基础攻击 ≤2,000 / 招式倍率全局 ≤8,000）；实战极值伤害软线为不进百万，详 CLAUDE.md §5.4 / GDD §5.2。
