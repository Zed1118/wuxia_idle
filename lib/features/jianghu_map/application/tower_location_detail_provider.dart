...
final towerLocationDetailProvider = FutureProvider<TowerLocationDetail>(
  (ref) async {
    ...
    if (isar == null) {
      throw StateError(
        'Tower location detail unavailable: Isar not initialized',
      );
    }
    ...
    final candidates = await ref.watch(
      towerParticipantCandidatesProvider.future,
    );
    final eligibleParticipantCount = candidates
  ...
  dependencies: [
    isarProvider,
    towerProgressProvider,
    towerParticipantCandidatesProvider,
  ],
);
