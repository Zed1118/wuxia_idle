...
///
/// **数值取值**(T19b 技术债清账):
///   - 已升 [PvpDef] 强类型(spec §9 挂账原"Phase 4+ 升强类型"清账);
///     `numbers.pvp.elo.kFactor` / `numbers.pvp.matchRange.eloWindow` 直接读。
///
...
class PvpService {
  ...
  Future<PvpRecord> match({
    ...
    final kFactor = numbers.pvp.elo.kFactor;
  ...
  String _newMatchId(Random? rng) {
    final r = rng ?? Random();
    final ts = clock.now().microsecondsSinceEpoch.toRadixString(16);
